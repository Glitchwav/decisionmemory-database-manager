...
def update_semantic_from_decision(
    db: Database,
    symbol: str,
    strategy_name: str,
    pnl: float,
    ...
    """Update semantic memories with Bayesian evidence from a new decision.
    ...
    After Bayesian update, checks episodic drift: if the recent 20 decisions'
    ...
    if existing:
        ...
        db.insert_semantic({
            ...
            "last_confirmed": decision_id if confirmed else None,
            "last_contradicted": None if confirmed else decision_id,
...
def _compute_kelly_simple(win_rate: float, avg_win: float, avg_loss: float) -> Optional[float]:
    ...
    Args:
        win_rate: proportion of winning decisions [0, 1]
...
def update_procedural_from_decision(
    db: Database,
    symbol: str,
    strategy_name: str,
    pnl: float,
    ...
    """Update procedural memory with running averages from a new decision.
    ...
    if existing:
        ...
        episodic_decisions = db.query_episodic(strategy=strategy_name, limit=50)
        ep_same_symbol = [
            ep for ep in episodic_decisions
            if (ep.get("context_json") or {}).get("symbol") == symbol
        ]
        ...
        db.upsert_procedural({
            ...
            "strategy": strategy_name,
            "symbol": symbol,
            "behavior_type": "decision_execution",
            "sample_size": new_n,
...
def update_affective_from_decision(
    db: Database,
    pnl: float,
    confidence: float,
    ...
    """Update affective state: EWMA confidence, streaks, score/drawdown.
    ...
    current_score = state.get("current_score", 10000.0) + pnl
    peak_score = max(state.get("peak_score", current_score), current_score)
    drawdown_state = (peak_score - current_score) / peak_score if peak_score > 0 else 0.0
