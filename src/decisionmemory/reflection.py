...
class ReflectionEngine:
    """
    ReflectionEngine analyzes decision patterns and generates insights.
    Supports daily, weekly, and monthly reflection cycles.
    """

    def __init__(self, journal: Optional[DecisionJournal] = None):
        """
        Initialize ReflectionEngine.

        Args:
            journal: DecisionJournal instance (creates new if None)
        """
    ...
    def generate_daily_summary(
        ...
        """
        Generate daily decision_making summary with AI reflection.
        ...
        decisions = self._get_decisions_for_date(target_date)

        if len(decisions) == 0:
            return self._format_no_decisions_summary(target_date)

        # Calculate performance metrics
        metrics = self._calculate_daily_metrics(decisions)

        # Generate summary (LLM or rule-based)
        if llm_provider:
            return self._generate_llm_summary(target_date, decisions, metrics, llm_provider)
        else:
            return self._generate_rule_based_summary(target_date, decisions, metrics)

    def _get_decisions_for_date(self, target_date: date) -> List[DecisionRecord]:
        """Get all decisions for a specific date (UTC timezone)"""
        # Query all recent decisions
        all_decisions = self.journal.query_history(limit=1000)
        ...
        for decision in all_decisions:
            ...
            if isinstance(decision.timestamp, str):
                decision_date_str = decision.timestamp[:10]  # YYYY-MM-DD
            else:
                decision_date_str = decision.timestamp.date().isoformat()

            if decision_date_str == target_str:
                date_decisions.append(decision)

        return date_decisions

    def _calculate_daily_metrics(self, decisions: List[DecisionRecord]) -> Dict[str, Any]:
        """Calculate performance metrics for a set of decisions"""
        total = len(decisions)
        winners = sum(1 for t in decisions if t.pnl and t.pnl > 0)
        losers = sum(1 for t in decisions if t.pnl and t.pnl < 0)
        breakeven = total - winners - losers

        total_pnl = sum(t.pnl for t in decisions if t.pnl is not None)
        win_rate = (winners / total * 100) if total > 0 else 0.0

        # Average R-multiple
        r_values = [t.pnl_r for t in decisions if t.pnl_r is not None]
        avg_r = sum(r_values) / len(r_values) if r_values else 0.0

        # Average confidence
        avg_confidence = sum(t.confidence for t in decisions) / total if total > 0 else 0.0
    ...
    def _format_no_decisions_summary(self, target_date: date) -> str:
        """Format summary when no decisions occurred"""
...
PERFORMANCE:
Decisions: {metrics['total']} | Winners: {metrics['winners']} | Losers: {metrics['losers']}
...
"""
        ...
        # Mistakes (identify losing decisions with high confidence)
        mistakes = []
        for decision in decisions:
            if decision.pnl and decision.pnl < 0 and decision.confidence > 0.75:
                mistakes.append(f"{decision.id}: High confidence ({decision.confidence:.2f}) but lost ${abs(decision.pnl):.2f}")
    ...
    def _generate_llm_summary(
        ...
        # Prepare decisions JSON
        decisions_json = json.dumps(
            [t.model_dump() for t in decisions],
            indent=2,
            default=str
        )
...
- 不要廢話，不要鼓勵性語言
- 只寫有數據支撐的觀察
- 如果今天沒有交易，只寫 "No decisions today."
...
"""
    ...
    def _get_decisions_for_date_range(
        self, start_date: date, end_date: date
    ) -> List[DecisionRecord]:
        """Get all decisions within a date range (inclusive, UTC timezone)."""
        all_decisions = self.journal.query_history(limit=10000)
        ...
        for decision in all_decisions:
            if isinstance(decision.timestamp, str):
                decision_date_str = decision.timestamp[:10]
            else:
                decision_date_str = decision.timestamp.date().isoformat()
            if start_str <= decision_date_str <= end_str:
    ...
    def generate_weekly_summary(
        ...
        """
        Generate weekly decision_making summary.
        ...
        decisions = self._get_decisions_for_date_range(week_start, week_ending)

        if len(decisions) == 0:
            return self._format_no_decisions_weekly(week_ending)

        metrics = self._calculate_weekly_metrics(decisions, week_start, week_ending)

        if llm_provider:
            return self._generate_llm_weekly_summary(
                week_ending, decisions, metrics, llm_provider
            )
        return self._generate_rule_based_weekly_summary(
            week_ending, decisions, metrics
        )
    ...
    def _calculate_weekly_metrics(
        ...
        base = self._calculate_daily_metrics(decisions)

        # --- Strategy breakdown ---
        strategy_map: Dict[str, List[DecisionRecord]] = defaultdict(list)
        for t in decisions:
            strategy_map[t.strategy].append(t)

        strategy_breakdown: Dict[str, Dict[str, Any]] = {}
        for strat, strat_decisions in strategy_map.items():
            total = len(strat_decisions)
            winners = sum(1 for t in strat_decisions if t.pnl and t.pnl > 0)
            total_pnl = sum(t.pnl for t in strat_decisions if t.pnl is not None)
        ...
        # --- Session patterns ---
        session_map: Dict[str, List[DecisionRecord]] = defaultdict(list)
        for t in decisions:
            sess = (
        ...
        for sess, sess_decisions in session_map.items():
            total = len(sess_decisions)
            winners = sum(1 for t in sess_decisions if t.pnl and t.pnl > 0)
            total_pnl = sum(t.pnl for t in sess_decisions if t.pnl is not None)
        ...
        dow_map: Dict[str, List[DecisionRecord]] = defaultdict(list)
        ...
        best_day = ("", -float("inf"))
        worst_day = ("", float("inf"))
        for day_name, day_decisions in dow_map.items():
            total_pnl = sum(t.pnl for t in day_decisions if t.pnl is not None)
            total = len(day_decisions)
            winners = sum(1 for t in day_decisions if t.pnl and t.pnl > 0)
        ...
        # --- Streak analysis ---
        sorted_decisions = sorted(decisions, key=lambda t: t.timestamp if isinstance(t.timestamp, str) else t.timestamp.isoformat())
        max_win_streak = 0
        max_loss_streak = 0
        current_win = 0
        current_loss = 0
        ...
        win_pnls = [t.pnl for t in decisions if t.pnl and t.pnl > 0]
        loss_pnls = [t.pnl for t in decisions if t.pnl and t.pnl < 0]
        ...
        best_decision = max((t.pnl for t in decisions if t.pnl is not None), default=0.0)
        worst_decision = min((t.pnl for t in decisions if t.pnl is not None), default=0.0)
    ...
    def _format_no_decisions_weekly(self, week_ending: date) -> str:
        week_start = week_ending - timedelta(days=6)
...
"""
    ...
    def _generate_llm_weekly_summary(
        ...
        week_start = week_ending - timedelta(days=6)
        decisions_json = json.dumps(
            [t.model_dump() for t in decisions], indent=2, default=str
        )
...
"""
        ...
        try:
            ...
            if self._validate_weekly_llm_output(response, week_ending):
                return response
            return self._generate_rule_based_weekly_summary(week_ending, decisions, metrics) + \
                   "\n(LLM output failed validation, using rule-based fallback)\n"
        except Exception as e:
            return self._generate_rule_based_weekly_summary(week_ending, decisions, metrics) + \
                   f"\n(LLM failed: {str(e)}, using rule-based fallback)\n"
    ...
    def generate_monthly_summary(
        ...
        """
        Generate monthly decision_making summary.
        ...
        decisions = self._get_decisions_for_date_range(start_date, end_date)

        if len(decisions) == 0:
            return self._format_no_decisions_monthly(year, month)

        metrics = self._calculate_monthly_metrics(decisions, start_date, end_date)

        if llm_provider:
            return self._generate_llm_monthly_summary(
                year, month, decisions, metrics, llm_provider
            )
        return self._generate_rule_based_monthly_summary(year, month, decisions, metrics)
    ...
    def _calculate_monthly_metrics(
        ...
        base = self._calculate_weekly_metrics(decisions, start_date, end_date)

        # --- DecisionMaking days ---
        decision_dates = set()
        for t in decisions:
            if isinstance(t.timestamp, str):
                decision_dates.add(t.timestamp[:10])
            else:
                decision_dates.add(t.timestamp.date().isoformat())
        decision_making_days = len(decision_dates)
        avg_decisions_per_day = base["total"] / decision_making_days if decision_making_days > 0 else 0.0
        ...
        while current <= end_date:
            ...
            w_decisions = [
                t for t in decisions
                if current.isoformat() <= (
                    t.timestamp[:10] if isinstance(t.timestamp, str) else t.timestamp.date().isoformat()
                ) <= w_end.isoformat()
            ]
            if w_decisions:
                total = len(w_decisions)
                winners = sum(1 for t in w_decisions if t.pnl and t.pnl > 0)
                total_pnl = sum(t.pnl for t in w_decisions if t.pnl is not None)
        ...
        all_strategies = set(t.strategy for t in decisions)
        for strat in all_strategies:
            ...
            strategy_evolution[strat] = {
                "first_half_decisions": fh_total,
                "first_half_wr": fh_wr,
                "second_half_decisions": sh_total,
                "second_half_wr": sh_wr,
                "direction": direction,
            }

        base.update({
            "decision_making_days": decision_making_days,
    ...
    def _format_no_decisions_monthly(self, year: int, month: int) -> str:
        return f"""=== MONTHLY SUMMARY: {year}-{month:02d} ===

PERFORMANCE:
No decisions this month.
...
Decisions: {metrics['total']} | Winners: {metrics['winners']} | Losers: {metrics['losers']}
Net P&L: ${metrics['total_pnl']:.2f} | Win Rate: {metrics['win_rate']:.1f}% | Avg R: {metrics['avg_r']:.2f} | Profit Factor: {pf_str}
DecisionMaking Days: {metrics['decision_making_days']} | Avg Decisions/Day: {metrics['avg_decisions_per_day']:.1f}
...
"""
        ...
        if metrics["weekly_trends"]:
            ...
            for wt in metrics["weekly_trends"]:
                summary += (
                    ...
                    f"{wt['decisions']} decisions, WR {wt['win_rate']:.1f}%, P&L ${wt['pnl']:.2f}\n"
                )
        ...
        if metrics["strategy_evolution"]:
            ...
            for strat, evo in metrics["strategy_evolution"].items():
                summary += (
                    ...
                    f"({evo['first_half_decisions']} decisions) -> "
                    f"2nd half WR {evo['second_half_wr']:.1f}% "
                    f"({evo['second_half_decisions']} decisions) "
                    f"[{evo['direction']}]\n"
                )
            summary += "\n"
        ...
        for strat, evo in metrics["strategy_evolution"].items():
            if evo["direction"] == "declining" and evo["first_half_decisions"] >= 3:
...
"""
    ...
    def discover_patterns_from_backtest(
        ...
        """
        Auto-discover L2 patterns from backtest data in decision_records.
    ...
    def _get_backtest_date_range(conn) -> str:
        """Get date range string from decision_records."""
        row = conn.execute(
            "SELECT MIN(timestamp), MAX(timestamp) FROM decision_records"
        ).fetchone()
    ...
    def _detect_symbol_fit(
        ...
        # Single-symbol specialization: detect strategies that only decision
        # one symbol while others decision multiple.  Compare the solo strategy's
