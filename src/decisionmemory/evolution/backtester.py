...
- Computes FitnessMetrics from decision log
"""
...
def backtest(
    ...
    return _compute_fitness(decisions, timeframe=timeframe, annualize=annualize)
...
def _compute_fitness(decisions: List[Decision], timeframe: str = "1h", annualize: bool = True) -> FitnessMetrics:
    """Compute FitnessMetrics from decision log."""
    if not decisions:
        return FitnessMetrics()
    ...
    win_count = len(wins)
    win_rate = win_count / decision_count if decision_count > 0 else 0
    ...
    avg_decision_pnl = total_pnl / decision_count if decision_count > 0 else 0
    avg_holding = sum(t.holding_bars for t in decisions) / decision_count if decision_count > 0 else 0

    # Sharpe ratio (annualized using timeframe-appropriate factor)
    # Guard: too few decisions produce extreme/meaningless Sharpe values
    if decision_count < MIN_DECISIONS_FOR_SHARPE:
        sharpe = 0.0
    elif len(pnls) > 1:
    ...
    return FitnessMetrics(
        ...
        avg_decision_pnl=round(avg_decision_pnl, 4),
...
def _compute_max_drawdown(pnls: List[float]) -> float:
    """Compute max drawdown as percentage of peak score."""
    if not pnls:
        return 0
...
def _max_consecutive_losses(pnls: List[float]) -> int:
    """Count maximum consecutive losing decisions."""
    max_streak = 0
    current = 0
    for p in pnls:
        if p <= 0:
