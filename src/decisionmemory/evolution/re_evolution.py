...
from decisionmemory.data.models import OHLCV, OHLCVSeries
...
from decisionmemory.evolution.statistical_gates import deflated_sharpe_ratio
...
class ReEvolutionConfig:
    ...
    min_is_decisions: int = 10  # minimum decisions in IS window
    min_is_sharpe: float = 0.0  # minimum IS Sharpe to be viable
    min_oos_decisions: int = 5  # minimum decisions in OOS window
...
class ReEvolutionPipeline:
    ...
    def run(
        ...
        viable = [
            ...
            and c.is_fitness.decision_count >= cfg.min_is_decisions
        ...
        # Step 6: Select best by OOS Sharpe (with min decision filter)
        oos_viable = [
            c
            for c in top_n
            if c.oos_fitness is not None
            and c.oos_fitness.decision_count >= cfg.min_oos_decisions
        ...
        dsr, p_value = deflated_sharpe_ratio(
            ...
            num_obs=best.oos_fitness.decision_count,
        )
        result.dsr = dsr
...
class LLMReEvolutionPipeline:
    ...
    Usage:
        ...
        from decisionmemory.evolution.llm import AnthropicClient
    ...
    def __init__(
        ...
        self.backtest_fn = backtest_fn
        self.min_oos_decisions = min_oos_decisions
    ...
    async def run(
        ...
        """
        from decisionmemory.evolution.engine import EvolutionEngine
        ...
        if oos_fitness.decision_count < self.min_oos_decisions or oos_fitness.sharpe_ratio <= 0:
            result.reason = (
                f"OOS not viable: Sharpe={oos_fitness.sharpe_ratio:.4f}, "
                f"decisions={oos_fitness.decision_count}"
            )
            logger.info(result.reason)
            return result
        ...
        dsr, p_value = deflated_sharpe_ratio(
            ...
            num_trials=max(cumulative_trials, 1),
            num_obs=oos_fitness.decision_count,
        )
        result.dsr = dsr
        result.dsr_pvalue = p_value
        result.passed_dsr_gate = dsr > 0
