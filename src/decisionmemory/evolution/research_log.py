...
from decisionmemory.evolution.models import EvolutionRun, Hypothesis
...
def _format_fitness_row(h: Hypothesis) -> str:
    ...
    is_decisions = str(h.fitness_is.decision_count) if h.fitness_is else "—"
    ...
    return f"| {name} | {gen} | {status} | {is_sharpe} | {is_wr} | {is_decisions} | {oos_sharpe} | {oos_wr} | {reason} |"
...
def format_experiment_log(run: EvolutionRun, experiment_id: Optional[str] = None) -> str:
    ...
    lines = [
        ...
        "### Results",
        "",
        "| Strategy | Gen | Status | Sharpe IS | WR IS | Decisions IS | Sharpe OOS | WR OOS | Elimination |",
        "|----------|-----|--------|-----------|-------|-----------|------------|--------|-------------|",
    ]
