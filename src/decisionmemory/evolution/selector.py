...
def rank_by_is_fitness(
    ...
    for h in with_fitness:
        f = h.fitness_is
        if f.decision_count < cfg.min_is_decision_count:
            continue
...
def validate_oos(
    ...
    if f.decision_count < cfg.min_oos_decision_count:
        return False, f"OOS decision_count={f.decision_count} < {cfg.min_oos_decision_count}"
...
def _is_elimination_reason(h: Hypothesis, cfg: SelectionConfig) -> str:
    ...
    reasons = []
    if f.decision_count < cfg.min_is_decision_count:
        reasons.append(f"decision_count={f.decision_count}")
    if f.sharpe_ratio < cfg.min_is_sharpe:
