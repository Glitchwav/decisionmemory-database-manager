...
Anti-false-positive:
- min_decisions = 20 before any assessment
- 2/3 signals must fire simultaneously
- cooling_period = 5 decisions after trigger
...
class DecisionResult:
    """Single live decision result for decay tracking."""
    is_win: bool
    pnl: float
    # Market features at decision time (for OOD detection)
...
class RegimeDetectorConfig:
    ...
    dd_min_decisions: int = 10  # min decisions in drawdown period
    # S3: Market OOD
...
class RegimeDecayDetector:
    ...
    # Internal state
    _decisions: List[DecisionResult] = field(default_factory=list)
    ...
    _last_trigger_decision_idx: int = field(default=-100)

    def add_decision(self, decision: DecisionResult) -> None:
        """Add a live decision result."""
    ...
    def assess(self) -> DecayAssessment:
        """Run full decay assessment on accumulated decisions."""
        total = len(self._decisions)

        if total < self.config.min_decisions:
            return DecayAssessment(
                decay_confirmed=False,
                signals_fired=0,
                signals=[],
                total_decisions=total,
                reason=f"Insufficient decisions: {total} < {self.config.min_decisions}",
            )

        # Check cooling period
        decisions_since_trigger = total - self._last_trigger_decision_idx
        if 0 < decisions_since_trigger <= self.config.cooling_period:
            return DecayAssessment(
                decay_confirmed=False,
                signals_fired=0,
                signals=[],
                total_decisions=total,
                in_cooling=True,
                reason=f"In cooling period: {decisions_since_trigger}/{self.config.cooling_period} decisions since last trigger",
            )
        ...
        if confirmed:
            self._last_trigger_decision_idx = total
    ...
    def _check_win_rate_decay(self) -> DecaySignal:
        ...
        # Prior from backtest
        bt_wins = int(self.baseline.win_rate * self.baseline.decision_count)
        bt_losses = self.baseline.decision_count - bt_wins

        # Posterior = Prior + live data
        live_wins = sum(1 for t in self._decisions if t.is_win)
        live_losses = len(self._decisions) - live_wins
    ...
    def _check_drawdown_exceedance(self) -> DecaySignal:
        ...
        for t in self._decisions:
            ...
            if score > peak:
                ...
                dd_decision_count = 0
            if peak > 0:
                current_dd = ((peak - score) / peak) * 100
            elif peak == 0 and score < 0:
                # Use absolute PnL as proxy
                current_dd = abs(score)
            else:
                current_dd = 0.0
        ...
        threshold = self.baseline.max_drawdown_pct * self.config.dd_multiplier
        fired = max_dd_pct > threshold and dd_decision_count >= self.config.dd_min_decisions
        ...
        return DecaySignal(
            ...
            detail=f"DD={max_dd_pct:.2f}% vs threshold={threshold:.2f}%, "
                   f"decisions_in_dd={dd_decision_count}",
        )
    ...
    def _check_market_ood(self) -> DecaySignal:
        ...
        # Compute mean Mahalanobis distance of recent decisions with features
        recent_with_features = [
            t for t in self._decisions
        ...
        if not recent_with_features:
            return DecaySignal(
                name="market_ood",
                fired=False, value=0.0, threshold=0.0,
                detail="No decisions with market features",
            )

        # Use last 10 decisions (or all if fewer)
