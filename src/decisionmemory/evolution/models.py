...
Hypothesis: A testable decision_making strategy with entry/exit rules.
...
class RuleCondition(BaseModel):
    """Single condition in a decision_making rule.
...
class CandidatePattern(BaseModel):
    ...
    sample_count: int = 0  # how many bars/decisions LLM based this on
...
class FitnessMetrics(BaseModel):
    ...
    def is_viable(self) -> bool:
        """Minimum viability: enough decisions + positive expectancy."""
        return self.decision_count >= 30 and self.sharpe_ratio > 0
...
class Hypothesis(BaseModel):
    """A testable decision_making strategy = CandidatePattern + fitness results.
