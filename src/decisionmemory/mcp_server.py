"""
DecisionMemory MCP Server — Memory system for AI decision_making agents.

Exposes decision memory operations as MCP tools via FastMCP.
...
from .embedding import embed_decision_context, get_embedding_backend
...
from .owm_helpers import (
    ...
    update_semantic_from_decision,
)

logger = logging.getLogger(__name__)

mcp = FastMCP("decisionmemory-protocol")
...
def _parse_stored_embedding(raw) -> Optional[List[float]]:
    """Parse a stored embedding from DB (JSON string or list) into a list of floats.

    Returns None if the embedding is missing, empty, or malformed.
    """
    if raw is None:
        return None
...
def _build_ref_evidence(
    ...
    """Hydrate ref decision IDs into (pnl_r, direction) tuples for consonance.

    Refs that can't be loaded (missing decision or DB error) are silently skipped —
    a missing ref contributes no evidence, which is the correct behaviour.
    """
    evidence: List[Dict[str, Any]] = []
    for ref_id in ref_ids or []:
        try:
            ref_decision = database.get_decision(ref_id)
        except Exception:
            continue
        if not ref_decision:
            continue
        evidence.append({
            "pnl_r": ref_decision.get("pnl_r"),
            "direction": ref_decision.get("direction"),
        })
    return evidence
...
#   - store_decision_memory → use remember_decision (writes all 5 memory layers)
#   - recall_similar_decisions → use recall_memories (OWM scoring + drift)
# ---------------------------------------------------------------------------
...
async def get_strategy_performance(
    ...
    # Only count closed decisions (have pnl)
    closed = [t for t in decisions if t.get("pnl") is not None]
    ...
    if not closed:
        return {
            ...
            "decision_count": 0,
            "message": "No closed decisions found",
        }
    ...
    strategies = {}
    for strat, strat_decisions in by_strategy.items():
        pnls = [t["pnl"] for t in strat_decisions]
        winners = [p for p in pnls if p > 0]
        losers = [p for p in pnls if p <= 0]
        total_pnl = sum(pnls)

        best = max(strat_decisions, key=lambda t: t["pnl"])
        worst = min(strat_decisions, key=lambda t: t["pnl"])

        strategies[strat] = {
            "decision_count": len(strat_decisions),
            "win_rate": round(len(winners) / len(strat_decisions) * 100, 1),
            "total_pnl": round(total_pnl, 2),
            "avg_pnl": round(total_pnl / len(strat_decisions), 2),
...
@mcp.tool()
async def get_decision_reflection(
    decision_id: str,
) -> dict:
    """Get the full context and reflection for a specific decision.

    Use this to deep-dive into a particular decision's reasoning and lessons.
    ...
    return {
        ...
        "exit_reasoning": decision.get("exit_reasoning"),
...
async def remember_decision(
    ...
    """Store a decision into OWM multi-layer memory with automatic updates.
    ...
    (EWMA confidence/streaks). Also writes to decision_records for backward
    compatibility.

    Args:
        symbol: DecisionMaking instrument (e.g. "XAUUSD")
        direction: "long" or "short"
        entry_price: Entry price of the decision
        exit_price: Exit price of the decision
        ...
        reflection: Lessons learned from this decision
        max_adverse_excursion: Maximum adverse excursion during the decision
        decision_id: Optional custom ID. Auto-generated if omitted.
    ...
    tid = decision_id or f"owm-{uuid.uuid4().hex[:12]}"
    ...
    # Compute DQS (best-effort — don't block decision storage on failure)
    try:
        ...
        logger.warning(f"DQS computation skipped for decision {tid}: {e}")
    ...
    update_semantic_from_decision(db, symbol_upper, strategy_name, pnl, pnl_r, context_regime, tid)
    update_procedural_from_decision(
        db, symbol_upper, strategy_name, pnl,
        hold_duration_seconds=episodic_data.get("hold_duration_seconds"),
        pnl_r=pnl_r,
    )
    update_affective_from_decision(db, pnl, confidence, strategy_name=strategy_name, symbol=symbol_upper)

    # 3) Backward compatibility: also store in decision_records
    decision_data = {
        "id": tid,
        "timestamp": ts,
        "symbol": symbol_upper,
    ...
    try:
        ...
        embedding = embed_decision_context(embed_input)
        if embedding is not None:
            db.update_episodic_embedding(tid, embedding)
            logger.info(f"Embedding stored for decision {tid} (dim={len(embedding)})")
    except Exception as e:
        logger.warning(f"Embedding generation skipped for decision {tid}: {e}")
    ...
    return {
        ...
        "strategy": strategy_name,
        "stored_at": ts,
        "memory_layers": ["episodic", "semantic", "procedural", "affective", "decision_records"],
        "status": "stored",
    }
    ...
    if "episodic" in memory_types:
        ...
        for ep in episodic:
            ...
            candidates.append({
                ...
                "embedding": _parse_stored_embedding(ep.get("embedding")),
            })
            ...
            if query_embedding is not None:
                for c in candidates:
                    ...
                    try:
                        ...
                        # Persist for future recalls — avoids O(N) recomputation
                        if c.get("memory_type") == "episodic" and c.get("id"):
                            try:
                                db.update_episodic_embedding(c["id"], c["embedding"])
                            except Exception:
                                pass  # Non-fatal — embedding still usable this session
                    except Exception:
...
async def get_behavioral_analysis(
    ...
    Returns aggregate decision_making behavior stats: hold times, disposition ratio,
...
async def get_agent_state() -> dict:
    ...
    win/loss streaks, score tracking, and a recommended action
    ...
    if drawdown_pct > 0.6:
        recommended_action = "stop_decision_making"
...
@mcp.tool()
async def create_decision_making_plan(
    trigger_type: str,
    ...
    """Create a prospective decision_making plan that activates when conditions are met.
    ...
    Args:
        ...
        planned_action: JSON string describing what to do (e.g. '{"type": "skip_decision"}')
    ...
    return {
        ...
        "status": "active",
        "expiry": expiry,
        "message": f"DecisionMaking plan created: {trigger_type} → {planned_act_parsed.get('type', 'action')}",
    }
...
async def check_active_plans(
    ...
    """Check active decision_making plans against current market context.
...
async def evolution_fetch_market_data(
    ...
    Args:
        symbol: DecisionMaking pair (e.g. "BTCUSDT", "ETHUSDT")
...
async def evolution_discover_patterns(
    ...
    """Discover decision_making patterns from market data using LLM analysis.

    Uses Claude to analyze OHLCV data and generate candidate decision_making patterns
    with entry/exit conditions. Each pattern can be backtested afterward.

    Args:
        symbol: DecisionMaking pair (e.g. "BTCUSDT")
        timeframe: Bar timeframe — "5m", "15m", "1h", "4h", "1d"
...
async def evolution_run_backtest(
    ...
    backtest. Returns fitness metrics: Sharpe ratio, win rate, decision count,
...
# =====================================================================
# Audit tools — DecisionMaking Decision Records (Phase 2)
# =====================================================================
...
async def export_audit_trail(
    ...
    """Export DecisionMaking Decision Records for audit and compliance review.

    Provides a complete, tamper-evident record of decision_making decisions including
    the memory context (similar decisions, beliefs) that informed each decision.

    Args:
        decision_id: Get a single TDR by decision ID (e.g., "MT5-7047640363").
    ...
    """
    import json as _json
    from .domain.tdr import DecisionMakingDecisionRecord, MemoryContext
    ...
    if decision_id:
        ...
        if not decision:
            return {"error": f"Decision {decision_id} not found", "records": [], "count": 0}
        records = [decision]
    else:
        records = database.query_decisions(strategy=strategy, limit=limit)
        if start or end:
            filtered = []
            for t in records:
                ts = t.get("timestamp", "")
    ...
    for decision in records:
        ...
        beliefs = []
        try:
            sem = database.query_semantic(strategy=decision.get("strategy"), limit=5)
            beliefs = [
        ...
        tdr = DecisionMakingDecisionRecord.from_decision_record(decision, memory_ctx=mem)
...
@mcp.tool()
async def verify_audit_hash(decision_id: str) -> dict:
    """Verify the integrity of a DecisionMaking Decision Record.
    ...
    Args:
        decision_id: Decision ID to verify (e.g., "MT5-7047640363").
    ...
    decision = database.get_decision(decision_id)
    if not decision:
        return {"error": f"Decision {decision_id} not found", "verified": False}

    market_context = decision.get("market_context", {})
    if isinstance(market_context, str):
        market_context = _json.loads(market_context)

    recomputed = DecisionMakingDecisionRecord.compute_hash(
    ...
    # Build TDR from stored decision to get the stored hash
...
async def validate_strategy(
    ...
    """Validate a decision_making strategy using statistical tests (DSR + Walk-Forward + Regime + CPCV).

    For educational and research purposes only. Not decision advice.

    Upload a decision log CSV (QuantConnect format) or daily returns CSV.
    ...
    Args:
        ...
        format: CSV format — "quantconnect" for decision logs (columns: Entry Time, Exit Time,
    ...
    """
    from .strategy_validator import validate_from_decisions, validate_from_returns
    ...
    try:
        if format == "returns":
            ...
            return validate_from_decisions(
                file_path=file_path,
                format=format,
                strategy_name=strategy_name,
                num_strategies=num_strategies,
...
@mcp.tool()
async def check_decision_legitimacy(
    strategy_name: str,
    symbol: str = "XAUUSD",
    current_regime: Optional[str] = None,
    current_atr_d1: Optional[float] = None,
) -> dict:
    """Check if the agent has sufficient data and confidence to decision.

    Call this before making any decision decision. Evaluates sample size,
    memory quality, regime experience, streak state, and drawdown to
    determine whether the agent has earned the right to decision at full size.

    Args:
        strategy_name: Strategy to evaluate (e.g. "VolBreakout").
        symbol: DecisionMaking instrument (default "XAUUSD").
    ...
    # 1. Total decisions for this strategy
    decisions = db.query_decisions(strategy=strategy_name, symbol=symbol, limit=10000)
    memory_count = len(decisions)

    # 2. Regime-specific decision count
    regime_decision_count = 0
    if current_regime:
        episodic = db.query_episodic(strategy=strategy_name, regime=current_regime, limit=10000)
        regime_decision_count = len(episodic)
    ...
    result = compute_legitimacy_score(
        ...
        drawdown_pct=drawdown_pct,
        regime_decision_count=regime_decision_count,
    )
    ...
    result["context"] = {
        ...
        "total_decisions": memory_count,
        "regime_decisions": regime_decision_count,
...
async def compute_dqs(
    ...
    """Compute Decision Quality Score before executing a decision.
...
def main():
    ...
    _data_dir = Path.home() / ".decisionmemory"
    if not (_data_dir / ".setup_done").exists():
        print("First time? Run: decisionmemory setup", file=sys.stderr)
    mcp.run()
