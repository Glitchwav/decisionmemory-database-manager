...
Design
------
- Each TDR has a content hash H_c (DecisionMakingDecisionRecord.compute_hash).
...
class ChainBuilder:
    ...
    def backfill(
        self,
        decision_iter: Iterable[Tuple[str, str, str]],
    ) -> Dict[str, int]:
        ...
        for record_id, content_hash, chained_at in decision_iter:
            data_hash = chained_hash(prev_hash, content_hash)
            self.conn.execute(
                "INSERT INTO audit_chain "
                "(record_id, sequence_num, content_hash, prev_hash, "
