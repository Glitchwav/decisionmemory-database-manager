"""Decision Quality Score (DQS) Engine — process-oriented pre-decision evaluation.
...
Five factors, each 0-2, total 0-10:
  ...
  3. Process Adherence — similarity to past successful decisions (OWM score)
...
class DQSEngine:
    ...
    def set_adaptive_thresholds(self, dqs_scores: List[float]) -> None:
        ...
        - Everything above → go (1.0) — normal decision_making

        This ensures the default action is "decision normally" and only
        statistically anomalous decisions get reduced. Like GTO: deviate
    ...
    def _regime_match(
        ...
        # All decisions for this strategy
        all_decisions = self.db.query_episodic(strategy=strategy, limit=10000)
        if not all_decisions:
            return 1.0, "No history — neutral score"

        total = len(all_decisions)
        wins_total = sum(1 for t in all_decisions if (t.get("pnl") or 0) > 0)
        overall_wr = wins_total / total if total > 0 else 0.5

        # Decisions in current regime
        regime_decisions = self.db.query_episodic(
            strategy=strategy, regime=current_regime, limit=10000
        )
        regime_count = len(regime_decisions)

        if regime_count == 0:
            return 1.0, f"No decisions in '{current_regime}' regime — neutral (not penalized)"

        regime_wins = sum(1 for t in regime_decisions if (t.get("pnl") or 0) > 0)
        ...
        desc = (
            ...
            f"({regime_count} decisions) vs overall WR={overall_wr:.0%}"
        )
        return score, desc
    ...
    def _process_adherence(
        ...
        """Score based on OWM similarity to past successful decisions."""
        ...
        if not scored:
            return 0.5, "No similar decisions found — low but not penalized"
    ...
    def _historical_pattern(
        ...
        """Score based on average pnl_r of similar past decisions."""
        from .context import ContextVector
        from .recall import outcome_weighted_recall
        ...
        if not scored:
            return 1.0, "Insufficient similar decisions — neutral"
        ...
        desc = f"Top {len(pnl_rs)} similar decisions avg pnl_r={avg_pnl_r:.3f}"
        return factor_score, desc
    ...
    def _build_recommendation(
        ...
        return (
            f"{strategy}: DQS {score:.1f}/10 — skip this decision. "
            f"Weak: {warning_text}."
        )
    ...
    def calibrate(self, min_decisions: int = 50) -> dict:
        """Learn factor weights from historical decision outcomes.
        ...
        Args:
            min_decisions: Minimum decisions required for calibration.

        Returns:
            Dict with learned_weights, r_squared, sample_size, and status.
        """
        all_decisions = self.db.query_episodic(limit=100000)
        if len(all_decisions) < min_decisions:
            return {
                "status": "insufficient_data",
                "sample_size": len(all_decisions),
