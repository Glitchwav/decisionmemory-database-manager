...
def migrate_decisions_to_episodic(db) -> int:
    """
    Migrate all decision_records to episodic_memory.

    Mapping:
    - decision id → episodic id
    ...
    try:
        rows = conn.execute("SELECT * FROM decision_records").fetchall()
        count = 0
        ...
        for row in rows:
            ...
            raw_ctx = decision.get("market_context") or "{}"
            try:
            ...
            episodic = {
                ...
                "hold_duration_seconds": decision.get("hold_duration"),
                "max_adverse_excursion": None,
                "reflection": decision.get("lessons"),
...
def initialize_affective(db, score: float = 10000.0) -> bool:
    """
    Create initial affective_state row with given score.
