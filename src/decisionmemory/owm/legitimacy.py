"""Decision Legitimacy Gate — evaluate whether an agent has earned the right to decision."""
...
def _sample_sufficiency(n: int) -> float:
    """Score based on total decision count for this strategy."""
    if n >= 30:
        return 1.0
    if n >= 15:
        return 0.7
...
def _regime_confidence(regime_decision_count: int) -> float:
    """Score based on decisions in the current regime."""
    if regime_decision_count >= 10:
        return 1.0
    if regime_decision_count >= 5:
        return 0.7
    if regime_decision_count >= 2:
        return 0.4
    return 0.1
...
def compute_legitimacy_score(
    ...
    drawdown_pct: float,
    regime_decision_count: int = 0,
) -> dict:
    """Compute whether an agent has 'earned the right' to decision at full confidence.

    Args:
        strategy_name: Name of the strategy being evaluated.
        current_regime: Current market regime (e.g. "trending_up", "ranging").
        memory_count: Total number of decisions for this strategy.
        ...
        regime_decision_count: Number of decisions in the current regime for this strategy.
    ...
    factors = {
        ...
        "regime_confidence": _regime_confidence(regime_decision_count),
...
def _build_recommendation(
    *,
    ...
    if tier == "full":
        return f"{strategy_name}: Full confidence. {memory_count} decisions in memory, conditions favorable."

    warnings: list[str] = []

    if factors["sample_sufficiency"] < 0.7:
        warnings.append(f"only {memory_count} decisions in memory (need 15+ for confidence)")

    if factors["regime_confidence"] < 0.7:
        regime_str = current_regime or "unknown"
        warnings.append(
            f"only {regime_decision_count} decisions in '{regime_str}' regime (need 5+ for confidence)"
        )
    ...
    if tier == "reduced":
        ...
        return f"{strategy_name}: Skip this decision. Issues: {warning_text}."
