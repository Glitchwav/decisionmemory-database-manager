"""
Strategy Validator — Statistical validation engine for decision_making strategies.
...
DISCLAIMER = (
    "DISCLAIMER: Statistical analysis only. Not decision advice. "
    "Past performance is not indicative of future results. "
    "This tool does not execute decisions or provide investment recommendations. "
    "Users are solely responsible for their decision_making decisions."
)
...
def parse_quantconnect_csv(file_path: str) -> list[dict]:
    """Parse QuantConnect decision log CSV.
    ...
    with open(file_path, "r", encoding="utf-8-sig") as f:
        ...
        for i, row in enumerate(reader):
            ...
            try:
                ...
                continue
    decisions.sort(key=lambda t: t["exit_time"])
    return decisions
...
def compute_basic_stats(decisions: list[dict]) -> dict:
    """Compute basic performance statistics from decision log."""
    n = len(decisions)
    if n == 0:
        return {"total_decisions": 0, "error": "No decisions"}

    wins = sum(1 for t in decisions if t["is_win"])
    total_pnl = sum(t["pnl"] for t in decisions)
    ...
    # Direction breakdown
    buy_count = sum(1 for t in decisions if t["direction"] == "Buy")

    # Daily P&L series
    daily_pnl = _daily_pnl_series(decisions)
    ...
    return {
        ...
        "first_date": str(decisions[0]["entry_time"].date()),
        "last_date": str(decisions[-1]["exit_time"].date()),
    }
...
def walk_forward_validation(
    ...
    first_year = decisions[0]["entry_time"].year
    last_year = decisions[-1]["exit_time"].year
    ...
    for w in windows:
        ...
        is_sharpe, is_wr = _sharpe_and_wr(is_decisions)
        oos_sharpe, oos_wr = _sharpe_and_wr(oos_decisions)

        if not oos_decisions:
            verdict = "N/A"
...
def regime_analysis(decisions: list[dict]) -> dict:
    """Analyze performance across market regimes (bull/bear/crisis/range)."""
    if not decisions:
        return {"regimes": {}, "verdict": "NO_DATA"}

    regime_decisions: dict[str, list[dict]] = defaultdict(list)
    for t in decisions:
        year = t["exit_time"].year
        regime = _classify_year(year)
        regime_decisions[regime].append(t)

    regimes = {}
    for regime, r_decisions in regime_decisions.items():
        n = len(r_decisions)
        wins = sum(1 for t in r_decisions if t["is_win"])
        total_pnl = sum(t["pnl"] for t in r_decisions)
        daily = _daily_pnl_series(r_decisions)
...
def validate_from_decisions(
    file_path: str,
    format: str = "quantconnect",
    strategy_name: str = "",
    num_strategies: int = 1,
) -> dict:
    """Full validation pipeline for decision log CSV.
    ...
    try:
        if format == "quantconnect":
            decisions = parse_quantconnect_csv(file_path)
        else:
            return {"error": f"Unsupported decision log format: {format}", "disclaimer": DISCLAIMER}
    except (FileNotFoundError, ValueError) as e:
        return {"error": str(e), "disclaimer": DISCLAIMER}

    if not decisions:
        return {"error": "No valid decisions parsed from CSV", "disclaimer": DISCLAIMER}

    stats = compute_basic_stats(decisions)
    dsr = compute_dsr(stats["sharpe_raw"], stats["daily_observations"], num_strategies)
    wf = walk_forward_validation(decisions)
    regime = regime_analysis(decisions)
...
def _daily_pnl_series(decisions: list[dict]) -> dict:
    """Group decisions by exit date, sum P&L."""
...
def _sharpe_and_wr(decisions: list[dict]) -> tuple[float, float]:
    """Compute Sharpe and win rate for a subset of decisions."""
    if not decisions:
        return 0.0, 0.0
    daily = _daily_pnl_series(decisions)
    values = list(daily.values())
    sharpe = _raw_sharpe(values)
    wins = sum(1 for t in decisions if t["is_win"])
    wr = wins / len(decisions) * 100
    return sharpe, wr
