"""
SQLite database operations for DecisionMemory Protocol.
...
class Database:
    ...
    def __init__(self, db_path: str | None = None):
        ...
        Args:
            db_path: Path to SQLite database file.
                     Defaults to DECISIONMEMORY_DB env var, then ~/.decisionmemory/decisionmemory.db,
                     then data/decisionmemory.db (legacy fallback).
        """
        if db_path is None:
            import os
            db_path = os.environ.get("DECISIONMEMORY_DB")
            if not db_path:
                home_db = Path.home() / ".decisionmemory" / "decisionmemory.db"
                if home_db.exists():
                    db_path = str(home_db)
                else:
                    db_path = "data/decisionmemory.db"
        ...
        try:
            ...
            conn.execute("""
                CREATE TABLE IF NOT EXISTS decision_records (
                    ...
                    decision_references TEXT NOT NULL,
                    exit_timestamp TEXT,
                    exit_price REAL,
                    pnl REAL,
                    pnl_r REAL,
            ...
            conn.execute("""
                ...
                ON decision_records(timestamp DESC)
            """)
            conn.execute("""
                CREATE INDEX IF NOT EXISTS idx_strategy
                ON decision_records(strategy)
            """)
            ...
            # Multi-tenancy scaffold: tenant_id column on decision_records.
            ...
            if "tenant_id" not in cols:
                conn.execute(
                    "ALTER TABLE decision_records ADD COLUMN tenant_id TEXT"
                )
                conn.execute("""
    ...
    def insert_decision(self, decision_data: Dict[str, Any]) -> bool:
        ...
        Args:
            decision_data: Decision record dictionary
        ...
        try:
            with self.get_connection() as conn:
                ...
                decision_data['market_context'] = json.dumps(raw_market_ctx)
                decision_data['decision_references'] = json.dumps(decision_data.get('references', []))
                decision_data['tags'] = json.dumps(decision_data.get('tags', []))

                # Default tenant_id to None when caller didn't supply one
                # (v0.5.1 callers won't know about this field).
                decision_data.setdefault('tenant_id', None)
                ...
                if cursor.rowcount == 1:
                    try:
                        ChainBuilder(conn).append(
                            record_id=decision_data['id'],
                            content_hash=content_hash,
                        )
                    except Exception as chain_err:
                        # Chain append failure should not silently corrupt — but
                        # we don't want to roll back the decision either. Log loudly.
                        logger.error(
                            "audit chain append failed for %s: %s",
                            decision_data.get('id'), chain_err,
                        )
                        raise
                return True
    ...
    def update_decision_outcome(self, decision_id: str, outcome_data: Dict[str, Any]) -> bool:
        ...
        try:
            with self.get_connection() as conn:
                query = f"UPDATE decision_records SET {', '.join(fields)} WHERE id = :id"  # nosec B608 — fields from internal whitelist
                outcome_data['id'] = decision_id
    ...
    def get_decision(self, decision_id: str) -> Optional[Dict[str, Any]]:
        ...
        with self.get_connection() as conn:
            ...
            decision['references'] = json.loads(decision['decision_references'])
            del decision['decision_references']  # Remove DB column name
    ...
    # ==================================================================
    # Factory — use get_database() to select backend
    # ==================================================================


def get_database(db_path: str | None = None):
    """Return SQLite, optionally decorated with a fail-open secondary store."""
    from .secondary_store import wrap_database

    return wrap_database(Database(db_path))
