"""
DecisionMemory MCP Server - FastAPI implementation.
...
from .models import SessionState, DecisionDirection, DecisionProposal
...
from .owm.migration import (
    ...
    migrate_decisions_to_episodic,
)
from .reflection import ReflectionEngine
from .state import StateManager

# Auth: bearer-token middleware. Opt-in via DECISIONMEMORY_API_KEYS.
...
app = FastAPI(
    title="DecisionMemory Protocol",
    description="AI Agent DecisionMaking Memory & Adaptive Decision Layer",
    version="0.5.2"
)
...
# Bearer-token auth middleware. No-op if DECISIONMEMORY_API_KEYS is unset.
app.add_middleware(BearerAuthMiddleware)
if auth_enabled():
    logger.info("DecisionMemory auth enabled — bearer tokens required.")
else:
    logger.info(
        "DecisionMemory auth DISABLED (DECISIONMEMORY_API_KEYS unset). "
        "All endpoints open. Set DECISIONMEMORY_API_KEYS=key:tenant to enable."
    )
...
class RecordDecisionRequest(BaseModel):
    """Request for decision.record_decision"""
    decision_id: str
    symbol: str
    direction: str
    lot_size: float
...
class RecordOutcomeRequest(BaseModel):
    """Request for decision.record_outcome"""
    decision_id: str
    exit_price: float
    pnl: float
    exit_reasoning: str
...
class QueryHistoryRequest(BaseModel):
    """Request for decision.query_history"""
...
@app.post("/decision/record_decision")
async def decision_record_decision(req: RecordDecisionRequest):
    """
    MCP Tool: decision.record_decision
    Log a decision decision with reasoning and context.
    """
    try:
...
@app.post("/decision/record_outcome")
async def decision_record_outcome(req: RecordOutcomeRequest):
    """
    MCP Tool: decision.record_outcome
    Log decision result after position closes.
    """
    try:
...
@app.post("/decision/query_history")
async def decision_query_history(req: QueryHistoryRequest):
    """
    MCP Tool: decision.query_history
    Search past decisions by strategy/date/result.
    """
    try:
        decisions = journal.query_history(
            strategy=req.strategy,
            symbol=req.symbol,
            limit=req.limit
        )
        ...
        return {
            "success": True,
            "count": len(decisions),
            "decisions": [t.model_dump() for t in decisions]
        }
...
@app.get("/decision/get_active")
async def decision_get_active():
    """
    MCP Tool: decision.get_active
    Get current open positions with context.
    """
    try:
        active_decisions = journal.get_active_decisions()

        return {
            "success": True,
            "count": len(active_decisions),
            "decisions": [t.model_dump() for t in active_decisions]
        }
...
async def mt5_sync(agent_id: str = "ng-gold-agent"):
    """
    MCP Tool: mt5.sync
    Sync MT5 demo decisions to DecisionJournal.

    Args:
        agent_id: Agent identifier for state tracking
    """
    try:
        stats = mt5_connector.sync_decisions(agent_id=agent_id)
...
class CheckDecisionRequest(BaseModel):
    """Request for risk.check_decision"""
    agent_id: str
    symbol: str
    direction: str
    lot_size: float
...
@app.post("/risk/check_decision")
async def risk_check_decision(req: CheckDecisionRequest):
    """
    MCP Tool: risk.check_decision
    Check a proposed decision against current risk constraints.
    """
    try:
        proposal = DecisionProposal(
            symbol=req.symbol,
            direction=DecisionDirection(req.direction),
            lot_size=req.lot_size,
            strategy=req.strategy,
            confidence=req.confidence,
            session=req.session,
        )
        result = adaptive_risk.check_decision(
            agent_id=req.agent_id,
            proposal=proposal,
        )
...
async def reflect_discover_patterns(req: DiscoverPatternsRequest):
    ...
    """
    try:
        db = None  # uses default decisionmemory.db
...
async def health_check():
    ...
    return {
        "status": "healthy",
        "service": "DecisionMemory Protocol",
        "version": "0.5.1"
    }
...
async def owm_remember(req: RememberDecisionRequest):
    """Store a decision into OWM multi-layer memory with automatic updates."""
    try:
        db = journal.db
        tid = req.decision_id or f"owm-{uuid.uuid4().hex[:12]}"
        ...
        update_semantic_from_decision(db, symbol_upper, req.strategy_name, req.pnl, req.pnl_r, req.context_regime, tid)
        update_procedural_from_decision(db, symbol_upper, req.strategy_name, req.pnl)
        update_affective_from_decision(db, req.pnl, req.confidence, strategy_name=req.strategy_name, symbol=symbol_upper)
...
async def owm_plan(req: CreatePlanRequest):
    """Create a prospective decision_making plan."""
    try:
        db = journal.db
        ...
        return {
            ...
            "status": "active",
            "expiry": expiry,
            "message": f"DecisionMaking plan created: {req.trigger_type} → {planned_act_parsed.get('type', 'action')}",
        }
    except HTTPException:
        raise
    except Exception as e:
...
async def owm_migrate():
    """Trigger OWM migration: decisions→episodic, patterns→semantic, initialize affective."""
    try:
        db = journal.db
        episodic_count = migrate_decisions_to_episodic(db)
...
# =====================================================================
# Audit API — DecisionMaking Decision Records (Phase 2)
# =====================================================================

from .domain.tdr import DecisionMakingDecisionRecord, MemoryContext


def _build_tdr(decision: Dict[str, Any], database=None) -> DecisionMakingDecisionRecord:
    """Build a TDR from a decision_records row, enriching with memory context."""
    if database is None:
        database = journal.db

    # Try to get memory context from the decision's references
    ...
    for ref_id in refs or []:
        try:
            rt = database.get_decision(ref_id)
        except Exception:
            continue
        if not rt:
            continue
    ...
    consonance = compute_recall_consonance(ref_evidence, decision.get("direction"))
    neg_count = sum(
    ...
    return DecisionMakingDecisionRecord.from_decision_record(decision, memory_ctx=mem)


@app.get("/audit/decision-record/{decision_id}")
async def audit_get_decision_record(decision_id: str):
    """Get a complete DecisionMaking Decision Record for a single decision.
...
async def audit_export(
    ...
    """Export DecisionMaking Decision Records as JSON array.
    ...
    return [_build_tdr(t, db).model_dump(mode="json") for t in decisions]
...
async def audit_export_jsonl(
    ...
    """Export DecisionMaking Decision Records as JSON Lines (one JSON object per line).
...
@app.get("/audit/verify/{decision_id}")
async def audit_verify(decision_id: str):
    """Verify integrity of a DecisionMaking Decision Record.
...
_API_PREFIXES = (
    "audit/", "dashboard/", "decision/", "state/", "reflect/", "mt5/", "risk/",
...
def main():
    """Entry point for `decisionmemory` CLI command."""
    import uvicorn
