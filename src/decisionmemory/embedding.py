...
def embed_decision_context(decision_data: dict) -> Optional[list[float]]:
    """Embed a decision's context into a vector.
    ...
    if not parts:
        logger.warning("embed_decision_context: no context fields found in decision_data")
        return None
    ...
    try:
        ...
        logger.warning(f"Embedding failed for decision context: {e}")
        return None
