...
- PeriodicReduceAgent: Every N decisions, reduce lot for M decisions
- RandomSkipAgent: Randomly skip X% of decisions
...
class PeriodicReduceAgent(BaseAgent):
    """Baseline 1: Every N decisions, reduce lot by X% for next M decisions.
    ...
    def should_decision(self, context: MarketContext) -> Optional[DecisionSignal]:
        signal = super().should_decision(context)
        if signal is None:
            return None
        self._decision_count += 1
        if self._decision_count % self._period == 0:
...
class RandomSkipAgent(BaseAgent):
    """Baseline 2: Randomly skip X% of decisions.
...
class SimpleWRAgent(BaseAgent):
    ...
    def on_decision_complete(self, decision: SimulatedDecision):
        ...
        self._outcomes.append(decision.pnl > 0)
