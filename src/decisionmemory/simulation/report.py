...
6. Behavioral Analysis (changepoint + DQS + skipped decisions)
7. Limitations & Risks
...
class FullExperimentReport:
    ...
    def _aggregate_by(self, group_key: str) -> Dict[str, Dict[str, Any]]:
        ...
        for key, experiments in groups.items():
            ...
            aggregated[key] = {
                ...
                "total_skipped": sum(c.get("decisions_skipped", 0) for c in comps),
    ...
    def _skip_aggregate(self) -> Dict[str, Any]:
        """Aggregate skipped decision analysis."""
        total_skipped = 0
        total_skipped_losing = 0
        total_skipped_pnl = 0.0
        for r in self._valid_results():
            comp = r.get("comparison", {})
            total_skipped += comp.get("decisions_skipped", 0)
    ...
    def summary_table(self) -> str:
        ...
        lines = [
            "| Symbol | TF | Strategy | Sharpe A | Sharpe B | \u0394 Sharpe | DD A | DD B | \u0394 DD | Decisions A | Decisions B | Skipped | Skip PnL | DQS-PnL r | p-value | Sig? |",
            "|--------|-----|----------|----------|----------|---------|------|------|------|----------|----------|---------|----------|-----------|---------|------|",
        ]
        for r in self._valid_results():
            comp = r.get("comparison", {})
            ...
            lines.append(
                ...
                f"| {comp.get('decisions_a', 0)} "
                f"| {comp.get('decisions_b', 0)} "
                f"| {comp.get('decisions_skipped', 0)} "
    ...
    def to_markdown(self) -> str:
        ...
        total_skipped = sum(c.get("decisions_skipped", 0) for c in all_comps)

        sections = []

        # ── Header ──
        sections.append(f"# Self-Calibrating DecisionMaking Agent: Experiment Report")
        sections.append(f"> Generated: {ts} | DecisionMemory Protocol v0.5.1")
        sections.append("")
        ...
        sections.append(
            ...
            f"compared to Agent A (baseline). Agent B skipped {total_skipped} decisions total "
            f"with a skip precision of {skip_agg['skip_precision']:.1%} "
            f"(proportion of skipped decisions that were losers). "
        ...
        sections.append("- **Statistical tests**: Welch's t-test on decision PnLs, DSR (Deflated Sharpe Ratio), Pearson correlation")
        ...
        sections.append(f"- Positive r indicates DQS score is predictive of decision outcome")
        sections.append("")
        ...
        sections.append(f"- Total DQS-scored decisions: {dqs_agg['total_decisions']}")
        sections.append("")
        ...
        # 6.3 Skipped Decision Analysis
        sections.append("### 6.3 Skipped Decision Analysis")
        sections.append("")
        sections.append(f"- Total decisions skipped by Agent B: {skip_agg['total_skipped']}")
        ...
        sections.append(f"- Total PnL of skipped decisions (Agent A): {skip_agg['total_skipped_pnl']:.4f}")
        sections.append("")
        ...
        sections.append("- Crypto-only: results on BTCUSDT/ETHUSDT may not generalize to forex/subjects")
        sections.append("- IS/OOS split is time-based (correct for decision data, but only one split point)")
        ...
        if skip_agg["skip_precision"] > 0.5:
            sections.append(f"- DQS successfully identifies losing decisions (precision {skip_agg['skip_precision']:.1%})")
        sections.append("")
        ...
        sections.append("---")
        sections.append(f"*Report generated by DecisionMemory Protocol v0.5.1 Simulation Framework*")
