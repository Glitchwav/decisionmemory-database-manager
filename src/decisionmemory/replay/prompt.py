...
def build_system_prompt() -> str:
    """Return a generic decision_making agent system prompt.
    ...
    Example override::
        ...
        from decisionmemory.replay.models import ReplayConfig
    ...
    """
    return """You are a decision_making agent analyzing OHLCV price data on M15 bars.
...
def build_user_prompt(
    ...
    Args:
        ...
        recent_decisions: List of recent closed decision dicts.
        score: Current account score.
    ...
    if recent_decisions:
        ...
        for t in recent_decisions[-5:]:
            pnl = t.get("pnl", 0.0)
            strategy = t.get("strategy", "?")
            result = t.get("result", "?")
            decision_lines.append(f"- {strategy}: {result} PnL={pnl:+.2f}")
        parts.append("\n".join(decision_lines))

    # Score
    parts.append(f"\n## Account\nScore: ${score:,.2f} | Risk per decision: 0.25%")

    # Past similar decisions from memory recall
    if memory_context:
        section = memory_context.replace(
            "## Similar Past Decisions", "## Past Similar Decisions"
        )
        if not section.startswith("## Past Similar Decisions"):
            section = "## Past Similar Decisions\n" + section
        parts.append(section)

    parts.append("\nWhat is your decision_making decision? Respond with JSON.")
