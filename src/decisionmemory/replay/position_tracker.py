...
class PositionTracker:
    """Manages position open/close lifecycle, score, and drawdown tracking."""

    def __init__(self, lot_size: float = 0.10, initial_score: float = 10000.0):
        self.lot_size = lot_size
        self.score = initial_score
        self.peak_score = initial_score
    ...
    def _close(self, bar: Bar, exit_price: float, state: PositionState) -> Position:
        """Internal close: compute PnL, update score/drawdown, archive position."""
        pos = self.current_position
        ...
        # R-multiple: pnl / risk_per_decision
        ...
        # Update score and drawdown
        self.score += pos.pnl
        if self.score > self.peak_score:
            self.peak_score = self.score
        if self.peak_score > 0:
            self.drawdown_pct = (self.peak_score - self.score) / self.peak_score * 100
