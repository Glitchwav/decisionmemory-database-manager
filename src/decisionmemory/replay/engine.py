"""ReplayEngine — orchestrates the full LLM decision_making agent replay loop.
...
from decisionmemory.replay.data_loader import parse_mt5_csv, sliding_window
from decisionmemory.replay.indicators import compute_all_indicators, precompute_d1_atr_series
from decisionmemory.replay.llm_client import LLMClient
from decisionmemory.replay.models import (
    AgentDecision,
    Bar,
    DecisionType,
    Position,
...
from decisionmemory.replay.position_tracker import PositionTracker
from decisionmemory.replay.prompt import build_system_prompt, build_user_prompt
...
class ReplayEngine:
        ...
        for bar_idx, window, current_bar in sliding_window(
            ...
            if self.config.use_memory_recall:
                ...
                if self.config.memory_recall_fn is not None:
                    # Pluggable recall (e.g. decision-dreaming hybrid retrieval)
                    ...
                    from decisionmemory.replay.memory_recall import build_memory_context
            ...
            entry = {
                ...
                "score": self.tracker.score,
                "position": (
                    self.tracker.current_position.decision_id
    ...
    def _store_to_memory(
        ...
        data = {
            "id": f"replay_{position.decision_id}",
    ...
    def _classify_session(timestamp: datetime) -> str:
        """Classify decision_making session from server time (GMT+2)."""
        hour = timestamp.hour
        if 0 <= hour < 7:
            return "asian"
        elif 7 <= hour < 14:
    ...
    def _classify_regime(position: Position) -> str:
        """Classify market regime based on decision outcome and price movement.
    ...
    def _checkpoint(self, bar_idx: int) -> None:
        ...
        data = {
            "bar_idx": bar_idx,
            "score": self.tracker.score,
            "decisions": len(self.tracker.closed_positions),
    ...
    def _build_summary(self, llm: Optional[LLMClient] = None) -> Dict[str, Any]:
        """Build summary dict with decision_making performance metrics."""
