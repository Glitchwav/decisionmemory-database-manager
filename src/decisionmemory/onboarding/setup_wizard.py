"""Interactive first-time setup wizard for DecisionMemory."""
...
from .terms import DECISIONMEMORY_DISCLAIMER, DECISIONMEMORY_ACCEPT_PROMPT
...
BANNER = f"""
{CYAN}{BOLD}================================================
  DecisionMemory Protocol - First-Time Setup
  AI DecisionMaking Memory & Adaptive Decision Layer
================================================{RESET}
"""
...
def _step_terms() -> bool:
    ...
    print(DECISIONMEMORY_DISCLAIMER)
    return _ask_yes_no(DECISIONMEMORY_ACCEPT_PROMPT, default=False)
...
def _step_env_vars() -> None:
    ...
    if anthropic_key:
        ...
        print(f"  {YELLOW}Skipped. You can set this later in ~/.decisionmemory/.env{RESET}")
...
def _step_done() -> None:
    ...
    print(f"  {CYAN}\"Store a decision: bought XAUUSD at 2650, sold at 2680, +$300\"{RESET}")
    print(f"  {CYAN}\"What patterns do you see in my recent decisions?\"{RESET}")
    print()
    print(f"  For full diagnostic: {BOLD}decisionmemory doctor --full{RESET}")
    print()
...
def run_setup() -> None:
    ...
    if not _step_terms():
        print(f"\n  {YELLOW}Setup cancelled. You must accept the terms to use DecisionMemory.{RESET}\n")
        return
