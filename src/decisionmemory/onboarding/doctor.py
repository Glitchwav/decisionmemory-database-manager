"""Health check system for DecisionMemory installation."""
...
def _check_mcp_tools() -> CheckResult:
    ...
    start = time.monotonic()
    try:
        import decisionmemory.mcp_server  # noqa: F401
        elapsed = (time.monotonic() - start) * 1000
        return CheckResult(
            name="MCP tools import",
            status="pass",
            detail="decisionmemory.mcp_server loaded",
            elapsed_ms=elapsed,
        )
    except Exception as e:
        elapsed = (time.monotonic() - start) * 1000
...
def _check_write_read_delete() -> CheckResult:
    """Write, read, and delete a test decision to verify DB operations."""
    start = time.monotonic()
    ...
    try:
        ...
        # Write — use insert_decision with all schema fields
        db.insert_decision({
            "id": test_id,
        ...
        # Read
        decision = db.get_decision(test_id)
        if decision is None:
            raise RuntimeError("Stored test decision but could not read it back")

        # Delete (no delete method on Database, use raw SQL)
        conn = db._get_connection()
        try:
            conn.execute("DELETE FROM decision_records WHERE id = ?", (test_id,))
            conn.commit()
        finally:
            conn.close()

        # Verify deletion
        decision_after = db.get_decision(test_id)
        if decision_after is not None:
            raise RuntimeError("Test decision was not deleted properly")
...
def _check_metadecision_maker5() -> CheckResult:
    """Check if MetaDecisionMaker5 Python package is installed."""
    start = time.monotonic()
    try:
        import MetaDecisionMaker5  # noqa: F401
        elapsed = (time.monotonic() - start) * 1000
        return CheckResult(
            name="MetaDecisionMaker5 package",
            status="pass",
            detail="Installed",
            elapsed_ms=elapsed,
        )
...
def _check_evolution_engine() -> CheckResult:
    ...
    start = time.monotonic()
    try:
        from decisionmemory.evolution.engine import EvolutionEngine  # noqa: F401
        elapsed = (time.monotonic() - start) * 1000
        return CheckResult(
...
def run_doctor(full: bool = False) -> List[CheckResult]:
    ...
    if full:
        ...
        results.append(_check_metadecision_maker5())
...
def print_results(results: List[CheckResult]) -> None:
    ...
    print(f"\n{BOLD}DecisionMemory Doctor{RESET}\n")
    ...
    if fail_count:
        print(f"\n  {RED}Some checks failed. Fix issues above before using DecisionMemory.{RESET}")
    elif warn_count:
