...
Design goals
------------
- **Opt-in.** If `DECISIONMEMORY_API_KEYS` is unset, all endpoints stay open
...
Configuration
-------------
Set `DECISIONMEMORY_API_KEYS` to a comma-separated list of `key:tenant_id`
pairs. Examples:

    DECISIONMEMORY_API_KEYS="abc123:acme,def456:bigfund"

A bare key with no tenant uses the literal `"default"` tenant:

    DECISIONMEMORY_API_KEYS="sk-test-1,sk-test-2"
...
API_KEYS_ENV = "DECISIONMEMORY_API_KEYS"
...
async def require_auth(request: Request) -> AuthContext:
    ...
    Behaviour:
      - No keys configured (`DECISIONMEMORY_API_KEYS` unset/empty) →
...
class BearerAuthMiddleware(BaseHTTPMiddleware):
    ...
    No-op when `DECISIONMEMORY_API_KEYS` is unset (preserves v0.5.1 behaviour).
