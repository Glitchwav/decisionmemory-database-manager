...
from ..repositories.decision import DecisionRepository, DecisionRow
...
class DashboardService:
    ...
    def get_overview(self) -> dict:
        """
        Compute overview metrics from decision, memory, and score data.

        Returns a dict matching OverviewResponse schema.
        """
        decision_stats = self._repo.get_decision_stats()
        memory_stats = self._repo.get_memory_stats()
        score_stats = self._repo.get_score_stats()

        # Compute derived metrics
        win_rate = 0.0
        if decision_stats.total_decisions > 0:
            win_rate = round(decision_stats.win_count / decision_stats.total_decisions, 4)

        profit_factor = 0.0
        if decision_stats.gross_loss > 0:
            profit_factor = round(
                decision_stats.gross_profit / decision_stats.gross_loss, 4
            )
        elif decision_stats.gross_profit > 0:
            # All wins, no losses
            profit_factor = float("inf")

        # max_drawdown_pct from affective_state drawdown_state
        max_drawdown_pct = round(score_stats.drawdown_state * 100, 2)

        return {
            "total_decisions": decision_stats.total_decisions,
            "total_pnl": round(decision_stats.total_pnl, 2),
            "win_rate": win_rate,
            "profit_factor": profit_factor,
            "current_score": round(score_stats.current_score, 2),
            "max_drawdown_pct": max_drawdown_pct,
            "memory_count": memory_stats.memory_count,
            "avg_confidence": memory_stats.avg_confidence,
            "last_decision_date": decision_stats.last_decision_date,
            "strategies": decision_stats.strategies,
        }
    ...
    def get_score_curve(
        ...
        """
        Compute score curve from closed decisions.

        Groups by date. Computes cumulative PnL and drawdown_pct per day.
        """
        decisions = self._repo.get_closed_decisions(
        ...
        daily: dict[str, list[DecisionRow]] = defaultdict(list)
        for t in decisions:
        ...
        for date_str in sorted(daily.keys()):
            day_decisions = daily[date_str]
            for t in day_decisions:
    ...
    def get_rolling_metrics(
        ...
        """
        Compute rolling window metrics over closed decisions.

        For each decision after the first `window_size` decisions, compute:
        ...
        Groups output by date (last decision date in window).
        """
        decisions = self._repo.get_closed_decisions(
            start_date=start_date, end_date=end_date, strategy=strategy
        )
        ...
        result = []
        for i in range(window_size, len(decisions) + 1):
            window = decisions[i - window_size : i]
            last_decision = window[-1]
            date_str = last_decision.timestamp[:10]
    ...
    def get_confidence_calibration(self) -> List[dict]:
        ...
        Returns list of {decision_id, entry_confidence, actual_pnl_r, strategy}.
        """
    ...
    def get_dream_results(self, dream_path: Optional[Path] = None) -> List[dict]:
        """
        Load decision-dreaming simulation results from JSON files.
        ...
        if dream_path is None:
            dream_path = Path(
                os.environ.get(
                    "DREAM_DATA_PATH",
                    r"C:\Users\johns\projects\decision-dreaming\data",
                )
            )
            ...
            for sess in sessions:
                try:
                    results.append({
                        ...
                        "decisions": sess.get("decisions", 0),
                        "pf": sess.get("pf", 0.0),
    ...
    def get_strategy_detail(self, name: str) -> dict:
        ...
        Raises StrategyNotFoundError if no decisions exist for the strategy.
        """
        ...
        decisions = self._repo.get_strategy_decisions(name)
        if not decisions:
            raise StrategyNotFoundError(f"Strategy '{name}' has no closed decisions")

        total_decisions = len(decisions)
        wins = sum(1 for t in decisions if t.pnl > 0)
        win_rate = round(wins / total_decisions, 4) if total_decisions > 0 else 0.0
        ...
        pnl_r_values = [t.pnl_r for t in decisions if t.pnl_r is not None]
        avg_pnl_r = round(sum(pnl_r_values) / len(pnl_r_values), 4) if pnl_r_values else 0.0

        hold_values = [t.hold_duration for t in decisions if t.hold_duration is not None]
