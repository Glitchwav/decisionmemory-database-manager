...
from decisionmemory.data.protocol import (
    DataSourceError,
    RateLimitError,
    SymbolNotFoundError,
)
...
DEFAULT_CACHE_DIR = Path.home() / ".decisionmemory" / "cache" / "binance"
...
class BinanceDataSource:
    ...
    def _parse_klines(self, raw: list[list]) -> list[OHLCV]:
        ...
        Binance kline format: [open_time, open, high, low, close, volume,
                               close_time, quote_volume, decisions, ...]
        """
        bars = []
        for k in raw:
            bars.append(
    ...
    async def available_symbols(self) -> list[str]:
        """List all decision_making pairs on Binance."""
        await self._rate_limit()
        client = await self._get_client()
        ...
        data = resp.json()
        return [s["symbol"] for s in data.get("symbols", []) if s.get("status") == "DECISION_MAKING"]
