...
All data access lives in repositories/decision.py.
"""
...
async def overview(
    ...
    """Get dashboard overview with key decision_making metrics."""
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to query decision_making data"
        )


@dashboard_router.get("/score-curve", response_model=List[ScorePoint])
async def score_curve(
    start_date: Optional[str] = Query(None, description="Filter from date (YYYY-MM-DD)"),
    end_date: Optional[str] = Query(None, description="Filter to date (YYYY-MM-DD)"),
    strategy: Optional[str] = Query(None, description="Filter by strategy name"),
    service: DashboardService = Depends(get_dashboard_service),
):
    """Get score curve with cumulative PnL and drawdown per day."""
    try:
        return service.get_score_curve(
        ...
        logger.error(f"Database query error: {e}")
        raise HTTPException(
            status_code=500, detail="Failed to query score curve"
        )


@dashboard_router.get("/rolling-metrics", response_model=List[RollingMetricPoint])
async def rolling_metrics(
    window_size: int = Query(10, ge=2, description="Rolling window size in decisions"),
    start_date: Optional[str] = Query(None, description="Filter from date (YYYY-MM-DD)"),
    end_date: Optional[str] = Query(None, description="Filter to date (YYYY-MM-DD)"),
    strategy: Optional[str] = Query(None, description="Filter by strategy name"),
    service: DashboardService = Depends(get_dashboard_service),
):
    """Get rolling performance metrics over a sliding window of decisions."""
    try:
...
async def dream_results(
    ...
    """Get decision dreaming simulation results.

    Returns empty list if dream data path doesn't exist — this is
    expected until Decision Dreaming Phase 2 is implemented.
    """
    try:
