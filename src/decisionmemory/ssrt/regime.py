...
from decisionmemory.ssrt.models import RegimeBaseline, DecisionResult
...
class RegimeAwareNull:
    ...
    When a regime has sufficient data (>= min_decisions_per_regime), its mean
    ...
    def __init__(self, min_decisions_per_regime: int = 10, default_null: float = 0.0):
        self.min_decisions_per_regime = min_decisions_per_regime
    ...
    def update(self, decision: DecisionResult) -> None:
        ...
        Args:
            decision: Decision result with regime and pnl_r fields.
        """
        self._validate_regime(decision.regime)
        ...
        acc["n"] += 1
        n = acc["n"]
        delta = decision.pnl_r - acc["mean"]
        acc["mean"] += delta / n
        delta2 = decision.pnl_r - acc["mean"]
        acc["m2"] += delta * delta2

        self._last_updated[regime] = decision.timestamp
    ...
    def get_null(self, regime: str) -> tuple[float, float]:
        ...
        if acc is None or acc["n"] < self.min_decisions_per_regime:
    ...
    def get_state(self) -> dict[str, Any]:
        ...
        return {
            "min_decisions_per_regime": self.min_decisions_per_regime,
    ...
    def from_state(cls, state: dict[str, Any]) -> RegimeAwareNull:
        ...
        obj = cls(
            min_decisions_per_regime=state["min_decisions_per_regime"],
