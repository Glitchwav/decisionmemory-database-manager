...
class MixtureSPRT:
    ...
    Args:
        ...
        tau: Mixing distribution scale. Controls sensitivity:
            small tau = detect small effects (more decisions needed),
            large tau = detect large effects (fewer decisions).
        sigma: Assumed observation std (estimated from history).
        null_mean: Baseline mean under H0 (regime-dependent).
        burn_in: Minimum decisions before making decisions.
    """
    ...
    def update(self, observation: float) -> SSRTVerdict:
        ...
        Args:
            observation: Decision R-multiple (NOT shifted -- we shift
