# DecisionMemory Protocol — Claude Code 指令

## 專案概述

DecisionMemory Protocol 是 Mnemox AI 的核心產品。MT5/forex 交易記憶層，3 層架構（L1 原始交易 → L2 模式發現 → L3 策略調整），MCP server。

- GitHub: mnemox-ai/decisionmemory-protocol
- PyPI: decisionmemory-protocol
- 版本: v0.5.1
...
- 不要修改核心 MCP tools（src/decisionmemory/mcp_server.py），除非 ROADMAP 明確要求
...
- MCP tools 定義：`src/decisionmemory/mcp_server.py`（4 個 MCP tools：store/recall/performance/reflection）
- FastAPI REST server：`src/decisionmemory/server.py`（35+ REST endpoints）
- MT5 同步：`docs/MT5_SYNC_SETUP.md` + `scripts/mt5_sync.py`
- 每日反思：`scripts/daily_reflection.py`
- Demo：`src/decisionmemory/demo.py`（`decisionmemory demo [--fast]`，30 筆模擬交易跑完整 L1→L2→L3 pipeline）
- 測試：`tests/`
- OpenClaw Skill：`.skills/decisionmemory/SKILL.md`
...
- [2026-04-10] **arXiv paper major revision** — 6300 words, 17 refs. MaxDDStop (score DD threshold) outperforms CUSUM 93.5%. Reframed CUSUM as diagnostic tool. Added k=0 justification, robustness check (without BTCUSDT 1h: vs SimpleWR p=0.179), strategy dependence caveat. h sensitivity pending.
...
- [2026-04-10] **Research framework** — 4-level validation protocol (L0 component → L1 controlled → L2 multi-strategy → L3 multi-decision context)
- [2026-04-09] Phase 4b fixes — warm-start, score metrics, adaptive DQS thresholds, CUSUM adaptive target_wr
...
- [2026-04-08] **深度重構 — 審計行動計劃全部完成**：
  - Phase A: 刪除 store_decision_memory + recall_similar_decisions（19→17 tools），修正行銷語言，加 Research Status
  ...
  - Phase C: db.py 錯誤處理改 raise DecisionMemoryDBError（18→2 個 return False），加 get_connection context manager
...
- **SSRT Module**: `src/decisionmemory/ssrt/` — mSPRT engine (tau=0.3 default), shift_null, regime-aware null, simulator, baselines. Phase 1+2 results in `validation/ssrt/`. Best method: mSPRT_t03 (81.4% power, Type I=0.008). Regime-aware approaches both fail.
...
- **Phase 5 Rigorous Validation Complete**: 100 experiments (2 symbols × 1h × 50 grid strategies × 5 agents)
  - Result: **INVALID** — CalibratedAgent skips 97% decisions (48/100 zero-decision), DD "reduction" from NOT ACTING
...
- **方向**: Decision-Making AI Service 接案→數據驗證→SaaS
