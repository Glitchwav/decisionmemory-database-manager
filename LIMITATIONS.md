# DecisionMemory Protocol — Known Limitations & Roadmap
...
community deserve to know what DecisionMemory does **today**, what is **research-
...
- **Phase 5 rigorous validation: INVALID.** 100 experiments (2 symbols × 1h ×
  ...
  of decisions, so the apparent drawdown reduction came from *not decision-making*, not
...
- **Phase 6 (Q3 2026):** revisit DQS calibration on a denser real-decision
...
- **Live partner pilot:** the OTSO conversation, if it converges, will
  generate the multi-provider, multi-strategy ground truth we currently lack.
...
- **Backfill script** at `scripts/backfill_audit_chain.py` deterministically
  rebuilds the chain from `decision_records` (ORDER BY timestamp, id).
...
- **zkML proof of inference** — proving "this strategy was actually run on
  this decision context to produce this decision" is on the 90-day roadmap
...
- `suppression_recommended = score < 0.2` (strong counter-evidence — agent
  should consider blocking the decision).
...
- Thresholds (0.4 / 0.2) are reasonable defaults but not yet tuned on real
  decision data. They will be SSRT-tuned in Phase 6.
...
### Today
- **MetaDecision-Maker 5 only** (`mt5_connector.py`). Polling sync, not event-driven.

### Roadmap
- IBKR (30-60 day) and Alpaca (60-90 day) connectors for the US score
  segment.
- CCXT (60-90 day) for crypto venues — driven by the Polystrat / agent-
  decision contextplace direction.
