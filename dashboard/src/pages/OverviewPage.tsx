...
import ScoreCurveChart from '../components/charts/ScoreCurveChart';
import RollingMetricsChart from '../components/charts/RollingMetricsChart';
import { useOverview, useScoreCurve, useRollingMetrics } from '../api/hooks';
...
export default function OverviewPage() {
  ...
  const score = useScoreCurve();
  const rolling = useRollingMetrics();
  const [fetchedAt] = useState(() => new Date());

  const isLoading = overview.isLoading || score.isLoading || rolling.isLoading;
  const error = overview.error || score.error || rolling.error;
  ...
  if (!overview.data || overview.data.total_decisions === 0) {
    return (
      <PageShell>
        <EmptyState
          icon="&#128200;"
          title={t('common.empty')}
          description="Store decisions via MCP tools or REST API to see your overview dashboard."
        />
      </PageShell>
    );
  }
      ...
      <div className={`${styles.page} pageFadeIn`}>
        ...
        <RevealDiv className={styles.metricRow}>
          <MetricCard
            ...
            trendValue={`${d.total_decisions} ${t('overview.decisions')}`}
          />
          <MetricCard
            title={t('overview.winRate')}
            value={formatPct(d.win_rate)}
            trend={d.win_rate >= 0.5 ? 'up' : 'down'}
            trendValue={`${Math.round(d.win_rate * d.total_decisions)}W / ${d.total_decisions - Math.round(d.win_rate * d.total_decisions)}L`}
          />
          <MetricCard
          ...
          <MetricCard
            ...
            trendValue={`${t('overview.score')}: $${d.current_score.toLocaleString('en-US', { minimumFractionDigits: 2 })}`}
          />
          <MetricCard
        ...
        {/* Score Curve */}
        {score.data && score.data.length > 0 && (
          <RevealDiv className={styles.chartSection}>
            <div className="sectionHeader">
              <p className={styles.chartTitle}>{t('overview.scoreCurve')}</p>
              <button
                className="csvExportBtn"
                onClick={() => {
                  const today = new Date().toISOString().slice(0, 10);
                  downloadCSV(score.data as unknown as Record<string, unknown>[], `score-curve-${today}.csv`);
                }}
              >
            ...
            <div className={styles.chartContainer}>
              <ScoreCurveChart data={score.data} />
            </div>
          </RevealDiv>
        )}
