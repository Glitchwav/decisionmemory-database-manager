        ...
        <RevealDiv className={styles.section}>
          ...
          {!growth.isLoading && !growth.error && (!growth.data || growth.data.length === 0) && (
            <EmptyState
              ...
              description="Store decisions via MCP tools to see memory growth across regimes."
            />
          )}
          {!growth.isLoading && !growth.error && growth.data && growth.data.length > 0 && (
          ...
          <RevealDiv className={styles.section}>
            ...
            {!cal.isLoading && !cal.error && (!cal.data || cal.data.length === 0) && (
              <EmptyState
                icon="&#127919;"
                title={t('common.empty')}
                description="Decisions with entry_confidence and R-multiple will appear here."
              />
            )}
            {!cal.isLoading && !cal.error && cal.data && cal.data.length > 0 && (
