...
For the analyze_decision_maker.py product: this means we should run discovery 2-3 times internally and only surface patterns that appear in the majority of runs. 20% more API cost, but dramatically more trustworthy output.
...
3. **For analyze_decision_maker.py**: Use temp=0.3 (not 0.0, still want some creativity) + 2 runs + intersection. Balances cost and reliability.

Run 3 is notable -- it uses in [14,15,16] instead of eq 14, adding atr_percentile conditions. This is the most sophisticated output and suggests higher temperature occasionally produces richer hypotheses. Decisionoff: richer but less stable.

## Next Steps
- Quant: Proceed to B2 cross-asset transfer. The semantic stability justifies testing whether these themes transfer to ETH.
- Business: Implement "only show backtested patterns" rule in analyze_decision_maker.py. Never expose raw LLM output to users.
