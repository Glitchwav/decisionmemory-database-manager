...
- CUSUM: median delay 22-79 decisions
- RollingSharpe: median delay 17-61 decisions
- mSPRT: median delay 64-119 decisions
...
The regime-aware variant loses evidence on every regime switch (reset). In this setup with 25-decision regime blocks, that's frequent resets destroying accumulated evidence. The fixed-null mSPRT that doesn't reset accumulates evidence across both regimes and catches the overall performance drop faster.

**Why**: Phase 1 chose Option A (reset on regime change) for statistical purity. The cost is real — 18 percentage points worse detection. Phase 2 could explore Option B (adjust shift, keep evidence) or only reset after N consecutive decisions in new regime.

### 4. Economic value (P&L saved)?

Mixed. Mean PnL saved is often NEGATIVE because:
1. mSPRT triggers late (64-119 decisions after decay), so you eat losses before retiring
...
- **Always-valid p-values**: Can peek after every decision without inflation
...
For systematic decision_making where you have many strategies, false retirements are very expensive (you lose your edge). mSPRT is the right choice.
...
1. The experimental setup (25-decision regime blocks) may not be representative
...
| mSPRT_regime | Same + RegimeAwareNull(min_decisions=10), reset on regime change |
...
| Scenario | Decisions | Pre-mean | Post-mean | Decay at | Std |
|----------|--------|----------|-----------|----------|-----|
...
5. **Real data validation**: Apply to NG_Gold decisions with known regime periods
