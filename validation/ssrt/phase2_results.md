...
Why both regime variants lose to fixed null: The regime_specific scenario has 25-decision regime blocks. Both regime-aware methods disrupt the evidence accumulation (reset destroys it, shift corrupts it), while the fixed-null mSPRT simply accumulates evidence across all decisions against a constant baseline. The overall performance drop (from pre_mean=0.5 to post_mean=-0.3) is strong enough that a single fixed null catches it even without regime decomposition.
...
1. **Regime-aware null is a dead end** — both Option A (reset) and Option B (shift) are worse than fixed-null mSPRT. The 25-decision regime blocks in our simulation are too short for regime-specific estimation to help.

2. **tau=0.3 is the recommended default** — consistent improvement in power (+5pp) with better Type I control (0.008 vs 0.012). No decision-off.
...
4. **The fundamental gap remains** — mSPRT_t03 detects at median delay 70 decisions vs MaxDD's -9 to 11. For a decision_maker who needs fast reaction, heuristic methods with careful threshold calibration (accepting higher false positives) may be preferable. The paper's contribution is giving decision_makers a method with known, controlled error rates.
...
| mSPRT_regime_reset | mSPRT + RegimeAwareNull(min_decisions=10), reset on regime change |
| mSPRT_regime_shift | mSPRT + RegimeAwareNull(min_decisions=10), shift_null on regime change |
...
2. **Longer regime blocks**: Test with 50-100 decision regime blocks to give regime estimation more data
3. **Two-sided test**: Current mSPRT only tests for decay (one-sided). A strategy that improves should NOT be retired.
4. **Real data validation**: Apply mSPRT_t03 to NG_Gold decisions
