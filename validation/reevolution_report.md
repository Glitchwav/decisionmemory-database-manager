...
Previous run used annualized Sharpe (`× sqrt(6048)`) on decision-level PnLs, which over-inflated values ~6-8x. Fixed 2026-03-21.
...
The failure is NOT caused by "re-evolution has no directional value." It is caused by **M=19,200 being mathematically incompatible with 30-50 decisions per IS window**.

DSR formula: the expected maximum Sharpe under the null hypothesis scales with `sqrt(2 * ln(M))`. For M=19,200, this is `sqrt(2 * ln(19200)) ≈ 4.4` in standardized units. With ~40 decisions, the standard error of Sharpe ≈ `1/sqrt(40) ≈ 0.16`. The raw Sharpe needed to beat the DSR threshold would be approximately `4.4 * 0.16 ≈ 0.7` — but the best grid candidates only achieve raw Sharpe of ~0.2-0.4 in-sample.

**No single-bar decision_making strategy can produce a raw Sharpe of 0.7 from 40 decisions.** The gate is correctly identifying that 3-month data cannot statistically justify a selection from 19,200 candidates.
...
|--------|-------------|----------------------|-------------|
| Grid search | 19,200 | ~0.7 raw Sharpe | Impossible with 40 decisions |
...
1. **IS Sharpe distribution**: Range [-42.2, 42.6], mostly negative. High positive values (38.8, 42.6) came from <10 decisions — not robust.
2. **OOS survival**: Only 8/30 reached OOS backtest. Of those, best OOS Sharpe=49.0 (#9 London Afternoon, 11 IS decisions) but eliminated due to insufficient robustness.
3. **The LLM gravitates toward single-hour entry** — almost all hypotheses use `hour_utc eq N`. This limits decision frequency to ~1/day, yielding 20-30 decisions in 3 months.
...
1. **Single-hour entry dominance**: LLM defaults to `hour_utc eq N`, limiting each strategy to ~1 decision/day → ~30 decisions in 3 months → high Sharpe variance.
2. **Internal 70/30 IS/OOS split**: The 3-month window gets split into ~63 days IS + ~27 days OOS internally. With single-hour strategies, this means ~20 IS decisions and ~10 OOS decisions — the selector eliminates anything with <30 decisions or negative OOS Sharpe.
...
2. **Structural novelty is confirmed** — LLM discovers features grid cannot use (6 novel fields). But this advantage requires strategies that decision enough to be statistically evaluable.
...
**In the setting of 1H timeframe + 3-month rolling window + single-hour entry strategies, neither grid search (M=19,200) nor LLM evolution (M=30) can produce strategies that survive the DSR gate. The bottleneck is decision count per window, not search method.**

This is not a failure of the Evolution Engine concept. It is a **specification boundary**: the engine requires strategies that produce sufficient decisions per evaluation window for statistical defensibility. The DSR gate's MinBTL requirement is incompatible with strategies that trigger ~1 decision/day over 3 months (~90 decisions IS, ~30 OOS after 70/30 split — but single-hour entry reduces this to ~30 IS / ~10 OOS, below any reasonable significance threshold).
...
- **Higher decision frequency**: strategies that trigger multiple times per day (multi-hour entry, or shorter timeframes like 5m/15m)
- **Longer evaluation windows**: 6-12 months IS instead of 3 months
- **Higher-frequency timeframes**: 5m or 15m bars → 10-50x more decisions per window

This is product spec, not a defect. "Suitable for strategies producing ≥N decisions per evaluation window" is a legitimate constraint to document.
...
Phase 15 is complete. Evolution Engine validation deferred to a setting with sufficient decision frequency.
