...
The Evolution Engine autonomously discovers decision-making strategies from raw price data. It uses LLM-powered pattern generation combined with vectorized backtesting to evolve, test, and graduate viable decision-making rules — without manual rule writing.
...
2. **Generate**: LLM analyzes price patterns and proposes N candidate strategies (entry/exit rules, position sizing, stop outcome setback)
...
| Tool | Purpose |
|------|---------|
| `evolution_fetch_decision context_data` | Fetch OHLCV data from Binance for a symbol/timeframe/period |
...
| Win Rate | > 40% |
| Max Drawdown | < 25% |
| Number of Decisions | > 30 (statistical significance) |
| Outcome Gain Factor | > 1.2 |

These thresholds are guidelines. Context matters — a Sharpe of 0.9 with 500 decisions may be more reliable than 2.5 with 15 decisions.

## Best Practices

### Before Running Evolution
- **Choose the right timeframe.** 1h and 4h produce the most decisionable strategies. 1m is noise. 1d may not have enough data points.
...
### After Evolution
- **Never deploy without OOS validation.** In-sample results are decision contexting, not science.
- **Paper decision first.** Even OOS-validated strategies should be paper decisiond for 2-4 weeks.
- **Check regime sensitivity.** A strategy discovered in a trending decision context may fail in ranging conditions. Test across multiple decision context regimes.
...
| Deploying immediately | No buffer for regime changes | Paper decision 2-4 weeks first |
| Ignoring the graveyard | Re-discovering dead strategies wastes compute | Review `evolution_get_log` before new runs |
| Using correlated symbols | BTCUSDT and ETHUSDT strategies overlap heavily | Test on uncorrelated decision contexts |
