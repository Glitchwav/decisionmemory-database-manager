...
`mt5_sync_v3.py` 是 v2 的 FastAPI 重構版。內建 web dashboard、background poller、decision advisor Discord 通知。
...
FastAPI (port 9001)
    ...
    └── GET /recent-decisions  → JSON
    ↓
DecisionMemory API (record_decision + record_outcome)
    ↓
...
- MetaDecision-Maker5 Python package
...
pip install MetaDecision-Maker5 python-dotenv requests fastapi uvicorn
```

## .env 設定

確認 `C:\Users\johns\projects\decisionmemory-protocol\.env` 包含：
...
```powershell
schtasks /create /tn "MT5SyncV3_AutoStart" /xml "C:\Users\johns\projects\decisionmemory-protocol\scripts\MT5SyncV3_AutoStart.xml"
```
...
5. **Actions**：
   - Program: `cmd.exe`
   - Arguments: `/c "C:\Users\johns\projects\decisionmemory-protocol\scripts\start_mt5_sync_v3.bat"`
   - Start in: `C:\Users\johns\projects\decisionmemory-protocol`
6. **Settings**：
...
- v3 (`mt5_sync_v3.py`) 有 FastAPI dashboard + decision advisor
- 兩者 **不要同時跑**，會重複 sync 交易到 DecisionMemory
