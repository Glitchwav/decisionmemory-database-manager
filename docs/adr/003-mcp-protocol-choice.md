...
DecisionMemory needs an interface for AI agents to store decision decisions, recall similar past decisions, query strategy performance, and manage decision-making plans. The interface must support both programmatic AI agent access and human-driven tooling (dashboards, debugging, MT5 synchronization).
...
**Native AI agent integration.** MCP is the standard protocol for tool use in Claude, Cursor, Windsurf, and a growing list of AI coding environments. When an agent connects to DecisionMemory's MCP server, it automatically discovers all 19 tools -- `remember_decision`, `recall_similar_decisions`, `get_strategy_performance`, etc. No SDK installation, no API key management, no endpoint discovery.
...
**Schema enforcement via FastMCP.** Input validation happens at the protocol layer. If an agent calls `remember_decision` with a missing `symbol` field, FastMCP rejects it before our code runs. This prevents garbage-in problems that plague REST APIs consumed by LLMs (which frequently hallucinate optional fields or malform JSON).

**Ecosystem momentum.** The MCP registry (Smithery) lists 11,000+ servers as of early 2026. Anthropic, OpenAI (via Agents SDK), Google (via ADK), and Microsoft have adopted or announced MCP support. Building on MCP positions DecisionMemory in a growing ecosystem rather than a proprietary silo.

### Why REST as Secondary

**MT5 synchronization.** MetaDecisionMaker 5 Expert Advisors are written in MQL5, which supports HTTP requests (`WebRequest`) but cannot speak the MCP stdio protocol. The MT5 sync bridge sends closed decision data via REST endpoints.

**Dashboard frontend.** The web dashboard (Next.js) fetches strategy performance, decision history, and agent state via REST. Browsers cannot connect to stdio-based MCP servers.
...
Both MCP tools and REST endpoints call the same core logic layer. There is no feature divergence -- a decision stored via MCP is queryable via REST and vice versa.
...
- **MCP auth.** The protocol's authentication story is still evolving. Currently DecisionMemory runs locally (no auth needed). A hosted version would need MCP-level auth or a reverse proxy.
