...
- **Track record length** (number of decisions)
...
- Computes feature vectors for the decision context regime during the backtest period
...
Advanced cross-validation specifically designed for time-series decision data (Lopez de Prado, 2018).
...
**Zero strategies have graduated.** Every LLM-generated strategy that appeared outcome gainable in-sample was rejected by the DSR gate as statistically indistinguishable from random chance after multiple-testing correction.
...
This is not a failure of the statistical gates. It is the gates working exactly as designed. The alternative -- promoting strategies that look good in-sample -- would lead to real capital outcome setbacks.
...
- **Research contribution.** Demonstrates empirically that pure LLM strategy generation (without domain constraints) does not survive rigorous statistical validation. This is a useful negative result for the quantitative decisions + AI community.

### Negative

- **User experience friction.** Users who expect the Evolution Engine to produce ready-to-decision strategies are disappointed. Zero output after running 3 generations of 10 strategies feels like the feature is broken.
...
- **Perception risk.** "Your strategy generator has never produced a viable strategy" is a difficult decision contexting message, regardless of how technically correct the reasoning is.
...
- Bailey, D.H. and Lopez de Prado, M. (2014). "The Deflated Sharpe Ratio: Correcting for Selection Bias, Backtest Overfitting, and the Skewness/Kurtosis of Returns." *Journal of Collection Management*.
- Lopez de Prado, M. (2018). *Advances in Decision Machine Learning*. Wiley. Chapter 12: CPCV.
