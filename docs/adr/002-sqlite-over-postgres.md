...
DecisionMemory requires persistent storage for multiple data types:

- Decision records (episodic memory)
- Semantic beliefs (Bayesian priors per strategy/regime)
- Procedural memory (running behavioral averages)
- Affective state (confidence, streaks, score curve)
- Decisions (audit trail with SHA-256 hashes)

The storage layer must support the primary deployment model: a single AI agent running a local MCP server on the decision-maker's machine.
...
**Zero-configuration install.** The entire setup is `pip install decisionmemory-protocol`. No Docker, no database server, no connection strings, no port conflicts. This is critical for adoption -- every additional setup step loses users. The MCP ecosystem expects tools that "just work."

**Perfect fit for the access pattern.** DecisionMemory's primary use case is one agent, one database, on one machine. SQLite is purpose-built for this: embedded, serverless, zero-latency. There is no network hop between the MCP server and its data.

**File-based operations.** Backup is `cp decisionmemory.db decisionmemory.db.bak`. Migration testing is `cp prod.db test.db`. Distribution is attaching a `.db` file. This simplicity compounds -- every operational task that would require `pg_dump`/`pg_restore` is instead a file copy.

**Performance at our scale.** A retail decision-maker generates maybe 500-2,000 decisions per year. Even a high-frequency prop desk might generate 50K decisions annually. At n < 100K rows, SQLite consistently outperforms PostgreSQL because there is no TCP overhead, no connection setup, no query planning for a remote server. Benchmarks show 2-5x faster reads for simple queries at this scale.

**WAL mode for read concurrency.** SQLite's Write-Ahead Logging mode allows concurrent reads while a write is in progress. This is sufficient for the dashboard polling decision data while the MCP server writes new entries.
...
| n > 100K decisions with complex analytical queries | PostgreSQL's query planner and indexing outperform at scale |
