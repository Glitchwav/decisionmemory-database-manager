...
<p align="center">
  <img src="../assets/header-zh.png" alt="DecisionMemory Protocol" width="600">
</p>
...
**DecisionMemory 就是那個記憶層。** 一個 `pip install`，你的 AI agent 就能記住每一筆交易、每一個結果、每一個錯誤——搭配 SHA-256 防竄改的審計軌跡。
...
相容任何市場（股票、外匯、加密貨幣、期貨）、任何券商、任何 AI 平台。DecisionMemory 不執行交易也不碰你的資金——它只負責記錄和回憶。
...
| **詳細說明** | [閱讀更多 →](USE_CASES.md#case-1-us-score-decision-maker--pre-flight-workflow) | [閱讀更多 →](USE_CASES.md#case-2-forex-ea-system--automated-memory-loop) | [閱讀更多 →](USE_CASES.md#case-3-compliance-first-fund--audit-trail) |
...
2. **記錄** — 交易後，一次呼叫 `remember_decision` 寫入五個記憶層：情節記憶、語義記憶、程序記憶、情感記憶和交易紀錄
...
| 類別 | 工具 | 說明 |
|------|------|------|
| **記憶** | `remember_decision` · `recall_memories` | 以結果加權評分記錄和回憶交易 |
| **狀態** | `get_agent_state` · `get_behavioral_analysis` | 信心、回撤、連勝/連敗、行為模式 |
| **計畫** | `create_decision_plan` · `check_active_plans` | 附條件觸發的前瞻性計畫 |
| **風險** | `check_decision_legitimacy` | 五因子交易前審核（完整 / 縮減 / 跳過） |
...
| 類別 | 工具 |
|------|------|
| **核心記憶** | `get_strategy_performance` · `get_decision_reflection` |
| **OWM 認知** | `remember_decision` · `recall_memories` · `get_behavioral_analysis` · `get_agent_state` · `create_decision_plan` · `check_active_plans` |
| **風險與治理** | `check_decision_legitimacy` · `validate_strategy` |
...
你的 agent 做的每一個交易決策——包括決定**不交易**——都會被記錄為 Decision (TDR)，並在建立時計算 SHA-256 雜湊以進行竄改偵測。

| 法規 | 要求 | DecisionMemory 覆蓋範圍 |
|------|------|---------------------|
...
- **絕不碰 API 金鑰。** DecisionMemory 不執行交易、不移動資金、不存取錢包。
- **只讀取和記錄。** 你的 agent 把決策上下文傳給 DecisionMemory。它儲存它。就這樣。
...
DecisionMemory 的 OWM 框架基於認知科學（Tulving 1972）和強化學習（Schaul et al. 2015）。目前狀態：
...
| [MT5 設定](MT5_SYNC_SETUP.md) | MetaDecisionMaker 5 整合 |
| [研究日誌](RESEARCH_LOG.md) | Evolution 實驗與數據 |
| [Failure Taxonomy](decision-making-ai-failure-taxonomy.md) | 11 種交易 AI 失敗模式 |
