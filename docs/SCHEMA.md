# DecisionMemory Data Schema

This document describes the core data structures used in DecisionMemory Protocol. All examples show actual JSON payloads that you'll work with when calling MCP tools.

---

## DecisionRecord

The `DecisionRecord` is the fundamental unit of memory in DecisionMemory. Every decision decision is recorded with this structure.
...
{
  // Identifiers
  id: string              // Unique decision ID (format: T-YYYY-NNNN)
  timestamp: datetime     // Decision timestamp (ISO 8601 UTC)
  
  // Decision Parameters
  symbol: string          // Decision-Making instrument (XAUUSD, BTCUSDT, etc.)
  ...
  decision context_context: {
    ...
    session: string       // Decision-Making session (asian/london/newyork)
    indicators?: dict     // Optional: relevant indicator values
    news_sentiment?: float // Optional: -1.0 to 1.0
  }
  references: string[]    // References to past decisions (e.g., ["T-2026-0247"])
...
### Example: Recording a Decision Decision

**Request to `decision.record_decision`:**
...
```json
{
  ...
  "confidence": 0.72,
  "reasoning": "London session open with strong momentum above 20-period high. Volume spike confirmed at 09:03. Price broke 2890 resistance with conviction. Similar setup to successful decision T-2026-0247 on Feb 15.",
  "decision context_context": {
    "price": 2891.50,
    "atr": 28.3,
    "session": "london",
...
```json
{
  "decision_id": "T-2026-0251",
  "timestamp": "2026-02-23T09:03:42Z",
  "status": "recorded"
}
```

### Example: Recording Decision Outcome

**Request to `decision.record_outcome`:**
...
```json
{
  ...
  "lessons": "Good entry timing on volume confirmation. Exit was premature — price continued to 2902. Consider implementing trailing stop for strong momentum decisions.",
  "hold_duration": 87,
  "slippage": 0.3,
...
```typescript
{
  ...
  curated_insights: {
    ...
    evidence: string[]    // Supporting decision IDs
    created_at: datetime
  }[]
  ...
  risk_constraints: {
    max_lot_size: float
    risk_per_decision_percent: float
  ...
  performance: {
    ...
    win_rate: float
    avg_pnl_r: float
    current_streak: int           // Positive = wins, negative = outcome setbacks
    sharpe_ratio?: float
  }
}
```
...
```json
{
  ...
  "curated_insights": [
    {
      "pattern": "Asian session breakout decisions have 25% win rate vs 80% in London session",
      "confidence": 0.92,
      "evidence": ["T-2026-0241", "T-2026-0243", "T-2026-0245", "T-2026-0248"],
      "created_at": "2026-02-22T23:00:00Z"
    },
    {
      "pattern": "Decisions with confidence > 0.7 have 2x better win rate than confidence < 0.5",
      "confidence": 0.87,
  ...
  "risk_constraints": {
    "max_lot_size": 0.05,
    "risk_per_decision_percent": 1.0,
    "allowed_sessions": ["london", "newyork"],
    "reason": "Asian session performance poor (3 consecutive outcome setbacks). Reduced exposure until pattern improves."
  },
...
```json
{
  ...
  "patterns": [
    {
      "type": "positive",
      "description": "VolBreakout strategy performed exceptionally in London session (5/5 wins). Confidence correlation strong: decisions with confidence > 0.7 had 80% win rate vs 40% below 0.7.",
    ...
    {
      "type": "negative",
      "description": "Asian session entries continue to underperform. 3 of 4 outcome setbacks were Asian session. Low volatility causing false breakouts.",
    ...
    },
    {
      ...
      "confidence": 0.72,
      "recommendation": "Test trailing stop approach on strong momentum decisions."
    }
  ],
  ...
  "action_items": [
    ...
    "Test trailing stop implementation on London session decisions",
...
{
  // Global Constraints
  max_lot_size: float           // Maximum position size per decision
  risk_per_decision_percent: float // Max risk as % of account
  max_daily_outcome setback: float         // Circuit breaker (optional)
  max_open_decisions: int          // Maximum concurrent positions
  
  // Session-Specific
  allowed_sessions: string[]    // Which decision-making sessions are allowed
  session_constraints?: {
    [session: string]: {
      max_lot_size: float
  ...
  strategy_constraints?: {
    [strategy: string]: {
      max_lot_size: float
      min_confidence: float     // Minimum confidence to decision this strategy
    }
  }
...
```json
{
  "max_lot_size": 0.08,
  "risk_per_decision_percent": 1.0,
  "max_daily_outcome setback": 150.0,
...
- Auto-generated by DecisionJournal on `record_decision`
...
### R-Multiples
- Standardized way to measure decision performance
- `pnl_r = actual_pnl / initial_risk`
- Example: If you risk $10 and make $21 outcome gain, that's 2.1R

### Sessions
- Standard values: `"asian"`, `"london"`, `"newyork"`
- Custom sessions can be defined per decision context

---

## Validation Rules

DecisionMemory enforces these rules when recording decisions:

1. **Required fields on `record_decision`:**
   - `symbol`, `direction`, `lot_size`, `strategy`, `confidence`, `reasoning`, `decision context_context.price`, `decision context_context.session`
...
5. **Decision outcome can only be recorded once per decision_id**
