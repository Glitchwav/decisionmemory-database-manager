# DecisionMemory Protocol — Complete Tutorial
...
A Claude-powered decision-making assistant that **remembers every decision, discovers patterns, and evolves its strategy over time**.

By the end of this tutorial, you'll see an AI agent go from "stateless calculator" to "decision-maker with memory" — all in under 10 minutes.
...
```bash
curl -sSL https://raw.githubusercontent.com/mnemox-ai/decisionmemory-protocol/master/scripts/platform/install.sh | bash
...
This runs 30 simulated XAUUSD decisions through the full pipeline. No API key needed.

**What you'll see:**

1. **L1 — Decision Recording**: 30 decisions logged with session, strategy, confidence, P&L
2. **L2 — Pattern Discovery**: The reflection engine finds patterns like:
   ...
   - High confidence decisions vastly outperform low confidence
...
DecisionMemory uses a three-layer architecture:
...
DecisionMemory v0.5 adds **OWM (Outcome-Weighted Memory)** — five memory layers that work like a decision-maker's brain:

| Layer | What It Does |
|-------|-------------|
| **Episodic** | Stores each decision with full context (price, regime, reflection) |
| **Semantic** | Bayesian beliefs updated after every decision ("VolBreakout in trending decision contexts: 72% edge") |
| **Procedural** | Running averages per strategy — what actually works |
| **Affective** | EWMA confidence tracking, win/outcome setback streaks, drawdown state |
| **Prospective** | Decision-Making plans with entry/exit criteria and expiry |

### Store a decision via MCP

Ask your Claude agent:

> "Remember my XAUUSD long decision: entered at 5180, exited at 5210, made $90 using VolBreakout. Decision Context was trending up with strong London momentum. ATR was $155. I was 0.8 confident."

This calls `remember_decision` under the hood, which writes to all five memory layers simultaneously.

### Recall memories via MCP

Before entering a decision, ask:

> "What do my memories say about decision-making XAUUSD in a trending decision context? I'm looking at VolBreakout."

This calls `recall_memories` with outcome-weighted scoring — decisions with better outcomes, matching context, and higher confidence rank higher.

### Other OWM tools

- `get_behavioral_analysis` — "Show me my behavioral patterns" → win rates by regime, overdecision-making detection, confidence calibration
- `get_agent_state` — "What's my current state?" → affective state, drawdown, streak info
- `create_decision_plan` — "Create a plan to buy XAUUSD at 5150 if price pulls back to the 20 EMA"
- `check_active_plans` — "Do I have any active decision-making plans?"

---

## Step 5: Evolution Engine

The Evolution Engine uses Claude to **discover decision-making strategies from raw price data**, then backtests and selects survivors across generations.
...
2. Uses Claude to generate candidate decision-making patterns
...
# Calls: evolution_fetch_decision context_data

# 2. Discover patterns
> "Discover 5 decision-making patterns from the BTCUSDT data"
...
## Step 6: Record Your Own Decision (1 minute)
...
In another terminal, record a decision:
...
After recording several decisions, trigger the reflection engine:
...
- "Asian session outcome setbacks correlate with low-volume periods before 02:00 UTC"

---

## Step 8: Use Memory in Your Next Decision
...
db = Database("data/decisionmemory.db")
...
Your AI went from **"stateless calculator"** to **"decision-maker with memory."**
...
DecisionMemory exposes 17 MCP tools across four categories:

### Core Memory (2 tools)

| Tool | Description |
|------|-------------|
| `get_strategy_performance` | Win rate, avg outcome, decision count per strategy |
| `get_decision_reflection` | Get AI-generated reflection on recent decisions |

### OWM Cognitive Memory (6 tools)

| Tool | Description |
|------|-------------|
| `remember_decision` | Store decision into all 5 OWM layers (episodic → semantic → procedural → affective) |
| `recall_memories` | Outcome-weighted recall with context matching and recency scoring |
| `get_behavioral_analysis` | Win rates by regime, overdecision-making detection, confidence calibration |
| `get_agent_state` | Current affective state — confidence, drawdown, streak |
| `create_decision_plan` | Create a prospective plan with entry/exit criteria and expiry |
...
| Tool | Description |
|------|-------------|
| `evolution_fetch_decision context_data` | Fetch OHLCV data from Binance for backtesting |
...
| `verify_audit_hash` | Verify integrity of a decision by recomputing its hash |

---

## Next Steps

- **Connect to MT5**: See [MT5 Setup Guide](../MT5_SYNC_SETUP.md) to sync real decisions
...
**Questions?** Open an issue: [GitHub Issues](https://github.com/mnemox-ai/decisionmemory-protocol/issues)
