...
This document defines the output format for DecisionMemory's reflection reports. These reports are what users see when their agent "reflects" on its decision-making performance.

---

## Design Philosophy

**Goal:** Make the reflection report feel like a human decision-maker's journal entry — insightful, actionable, and honest about mistakes.
...
Asian session isn't working. I took 4 decisions there and lost 3 of them. 
...
|------|-----------|---------|--------|
| **Daily Summary** | End of each decision-making day | Quick performance check, immediate adjustments | ~150 words |
| **Weekly Review** | End of decision-making week | Pattern discovery, strategy analysis, risk parameter updates | ~500 words |
| **Monthly Deep Dive** | End of month | Strategic evolution, collection-level insights | ~1000 words |
| **Incident Review** | Triggered by large outcome setback | Root cause analysis, preventive measures | ~300 words |
...
**Trigger:** End of decision-making day (or when `reflect.run_daily` is called)

**Structure:**

```markdown
# Daily Decision-Making Summary — [Date]

## Today's Performance
- Decisions: [N] ([W] wins / [L] outcome setbacks)
- Net P&L: [+/-$XX.XX] ([+/-R.RR]R)
- Win rate: [XX%]

## Notable Events
[2-3 bullet points about interesting decisions or decision context conditions]
...
```markdown
# Daily Decision-Making Summary — February 23, 2026

## Today's Performance
- Decisions: 3 (2 wins / 1 outcome setback)
- Net P&L: +$42.30 (+1.8R)
- Win rate: 66.7%

## Notable Events
- ✅ Excellent London open decision on XAUUSD (T-2026-0251) — caught the momentum early, 2.1R gain
...
**Trigger:** End of decision-making week (Friday close or Monday morning)
...
> at the session open. Decisions with confidence > 0.7 in this setup have 
> an 80% win rate.
>
...
**Example:**
> Asian session continues to be a problem. 3 out of 4 outcome setbacks this week 
...
> next week. If the pattern doesn't improve in 2 weeks, I'll stop decision-making 
...
> I want to test a trailing stop approach on strong momentum decisions.
...
- [ ] Monitor Asian session pattern — stop decision-making there if performance doesn't improve
- [ ] Test trailing stop implementation on London momentum decisions
- [ ] Watch for NFP announcement Friday — high volatility expected

---

## Top Decisions of the Week

**Best Decision: T-2026-0251**
- **Strategy:** VolBreakout
- **P&L:** +$28.50 (+2.1R)
- **What went right:** Perfect entry on London open, volume confirmation, exited at 2R target
- **Lesson:** This is the template for a great decision

**Worst Decision: T-2026-0245**
- **Strategy:** VolBreakout
- **P&L:** -$15.20 (-1.0R)
- **What went wrong:** Asian session false breakout, stopped out before reversal
- **Lesson:** Stop decision-making Asian breakouts until volatility returns
...
I went 5 for 5 on London session breakout decisions this week. The pattern 
is clear: when I wait for volume confirmation at the London open (09:00-09:15), 
and price breaks a key level with conviction, the decision almost always works.

There's also a strong confidence correlation: my decisions with confidence > 0.7 
had an 80% win rate, while decisions with confidence < 0.5 only had a 40% win rate. 
This tells me I should trust my gut on these setups.

*Evidence: T-2026-0241, T-2026-0243, T-2026-0247, T-2026-0249, T-2026-0251*

**My decision:** I'm increasing my max lot size for London VolBreakout 
decisions from 0.05 to 0.08. I've earned this.
...
3 out of my 4 outcome setbacks this week happened during Asian session. The problem 
...
decision context actually moves in my direction — but I'm already out.
...
I'll stop decision-making Asian session entirely.
...
I noticed something interesting: the two decisions where I held past 45 minutes 
(T-2026-0247 and T-2026-0249) had an average R of 1.8, while my quick exits 
averaged only 1.1R.

My current exit strategy is to take outcome gain at 2R or if momentum fades. But 
...
on strong momentum decisions.

**Test plan:** Next week, on London session decisions with confidence > 0.75, 
...
| Risk per decision | 1.0% | 1.0% | No change — comfortable at this level |
...
- [ ] Test trailing stop approach on London momentum decisions (confidence > 0.75)
...
- **P&L:** -$15.20 (-1.0R)
- **Entry reasoning:** "Breakout above 2885 with moderate momentum. Similar to previous Asian session decisions."
...
- **Lesson:** Stop decision-making Asian breakouts. The volatility isn't there. This is the third time this pattern has happened.
...
```
You are a decision agent reflecting on today's performance. 

Review the following decisions from today:
[Insert decision data]

Write a brief daily summary (150 words max) that covers:
1. Performance metrics (decisions, win/outcome setback, P&L)
2. 2-3 notable events or interesting decisions
...
```
You are a decision agent reflecting on this week's performance.

Review the following decisions from this week:
[Insert decision data from the week]
...
5. Highlights best and worst decisions
