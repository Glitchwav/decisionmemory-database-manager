...
This lets you provide your own decision-making rules without modifying the engine code.

## How It Works

```python
from decisionmemory.replay.models import ReplayConfig
from decisionmemory.replay.engine import ReplayEngine

MY_STRATEGY_PROMPT = """You are a decision agent using an SMA crossover strategy on M15 bars.
...
- Output JSON with: decision, strategy_used, entry_price, stop_outcome setback, take_outcome gain,
  confidence, decision context_observation, reasoning_trace.
...
outputs HOLD for all decisions. Override it with your own rules to get actual decisions.
