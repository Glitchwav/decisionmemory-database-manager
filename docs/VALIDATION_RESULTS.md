...
| Strategy | Sharpe | Percentile | Decisions | Win Rate | PF | Total outcome | Pass? |
|----------|--------|------------|--------|----------|----|-----------|-------|
...
persists across different decision context regimes (bull, bear, sideways).
...
| Window | Test Period | IS Sharpe | OOS Sharpe | OOS Decisions | OOS WR | OOS outcome |
|--------|------------|-----------|------------|------------|--------|---------|
...
of computing DD% on short-window outcome accumulation where score is near zero. The formula
(peak - score) / peak * 100 explodes when peak score is very small. This does NOT
...
| **E** | P100% PASS | 61% positive, mean 3.24 | **Real edge** -- consistently outcome gainable OOS |
...
The trend filter alone makes 9-12/24 hours outcome gainable (vs 0/24 without it from Step 1 ablation).
...
| Decisions | 786 | 130 | 811 | 161 |
| Win Rate | 38.2% | 43.8% | 42.3% | 46.0% |
| Outcome Gain Factor | 0.98 | 2.10 | 0.99 | 2.62 |
| outcome | -$3,021 | -- | -$1,702 | -- |
...
1. BTC 2020-2024 includes extreme regime changes (COVID crash, 3x bull run, 75% bear decision context, recovery)
2. The strategies were discovered on 2024-2026 BTC data -- a specific decision context regime
