# DecisionMemory API Reference

DecisionMemory exposes its functionality via HTTP endpoints. Any MCP-compatible AI agent or HTTP client can use these endpoints.
...
Record a decision entry decision with full context.

**Request body:**

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `decision_id` | string | yes | Unique decision ID (format: `T-YYYY-NNNN`) |
| `symbol` | string | yes | Decision-Making instrument (e.g. `XAUUSD`) |
...
| `decision context_context` | object | yes | See [Decision ContextContext](#decision contextcontext) |
| `references` | string[] | no | Past decision IDs referenced (default: `[]`) |
...
resp = requests.post("http://localhost:8000/decision/record_decision", json={
    "decision_id": "T-2026-0251",
    "symbol": "XAUUSD",
    "direction": "long",
    "lot_size": 0.05,
...
data = resp.json()
print(data["decision_id"])   # T-2026-0251
print(data["timestamp"])  # 2026-02-23T09:03:42+00:00
```

---

#### `POST /decision/record_outcome`

Record the outcome of a decision after it closes.

**Request body:**

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `decision_id` | string | yes | ID of the decision to update |
...
| `lessons` | string | no | What was learned from this decision |
...
- `400` — Decision ID not found, invalid execution_quality (not 0.0-1.0), or database write failure.

**Example (Python):**

```python
resp = requests.post("http://localhost:8000/decision/record_outcome", json={
    "decision_id": "T-2026-0251",
    "exit_price": 2897.20,
    "pnl": 28.50,
    "pnl_r": 2.1,
...
#### `POST /decision/query_history`

Search past decisions by strategy, symbol, or both.
...
| `symbol` | string | no | Filter by decision-making instrument |
...
Decisions are returned ordered by `timestamp DESC` (newest first). Each decision object contains all `DecisionRecord` fields — see [SCHEMA.md](SCHEMA.md) for the full structure.

**Example (Python):**

```python
# Get all VolBreakout decisions
resp = requests.post("http://localhost:8000/decision/query_history", json={
    "strategy": "VolBreakout",
    "limit": 50
})

decisions = resp.json()["decisions"]
win_rate = sum(1 for t in decisions if t["pnl"] and t["pnl"] > 0) / len(decisions)
print(f"VolBreakout win rate: {win_rate:.0%}")
```

---

#### `GET /decision/get_active`

Get all currently open positions (decisions with no recorded outcome).
...
```python
resp = requests.get("http://localhost:8000/decision/get_active")
active = resp.json()["decisions"]
...
| `active_positions` | string[] | List of open decision IDs |
...
Connect to a MetaDecisionMaker 5 account. Requires the `MetaDecision-Maker5` Python package (Windows only).
...
| `server` | string | yes | Provider server name |
...
- `400` — `MetaDecision-Maker5` package not installed, or connection failed.

---

#### `POST /mt5/sync`

Sync closed decisions from MT5 to DecisionJournal. Groups MT5 deals by position, determines entry/exit, and records both decision and outcome.
...
| `risk.check_decision` | Validate a proposed decision against active constraints |
