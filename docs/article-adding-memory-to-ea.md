...
Your EA executes the same strategy parameters regardless of whether it lost 5 decisions in a row last week or hit a new score high yesterday. It has no memory.

What if your EA could remember every decision it made, why it made it, and what happened afterward — then use that context to inform the next decision?

This article shows how to connect any MQL5 EA to a structured memory layer using DecisionMemory Protocol.
...
3. Watch it decision through regime changes it wasn't optimized for
4. Re-optimize with new data
5. Repeat

The EA never *learns* from its own live experience. It doesn't know that:
- VolBreakout lost 4/5 decisions during last week's ranging decision context
- IntradayMomentum performs 2x better during London session
- Spread was 45pts during the last SL hit (vs normal 15pts)

This information exists in your decision history. But it's not accessible to the EA at decision time.

## Section 2: What is a Decision Memory Layer?

A memory layer sits between your EA and a database, providing:

1. **Episodic Memory** — Raw record of every decision with full context (WHY did the EA enter?)
2. **Semantic Memory** — Learned beliefs updated over time ("VolBreakout in trending regime: 72% outcome gainable")
3. **Anti-Resonance** — Forced recall of losing decisions to prevent overconfidence
...
void LogDecisionEvent(string evt, string reason, double score) {
...
```python
# mt5_sync reads the CSV, matches by position_id, sends to DecisionMemory
...
# Returns: reasoning, confidence, decision context_data (ATR, EMA, spread)
```

### Step 3: Query memory before new decisions

```python
# Before entering a new decision:
...
## Section 5: What You Get — Decision
...
- Belief convergence rate across decision context regimes

## Section 7: Getting Started

```bash
pip install decisionmemory-protocol
python -m decisionmemory  # starts MCP + REST server on port 8000
```

Connect your EA in 3 steps:
1. Write event_log CSV from MQL5 (10 lines of code)
2. Run mt5_sync_v3.py (provided, zero config)
3. Query `/owm/recall` before new decisions

GitHub: [mnemox-ai/decisionmemory-protocol](https://github.com/mnemox-ai/decisionmemory-protocol)
