...
ReflectionEngine 是 DecisionMemory Protocol 的核心模組，負責分析交易記錄並產生 AI 驅動的每日反思報告。
...
PERFORMANCE:
Decisions: N | Winners: W | Losers: L
...
- 只在交易日使用 LLM（非交易日檢查 decisions=0 自動 skip）
...
class CustomReflection(ReflectionEngine):
    def _generate_llm_summary(self, target_date, decisions, metrics, llm_provider):
        # 自訂 prompt
        custom_prompt = f"""
...
- ✅ `test_daily_summary_no_decisions` - 無交易日處理
- ✅ `test_daily_summary_with_decisions` - 正常交易日
...
```python
# 只在交易數 >= 3 時使用 LLM
decisions = engine._get_decisions_for_date(target_date)
if len(decisions) >= 3:
