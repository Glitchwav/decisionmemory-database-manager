...
2. 「建立基本工作」
3. 設定：
   - **名稱**：`DecisionMemory Daily Reflection`
   - **觸發程序**：每天
   - **時間**：23:55
   - **動作**：啟動程式
     - 程式：`C:\OpenClawWork\decisionmemory-protocol\start_daily_reflection.bat`
...
```powershell
# 以管理員權限執行 PowerShell
schtasks /create /tn "DecisionMemory Daily Reflection" /tr "C:\OpenClawWork\decisionmemory-protocol\start_daily_reflection.bat" /sc daily /st 23:55 /ru SYSTEM
```
...
PERFORMANCE:
Decisions: 3 | Winners: 2 | Losers: 1
Net P&L: $75.50 | Win Rate: 66.7% | Avg R: 1.5

KEY OBSERVATIONS:
- Win rate above 60% indicates edge is present
- London session decisions outperformed Asian session
...
# 傳遞給 DecisionMemory server（需要修改 server.py）
```
...
**每日自動化運行需要**：
- ✅ DecisionMemory server 正在運行（port 8000）
- ✅ Windows Task Scheduler 已設定 23:55 執行
- ✅ `.env` 設定正確（DECISIONMEMORY_API）
- ✅ `reflections/` 目錄可寫入
...
# 應回傳
{"status":"healthy","service":"DecisionMemory Protocol","version":"0.1.0"}
```
