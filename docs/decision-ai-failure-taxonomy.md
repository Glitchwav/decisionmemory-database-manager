# Decision AI Failure Taxonomy

> **Version**: 1.0 | **Date**: 2026-04-04 | **Author**: Mnemox AI Research
>
> A systematic classification of failure modes in AI-driven decision-making systems, based on 7+ rigorous experiments conducted by Mnemox AI across XAUUSD, BTCUSDT, and multi-strategy collections from 2024 to 2026.

## Why This Exists

RAG systems have WFGY's 16 failure modes (adopted by LlamaIndex, RAGFlow). Decision AI has no equivalent — despite failure rates being far higher and decision consequences being real.

Every failure mode below is backed by actual experiment data: Sharpe ratios, p-values, decision counts, and DSR (Deflated Sharpe Ratio) gates. No simulated scenarios. No hypothetical examples.

**Data sources**: Sulci Phase 1-2 (M=7, 35,813 bars), Phase 15 Re-Evolution (M=19,200 grid + M=30 LLM), Memory Injection Experiment (4 OOS windows), Permutation Test (1,000 random shuffles), DecisionDream Experiment (247 decisions / 27 months), Phase 15 Sharpe Bug analysis.
...
### Description
策略只在特定 decision context regime 有效，regime 切換後失效。大多數 Decision AI 不偵測 regime，導致在錯誤環境下持續交易。
...
**0/4 windows significant。Memory-based decision filtering 不比隨機好。** Sharpe 9.57 是幻覺。

### Symptoms
- 策略改善看起來很大 (Sharpe 3x 提升) 但沒做 permutation test
- 所有 entry context 長得一樣 (e.g. Strategy E: H14 + trend>0)，similarity 無法區分贏輸
- 文件宣稱 "+30% outcome improvement" 但標注 "Simulated"

### Root Cause
Decision filtering (skip/no-skip) 改變了 decision composition。如果 75% 交易被跳過，隨機跳 75% 也可能產生類似效果。Permutation test 是唯一能分離「smart selection」和「random luck」的方法。

### Fix Direction
任何 decision filtering 改善都必須通過 permutation test (P >= 95)。不是可選的 robustness check，是必要的因果驗證。
...
53 筆交易不足以做任何統計推斷。LightGBM 的 max probability 不是好的 decision filter — 高信心預測不等於高品質預測。
...
### Root Cause
ML 分類器的 probability output 是 calibrated 的 class probability，不是 decision quality score。高 probability 只代表模型「比較確定」這個方向，不代表這個方向的 edge 比較大。

### Fix Direction
先確認 decision filter 的 T 在過濾後仍足夠 (建議 T >= 100)，再評估 filter 效果。或者不做 binary filter，改用 continuous sizing。
...
### Real Evidence
DecisionDream Experiment: 3 策略 (VB 102 decisions, IM 112, PB 33) 合計 247 decisions / 27 months = ~9 decisions/month。原始計畫假設 200-500 decisions/month，實際差 20 倍。OOS window T >= 100 的門檻不可能達到 — 每月 OOS 只有 2-17 decisions。

Phase 15 Grid WFO 也有類似問題: M=19,200 + 30-50 decisions/period → DSR threshold ~0.7 raw Sharpe → 數學上不可能通過。

### Symptoms
- 每月交易量 < 30 筆
- Walk-forward OOS 窗口 decision count < 100
...
### Misrepair Risk
放寬統計門檻 (T >= 20) 會讓 false positive rate 上升。這不是修復，是 decisionoff。最誠實的做法可能是承認「這個頻率下無法統計驗證」。
...
### Root Cause
Grid search 的 multiple testing penalty 太重 (M=19,200 → threshold ~0.7 raw Sharpe)。在 30-50 decisions/period 的條件下，0.7 Sharpe 數學上不可能。不是演化概念失敗，是 search space 太大而 evidence 太少。
...
### Real Evidence
Phase 15 Sharpe Bug: `backtester.py` 用 `sqrt(6048)` 年化 decision-level outcomes — over-annualize ~6-8x。修正前 Grid WFO 顯示 "G > A 91.3%"（看起來 grid 比 random 好），修正後 **0/23 DSR gate pass, mean Sharpe = 0.0**。

前一版結論是「grid search 有效」，修正後結論是「grid search 完全失敗」。一個年化公式 bug 翻轉了整個實驗結論。

### Symptoms
- Decision-level Sharpe 被乘以 sqrt(N) 而 N 是年化的交易次數而非時間週期
- 同窗口比較不應年化 (用 raw Sharpe)
- 修正前後結論完全相反

### Root Cause
年化因子 sqrt(N) 中的 N 應該是「每年的獨立觀察次數」。如果用 decision count (e.g. 6048 decisions/year) 而不是 time periods (e.g. 252 decision-making days)，會嚴重高估。且 decision-level returns 不是等間隔的，sqrt(N) annualization 的前提不成立。

### Fix Direction
同窗口比較一律用 raw Sharpe。跨窗口比較才年化，且用 calendar-based periods (sqrt(252) for daily, sqrt(52) for weekly)，不用 decision count。
...
### Real Evidence
Memory Injection Experiment 發現 OWM recall 設計缺陷: `outcome_weight` 使用 sigmoid 函數偏向正面記憶 (positive outcome)，導致 recall 永遠抓到贏的交易 → 永遠不觸發 skip logic。
...
### Symptoms
- Memory recall 結果偏向正面 (高 outcome decisions)
- Skip logic 幾乎不觸發因為 recalled decisions 都是贏的
- 系統在連續虧損後仍然 "confident" 因為記憶裡都是好結果

### Root Cause
OWM 的 outcome_weight sigmoid 是 monotonically increasing — outcome 越高 weight 越大。這在「找最好的參考」場景合理，但在「評估風險」場景完全反向。
...
| T5 | Filter Overcorrection | Backtesting | High | Easy | 27,381 decisions → 53 decisions (99.8% 被過濾) |
| T6 | Sample Starvation | Statistical | High | Easy | 247 decisions / 27mo = 9/mo, 需要 200-500/mo |
...
**The single most important lesson from 7+ experiments**: Statistical significance is necessary but not sufficient. Permutation tests are the only way to establish causation in decision filtering. And edge, once found, resists all attempts to be made thicker.
