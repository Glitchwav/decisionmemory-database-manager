...
DecisionMemory 使用三層架構：

| 層級 | 名稱 | 儲存內容 | 範例 |
|------|------|---------|------|
| **L1** | 熱記憶 | 進行中的交易、當前 session | "XAUUSD 做多 @ 2847, 信心 0.78" |
| **L2** | 溫記憶 | 發現的規律、洞察 | "倫敦盤突破：73% 勝率" |
| **L3** | 冷記憶 | 完整交易歷史（SurrealDB） | 全部 30 筆交易的完整上下文 |

**關鍵洞察：** 大多數 AI agent 只有 L1（當前上下文）。DecisionMemory 加上了 L2 和 L3，讓 agent 能隨時間累積知識。

---

## 第四步：OWM 認知記憶

DecisionMemory v0.5 新增了 **OWM（Outcome-Weighted Memory）** — 五層記憶架構，模擬交易員的大腦運作方式：
...
這會呼叫 `remember_decision`，同時寫入五層記憶。
...
- `create_decision_plan` — 「建立計畫：如果 XAUUSD 回調到 20 EMA，在 5150 做多」
...
# 呼叫：evolution_fetch_decision context_data
...
DecisionMemory 提供 17 個 MCP 工具，分為四大類：
...
| `get_decision_reflection` | 取得 AI 生成的近期交易反思 |

### OWM 認知記憶（6 個工具）

| 工具 | 說明 |
|------|------|
| `remember_decision` | 將交易寫入 5 層 OWM 記憶（情景 → 語義 → 程序 → 情感） |
| `recall_memories` | 結果加權回憶，含上下文匹配與時間衰減評分 |
| `get_behavioral_analysis` | 各市場狀態勝率、過度交易偵測、信心校準 |
| `get_agent_state` | 目前情感狀態 — 信心、回撤、連勝/連虧 |
| `create_decision_plan` | 建立前瞻計畫，含進出場條件與到期時間 |
...
| 工具 | 說明 |
|------|------|
| `evolution_fetch_decision context_data` | 從 Binance 抓取 OHLCV 數據用於回測 |
...
**有問題？** 開一個 issue：[GitHub Issues](https://github.com/mnemox-ai/decisionmemory-protocol/issues)
