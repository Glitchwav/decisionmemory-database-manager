# DecisionMemory README Rewrite + Commercial Packaging — Implementation Plan
...
**Architecture:** Documentation-only changes. Three new/rewritten markdown files in the decisionmemory-protocol repo. No code changes. Content based on approved design spec at `docs/superpowers/specs/2026-04-05-readme-rewrite-design.md`.
...
git commit -m "docs: add Getting Started guide — dual track (decision-maker + developer)"
```
...
Content to preserve:
- Line 1: `<!-- mcp-name: io.github.mnemox-ai/decisionmemory-protocol -->`
...
It makes the same mistakes every session. It can't explain why it decisiond. It forgets everything when the context window ends. Meanwhile, MiFID II is raising the bar for algorithmic decision documentation ([Article 17](https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX:32014L0065)). The EU AI Act demands systematic logging of AI actions ([Article 14](https://eur-lex.europa.eu/eli/reg/2024/1689)). Your competitors' agents are learning from every decision.

The AI decision-making stack is missing a layer. Every MCP server handles execution — placing orders, fetching prices, reading charts. **None handle memory.**

Your agent can buy 100 units of AAPL but can't answer: *"What happened last time I bought AAPL in this condition?"*

**DecisionMemory is the memory layer.** One `pip install`, and your AI agent remembers every decision, every outcome, every mistake — with SHA-256 tamper-proof audit trail.

Used in production by decision-makers running pre-flight checklists before every position, and by EA systems logging thousands of decisions daily.

## What it does

- **Before decision-making:** ask your memory — what happened last time in this decision context condition? How did it end?
...
Works with any decision context (records, forex, crypto, futures), any provider, any AI platform. DecisionMemory doesn't execute decisions or touch your money — it only records and recalls.
...
**Full walkthrough:** [Getting Started](docs/GETTING_STARTED.md) (Decision-Maker Track + Developer Track)

## Who uses DecisionMemory

| | US Score Decision-Maker | Forex EA System | Compliance Team |
|---|---|---|---|
| **Decision Context** | Records (AAPL, TSLA, ...) | XAUUSD (Gold) | Multi-asset |
| **How** | Pre-flight checklist before every decision | Automated sync from MT5 | Full decision audit trail |
...
| **Details** | [Read more →](docs/USE_CASES.md#case-1-us-score-decision-maker--pre-flight-workflow) | [Read more →](docs/USE_CASES.md#case-2-forex-ea-system--automated-memory-loop) | [Read more →](docs/USE_CASES.md#case-3-compliance-first-fund--audit-trail) |
...
1. **Recall** — Before decision-making, retrieve past decisions weighted by outcome quality, context similarity, recency, confidence, and emotional state ([OWM Framework](docs/OWM_FRAMEWORK.md))
2. **Record** — After decision-making, one call to `remember_decision` writes to five memory layers: episodic, semantic, procedural, affective, and decision records
3. **Reflect** — Daily/weekly/monthly reviews detect behavioral drift, strategy decay, and decision-making mistakes
...
|----------|-------|-------------|
| **Memory** | `remember_decision` · `recall_memories` | Record and recall decisions with outcome-weighted scoring |
| **State** | `get_agent_state` · `get_behavioral_analysis` | Confidence, drawdown, streaks, behavioral patterns |
| **Planning** | `create_decision_plan` · `check_active_plans` | Prospective plans with conditional triggers |
| **Risk** | `check_decision_legitimacy` | 5-factor pre-decision gate (full / reduced / skip) |
...
**REST API:** 35+ endpoints for decision recording, reflections, risk, MT5 sync, OWM, evolution, and audit. [Full reference →](docs/API.md)
...
Every decision-making decision your agent makes — including decisions **not** to decision — is recorded as a Decision (TDR), SHA-256 hashed at creation for tamper detection.
...
- **Never touches API keys.** DecisionMemory does not execute decisions, move funds, or access wallets.
- **Read and record only.** Your agent passes decision context to DecisionMemory. It stores it. That's it.
...
| Doc | Description |
|-----|-------------|
| [Getting Started](docs/GETTING_STARTED.md) | Install → first decision → pre-flight checklist |
...
| [Failure Taxonomy](docs/decision-ai-failure-taxonomy.md) | 11 decision AI failure modes |
...
- `docs/README_ZH.md` ✓
- `docs/decision-ai-failure-taxonomy.md` ✓
...
```bash
for f in docs/GETTING_STARTED.md docs/USE_CASES.md docs/API.md docs/OWM_FRAMEWORK.md docs/ARCHITECTURE.md docs/TUTORIAL.md docs/MT5_SYNC_SETUP.md docs/RESEARCH_LOG.md docs/README_ZH.md docs/decision-ai-failure-taxonomy.md; do
...
- `mnemox.ai/decisionmemory` landing page (different repo: mnemox-web)
