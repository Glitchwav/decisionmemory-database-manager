...
```bash
pip install MetaDecision-Maker5 python-dotenv requests
```
...
```
============================================================
MT5 → DecisionMemory Sync Script
============================================================
API Endpoint: http://localhost:8000
Sync Interval: 60s
MT5 Account: your_login_here @ YourProvider-Server
============================================================

[OK] Connected to MT5: REDACTED_NAME (YourProvider-Server)
[OK] Account: your_login_here, Balance: $10000.00

[OK] Monitoring started. Press Ctrl+C to stop.

[SCAN] Found 1 new closed decision(s)
...
```batch
@echo off
cd /d C:\OpenClawWork\decisionmemory-protocol
python scripts/mt5_sync.py
pause
```
...
2. 「建立基本工作」
3. 名稱：`MT5 DecisionMemory Sync`
4. 觸發程序：「當電腦啟動時」
5. 動作：「啟動程式」
   - 程式：`C:\OpenClawWork\decisionmemory-protocol\start_mt5_sync.bat`
6. 完成
...
### 2. 查詢 DecisionMemory API

```bash
curl http://localhost:8000/decision/get_active
```

或用 Python：
```python
import requests
r = requests.get('http://localhost:8000/decision/get_active')
print(r.json())
```
...
### Q: `MetaDecision-Maker5 package not installed`

**解決**：
```bash
pip install MetaDecision-Maker5
```
...
**檢查**：
1. DecisionMemory server 是否運行？(`http://localhost:8000/health`)
2. 防火牆是否阻擋 localhost 連線？
3. `.env` 中的 `DECISIONMEMORY_API` 是否正確？
...
- ✅ DecisionMemory server 也設定自動啟動（或手動啟動）
...
3. **歷史交易**：腳本重啟後會重新掃描所有歷史交易（可能產生重複，DecisionMemory 會檢查 `decision_id` 去重）
