...
- 在 `.skills/decisionmemory/` 建立 SKILL.md
...
### Task 1.2：建立安裝腳本 ✅
- `.skills/decisionmemory/scripts/install.sh`：自動安裝 decisionmemory-protocol + 依賴
- `.skills/decisionmemory/scripts/setup_mt5.sh`：互動式引導 MT5 同步設定
...
### Task 2.1：建立 DecisionMemory Landing Page ✅
- 在 mnemox.ai repo 建立 /decisionmemory 頁面
- Hero section：「AI Decision Memory for MT5/Forex Decision-Makers」
- 功能展示：3-Layer Memory、AI Reflection、Pattern Discovery、Daily Reports
- 示範 GIF 或截圖（可以先用 placeholder）
- Claude Desktop 整合說明
- Commit: `feat: add DecisionMemory landing page` → `95cd5a7`

### Task 2.2：建立 Pricing Page ✅
- 在 landing page 下方加入 Pricing section
- 三個方案：Decision-Maker $29/月、Pro $79/月、Fund $299/月
...
- 在 repo 建立 `decision contexting/forex-factory-post.md`
- 標題、內容、截圖說明
- 重點：「First AI decision memory for MT5」
- 語調：交易者對交易者，不是工程師對工程師
- Commit: `docs: add Forex Factory promotional content`

### Task 3.2：撰寫 Reddit/MQL5 推廣帖 ✅
- `decision contexting/reddit-post.md`
- `decision contexting/mql5-post.md`
- 每個平台語調略有不同
- Commit: `docs: add Reddit and MQL5 promotional content`

### Task 3.3：撰寫 demo 腳本 ✅
- `decision contexting/demo-script.md`
- 影片示範的步驟腳本（2-3 分鐘）
...
- 定價邏輯（Free/Decision-Maker/Pro/Fund 月費 + Starter/Growth/Scale credit packs）
...
### Task 6.2：OWM 實作 ✅
- 5 個 OWM 模組（`src/decisionmemory/owm/`：context.py / recall.py / kelly.py / migration.py / __init__.py）
...
## Phase 7：Decision-Making Intelligence Dashboard ✅
...
- 分層架構：dashboard_api.py → services/dashboard.py → repositories/decision.py
- Pydantic response models（dashboard_models.py）
- CORS middleware + 自定義例外

### Task 7.5：Time Series + Memory Growth API ✅
- /dashboard/score-curve、/dashboard/rolling-metrics、/dashboard/memory-growth
...
- `src/decisionmemory/data/` 模組：OHLCV model、Timeframe enum、OHLCVSeries
...
- `src/decisionmemory/data/binance.py`：實作 DataSource Protocol
- Binance REST API /klines endpoint（無需 API key for public data）
- Rate limiting（respect 1200 req/min）
- Local parquet cache（~/.decisionmemory/cache/binance/）
- async httpx client
...
自動化的觀察→假說→回測→淘汰 loop。`src/decisionmemory/evolution/` 獨立模組。

### Task 10.1：Evolution Models + Pattern Discovery ✅
- `src/decisionmemory/evolution/` 模組：LLMClient Protocol, AnthropicClient, MockLLMClient
...
- `src/decisionmemory/evolution/backtester.py`
...
- 測試：known-outcome decisions、edge cases

### Task 10.3：Hypothesis Generator ✅
- `src/decisionmemory/evolution/generator.py`
...
- `src/decisionmemory/evolution/selector.py`
- IS fitness → 排名 → top N 進 OOS validation
- OOS validation：Sharpe > 1.0, decision_count > 30, max_dd < 20%
...
- `src/decisionmemory/evolution/engine.py`
...
### Task 11.1：fetch_decision context_data MCP Tool Function ✅
...
- Mock BTC 1H data + 3-generation evolution + text score curve
- 驗證引擎能自動發現正期望值策略
- 12 new tests
...
- Root cause: M=19,200 + 30-50 decisions = mathematically impossible
...
- **Conclusion**: 1H timeframe + 3mo window 交易數不夠通過 DSR gate。瓶頸是 decision count，不是 search method。Evolution Engine 驗證延後到更高頻設定。
...
| 3 | mt5_sync_v3 close retry 邏輯 | 🔧 未 commit | 5 次重試 + abandon，改善 closed decision sync 穩定性 |
...
| Phase 7：Decision-Making Intelligence Dashboard | ✅ 完成 | 2026-03-11 |
