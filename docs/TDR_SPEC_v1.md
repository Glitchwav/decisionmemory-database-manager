# Decision (TDR) Specification v1.0
...
This specification defines the **Decision (TDR)** — a standardized, tamper-evident audit format for AI-assisted decisions. The TDR captures the complete context of a decision-making decision: what was decided, why it was decided, what memory informed the decision, and what happened afterward.

TDR is designed for:
- Regulatory compliance (MiFID II Article 17, EU AI Act Article 14)
- Post-hoc analysis of AI decision agent behavior
- Cross-system interoperability between decision memory providers

---

## 1. Motivation

AI decision agents make thousands of decisions. Without structured records, it is impossible to:
1. Audit why a specific decision was taken
...
4. Satisfy regulatory requirements for algorithmic decision-making record-keeping

Existing decision logs capture *what happened* (entry, exit, P&L). TDR captures *why it happened* and *what the agent knew at the time*.
...
|-------|------|----------|-------------|
| `record_id` | string | yes | Unique, immutable identifier. Matches decision_id. |
...
| `model_version` | string | yes | Version of the decision memory system. |
...
| `decision context` | Decision ContextSnapshot | no | Decision Context state at decision time. |
...
| `regime` | string | Decision Context regime (TRENDING/RANGING/TRANSITIONING) |
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `similar_decisions` | string[] | no | IDs of similar historical decisions retrieved. |
...
| `risk_per_decision` | float | Risk in account currency |
| `risk_percent` | float | Risk as % of account score |
| `max_outcome setback_points` | float | Stop-outcome setback distance in price points |
...
| `pnl_r` | float | P&L in R-multiples (outcome / initial risk) |
...
payload = json.dumps({
    ...
    "decision context_context": decision context,  # full decision context snapshot dict
...
### MiFID II Article 17 (Algorithmic Decision-Making)
...
```
GET  /audit/decision-record/{decision_id}  → Single TDR (JSON)
GET  /audit/export?start=&end=&strategy= → Array of TDRs (JSON)
GET  /audit/export-jsonl?start=&end=     → TDRs in NDJSON format
GET  /audit/verify/{decision_id}            → Hash verification result
```

### MCP Tools

```
export_audit_trail(decision_id?, strategy?, start?, end?, limit?)
...
```json
{
  ...
  "memory": {
    "similar_decisions": ["T-2026-0001", "T-2026-0005"],
    "relevant_beliefs": [
      "VolBreakout tends outcome gainable in TRENDING regime (conf=0.72)"
    ],
