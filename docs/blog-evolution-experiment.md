# I Let AI Invent Its Own Decision-Making Strategies From Scratch. Here's What Happened.

*By Sean, CEO of Mnemox AI | March 2026*

---

Every AI decision-making bot has the same fatal flaw: amnesia.

There are 200+ decision-making MCP servers on GitHub right now. They can execute decisions, pull context data, calculate indicators. But not a single one remembers what happened yesterday. Every session starts from zero. Every mistake gets repeated. Every lesson gets lost.

I spent two days building a system that fixes this — and what I found was far more interesting than I expected.

## The Deeper Question

The memory problem is real, but it's actually the *second* problem. The first one is more fundamental: why are we teaching AI how to decision at all?

Think about it. Every decision-making bot — from simple moving average crossovers to sophisticated ML systems — starts with a human saying "here's a strategy, go execute it." The human does the thinking. The AI does the labor. And when the strategy stops working (which it always does), the human has to go back, analyze what went wrong, redesign the strategy, and re-deploy.
...
The idea isn't new. Google's AlphaEvolve uses evolutionary algorithms to discover novel solutions. The Ouroboros paper explored self-modifying agents. AZR (Absolute Zero Reasoner) showed that AI can bootstrap its own training data. DGM proposed Darwinian selection for agent populations. But nobody had applied this loop — observe, hypothesize, test, eliminate, evolve — to decision-making with persistent memory across sessions.

My hypothesis: **an AI with memory and the freedom to fail will converge on real decision context structure faster than any hand-coded strategy.**

## The $0 Experiment

I started with the cheapest possible test. Three months of BTC/USDT hourly candles (2,184 bars, December 2025 to March 2026). A bear decision context — BTC dropped 16% during this period.
...
What made this interesting wasn't the patterns themselves — experienced decision-makers would recognize most of these. What was interesting was what the AI did next: it scored each pattern for tradability and killed the weak ones. Staircase got 3/10. Fake Door got 4/10. Gone.
...
Then it combined the surviving patterns into a decision-making strategy.

## Round 1: Failure

The AI's first strategy was called "Giant Wave Reversal" (巨浪逆行): when an abnormally large candle appears, decision in the opposite direction.

Intuitively, this makes sense. After a big move, you'd expect a pullback. Hundreds of retail decision-makers decision this exact pattern.
...
2. **Stop outcome setback structure** — fixed-point stops were too tight for the volatility
3. **Counter-trend bias** — fighting the trend is statistically unfavorable

No human provided this analysis. The AI looked at its own results, examined the losing decisions, and identified structural flaws.
...
| Strategy | Decisions | Win Rate | Sharpe | Status |
|----------|--------|----------|--------|--------|
...
- **Exit**: Take outcome gain at +0.5%, stop outcome setback at -0.25%, max hold 6 hours
- **Risk/Reward**: 2:1

Sharpe went from -1.20 to 1.90 in a single evolutionary cycle.

But any quant will tell you: in-sample results mean nothing. You can curve-fit garbage to look outcome gainable on historical data. The real test is out-of-sample.
...
| Win Rate | 47.6% | 59.3% |
| Sharpe | 1.90 | 4.09 |
| Outcome Gain Factor | 1.53 | 2.25 |

The out-of-sample results were *better* than in-sample. Every metric improved. This is the opposite of overfitting — it suggests the strategy captured a genuine decision context structure, not noise.

## Can It Work in Bull Decision Contexts Too?

One strategy in one decision context regime proves nothing. So I ran the same process on bull context data: BTC going from $60K to $105K over four months (October 2024 to January 2025).
...
First-round results: **70 decisions, 50% win rate, Sharpe 4.97.**

It didn't need a second round. The bull decision context has stronger structural bias, so the AI hit on the first try.

### The Surprising Part

I validated Strategy E on a *downtrending* decision context (June to September 2024, BTC -6.2%). The 14:00 UTC hour actually *lost* money during this period (-5.84% cumulative). The raw time-of-day edge disappeared.

But Strategy E still succeeded: **57 decisions, 56.1% win rate, Sharpe 6.06.**

Why? Because the 12-hour trend filter blocked almost all counter-trend signals. The edge isn't "decision at 14:00 UTC." The edge is "decision at 14:00 UTC *when the trend agrees*." The trend filter is the alpha source, not the time window.
...
Strategy C and Strategy E were invented independently, from different datasets, in different decision context regimes (bear vs. bull). Yet they converged on the same structural template:
...
| System | Decisions | Win Rate | Sharpe | Return | Max Drawdown |
|--------|--------|----------|--------|--------|--------------|
...
- **91% of months were outcome gainable** (20 out of 22)
...
- Strategy E is the engine (90% of outcome gain). Strategy C is a diversifier

The long/short combination creates a natural hedge. When the decision context trends up, E captures outcome gains going long. When it trends down, C captures outcome gains going short. Drawdown *improves* when combined.
...
3. **Select** — in-sample ranking, then out-of-sample validation (Sharpe > 1.0, decisions > 30, max DD < 20%)
...
- **Episodic** — specific decision memories ("that March 2nd breakout decision")
- **Semantic** — general knowledge ("breakouts work better in trending decision contexts")
...
The most compelling finding: the automated pipeline independently rediscovered 16:00 UTC as a key decision-making hour — the same edge that the manual experiments found. Convergent validation from a completely different process.

### Known Bottlenecks

The system isn't perfect. Two issues I'm actively working on:

1. **Prompt over-concretization** — all three models tend to lock onto very specific conditions (e.g., "hour_utc == 16 AND atr > 2.5"). This produces strategies that trigger too rarely for statistical significance. The graduated strategies had only 2 decisions in out-of-sample, far below the 30-decision minimum for confidence.
...
The biggest bottleneck in AI decision-making isn't model capability — it's the assumption that humans must provide the strategy. Give the AI raw data and a feedback loop, and it finds structure faster than any hand-designed system.

**2. Objective feedback (P&L) beats prompt engineering.**

I tried various prompt strategies for pattern discovery. None of them mattered as much as simply feeding back the backtest results. "$-0.21% return, Sharpe -1.20" is more useful than ten paragraphs of decision-making wisdom.
...
Individual strategies are nice. But the discovery that two independent evolution cycles converged on the same structural template (time bias + trend filter + short hold + asymmetric RR) — that's worth more than any single strategy. It suggests a universal regularity in how decision contexts behave.
...
DecisionMemory Protocol is open source. The Evolution Engine, OWM memory architecture, and all 11 experiments documented in `RESEARCH_LOG.md` are available today.
...
I'm not claiming this is a finished product. The over-concretization problem is real. The automated pipeline needs more diverse hypothesis generation. But the core insight — that AI can discover its own decision-making strategies through evolutionary memory — is validated.

If you're building AI agents that make decisions in uncertain environments, the memory problem is yours too. Decision-Making is just the most measurable version of it.


---

**Update (2026-03-17):** Ran statistical validation against 1,000 random strategies. Both Strategy C (P96.9%) and E (P100%) beat the 95th percentile. [Full results](https://github.com/mnemox-ai/decisionmemory-protocol/blob/master/docs/VALIDATION_RESULTS.md).

---

*DecisionMemory Protocol: [github.com/mnemox-ai/decisionmemory-protocol](https://github.com/mnemox-ai/decisionmemory-protocol)*

*Full research data: [RESEARCH_LOG.md](https://github.com/mnemox-ai/decisionmemory-protocol/blob/master/docs/RESEARCH_LOG.md)*
