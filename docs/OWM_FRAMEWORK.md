# Outcome-Weighted Memory (OWM): A Cognitive Memory Architecture for AI Decision Agents
...
> This document is the intellectual foundation of DecisionMemory's competitive moat.
> It defines a complete, implementable memory theory for AI decision agents,
> grounded in cognitive science, reinforcement learning, and quantitative decisions.
...
```
L1 (Raw Decisions) → L2 (Patterns) → L3 (Adjustments)
```

This is a **batch ETL pipeline**. It has three fundamental problems:

**Problem 1: No feedback loop.** L3 adjustments do not modify how L1 data is stored or how L2 patterns are discovered. A real memory system has bidirectional influence — what you know changes what you notice, which changes what you remember. A decision-maker who has learned that "Friday NFP causes violent gold reversals" will encode future Friday decisions differently. L1/L2/L3 cannot represent this.

**Problem 2: No decay or forgetting.** Every L1 record has equal weight forever. A decision from 2022 when gold was $1,800 and a decision from 2026 when gold is $5,175 sit in the same table with the same retrieval priority. Human decision-makers (and any effective learning system) naturally down-weight stale information. The L1/L2/L3 model has no mechanism for this — you either delete old data (losing information) or keep it all (diluting signal).

**Problem 3: No context-dependent recall.** L2 pattern discovery is a batch process: scan all decisions, find statistical patterns, store them. But real decision memory is context-triggered. When a decision-maker sees a specific chart formation during London open with rising yields, they don't search "all patterns" — they recall specific memories that feel similar. The recall path matters more than the storage path.

### 1.2 What L1/L2/L3 Cannot Capture

| Phenomenon | Real Decision-Making | L1/L2/L3 Representation |
|------------|-------------|------------------------|
| "I've seen this exact setup before" | Vivid episodic recall, emotionally tagged | Keyword overlap on `decision context_context` text field |
| "Breakout strategies work in trending decision contexts" | Semantic knowledge, refined over years | A pattern row in L2 with a confidence number |
| "I always cut winners too early" | Procedural habit, often unconscious | Not representable at all |
| "I'm scared after three consecutive outcome setbacks" | Affective state affecting all decisions | Not representable at all |
...
**Failure Mode 2: Cold Memory.** L3 generates an adjustment like "increase RR from 2.5 to 3.5." This adjustment has no memory of why it was proposed, what decision context conditions prompted it, or whether those conditions still hold. If the decision context regime changes, the adjustment persists until a human manually reverts it or another L2 scan overrides it. There is no automatic "this adjustment was proposed during high-volatility conditions and we're now in low-volatility" awareness.

**Failure Mode 3: The Aggregation Trap.** L2 computes aggregate statistics: win rate, outcome gain factor, average outcome. But aggregates destroy the most valuable information — the specific conditions under which a strategy excels or fails. "IM has 94% outcome gainable variants" hides the fact that IM_XAUUSD_BUY_RR3.5_TH0.45 returned +166% while IM_XAUUSD_SELL_RR1.5_TH0.65 returned -12%. The L1/L2/L3 architecture pushes toward aggregation because that's what layers do. OWM pushes toward specific, context-tagged, outcome-weighted memories.
...
L1/L2/L3 has encoding (store a decision) and a crude form of consolidation (aggregate into patterns). It has no retrieval theory (keyword matching is not a theory), no reconsolidation (patterns are immutable once created), and no forgetting mechanism.
...
**Axiom 1 (Outcome Authority):** The decision outcome of a decision is the ultimate arbiter of memory value. A memory that led to a outcome gainable decision in similar conditions deserves stronger recall than one that did not. This is not the same as "only remember winners" — losing decisions in specific conditions are extremely valuable warnings.

**Axiom 2 (Context Is Everything):** A memory's value is not intrinsic — it depends on how similar the current context is to the context in which the memory was formed. A breakout pattern memory formed during a trending decision context is highly relevant when the current decision context is trending, and nearly worthless when the decision context is ranging.
...
These three axioms directly map to Tulving's memory theory (1972, 1985) and to the Prioritized Experience Replay framework from DeepMind (Schaul et al., 2015), adapted from game-playing to decision decision-making.

### 2.1 Episodic Memory (Event Memory)

#### Cognitive Analogue
Tulving's episodic memory — autonoetic consciousness, "mental time travel" to a specific past experience. A decision-maker remembering "that one decision on March 2nd where VB caught the London breakout for +$1,175."
...
- `r` : reflection (natural language — agent's own analysis post-decision)
...
CREATE TABLE episodic_memory (
    ...
    lot_size REAL,
    -- Outcome (NULL if decision still open)
    exit_price REAL,
    pnl REAL,
    pnl_r REAL,                    -- outcome in R-multiples (risk-normalized)
    hold_duration_seconds INTEGER,
    max_adverse_excursion REAL,    -- worst unrealized outcome setback during decision
    -- Meta
    reflection TEXT,
...
1. A decision is **closed** (has exit price and outcome), OR
2. A significant **non-decision event** occurs (e.g., agent decides NOT to decision — a deliberate inaction that should also be remembered)

The context vector `C` is computed at write time from current context data:

```python
def compute_context(symbol: str, mt5_data: dict) -> dict:
    """Compute context vector from current decision context state."""
    return {
        "symbol": symbol,
        "price": mt5_data["bid"],
        ...
        "consecutive_outcome setbacks": count_consecutive_outcome setbacks(),
    }
```
...
- `tau` = time constant (default: 30 days — one month is the natural decision-making cycle)
...
Why power law instead of exponential? Exponential decay (`e^(-t/tau)`) drops too fast — after 3*tau (90 days), a memory would retain only 5% strength. Power law decay is empirically more accurate for human memory and more useful for decision-making: a decision from 6 months ago in the exact same regime should still be retrievable, just with reduced weight.

#### Decision Grounding

Episodic memory maps to **decision journaling** — the most recommended practice in behavioral decisions for improving decision-making performance. Research by Odean (1998) and Barber & Odean (2000) demonstrates that decision-makers who systematically review past decisions make better decisions. OWM automates and enhances this: instead of a human reviewing a journal, the AI agent has structured, searchable, outcome-weighted access to every past decision.
...
#### Cognitive Analogue
Tulving's semantic memory — noetic consciousness, context-free general knowledge. "Breakout strategies work better in trending decision contexts." "Gold correlates with US dollar weakness." "High ATR means wider stops."
...
After observing a decision outcome that tests this knowledge:
...
This gives us not just a confidence value but a **confidence in our confidence** — crucial for distinguishing "I'm 70% sure based on 3 decisions" from "I'm 70% sure based on 300 decisions."
...
The `regime_match_factor` is the key innovation: semantic knowledge formed in a bull decision context rapidly loses retrieval strength when the decision context transitions to a bear decision context, even if the raw confidence is high. This prevents the agent from rigidly applying bull-decision context knowledge in a regime change.

#### Decision Grounding

Semantic memory maps to **statistical edge** — the accumulated knowledge of what works and what doesn't. In quantitative decisions, this is equivalent to a strategy's expected edge, but with crucial additions: domain restrictions (when does the edge exist?) and Bayesian uncertainty (how confident are we?). This directly addresses the problem identified by Marcos Lopez de Prado (2018) in "Advances in Decision Machine Learning" — that backtested edges are often overfit and lack proper out-of-sample validation. The Bayesian framework naturally handles this by starting with wide uncertainty and narrowing it only with genuine confirming evidence.
...
#### Cognitive Analogue
Tulving/Squire's procedural memory — anoetic consciousness, unconscious skills and habits. A decision-maker who "always" places stop-outcome setbacks 1.5 ATR away, or who "always" takes partial outcome gains at 1R, or who "always" hesitates before shorting.
...
This is where OWM fundamentally differs from L1/L2/L3. L1/L2/L3 cannot represent "what the agent actually does" because it only stores individual decisions (L1) and aggregate patterns (L2). Procedural memory tracks **behavioral tendencies** — the difference between the strategy's theoretical behavior and the agent's actual behavior.
...
class ProceduralRecord:
    strategy: str
    symbol: str
    # What the agent actually does (rolling window, last N decisions)
    ...
    outcome setback_aversion_score: float       # asymmetry in behavior after wins vs outcome setbacks
...
CREATE TABLE procedural_memory (
    ...
    window_size INTEGER NOT NULL DEFAULT 50,    -- decisions in window
    ...
    outcome setback_aversion REAL DEFAULT 0.0,
...
Procedural memory is updated after every closed decision. It is a **rolling computation**, not a one-time store:

```python
def update_procedural(strategy: str, symbol: str, new_decision: EpisodicMemory):
    """Update procedural memory with latest decision."""
    ...
    if record.expected_win_rate and record.expected_avg_pnl_r:
        ...
        if kelly_f > 0:
            ...
            # This requires account score to compute fraction — simplified
...
Procedural memory does not decay in the traditional sense — it is **continuously recomputed** from the rolling window. However, the window itself implements implicit decay: only the last N decisions contribute. The window size adapts:
...
Procedural memory maps to **position sizing and execution quality** — the practical mechanics of decision-making that separate a theoretical edge from realized outcome gains. The connection to the Kelly Criterion is direct:
...
Where `p` = win probability, `q` = 1 - p, `b` = win/outcome setback ratio. This is the theoretically optimal bet size. Procedural memory tracks how close the agent's actual sizing is to Kelly optimal, and whether this ratio drifts over time.
...
- `drawdown_state` : in [0, 1] — 0 = at score high, 1 = at max drawdown threshold
...
```
confidence(t) = lambda * confidence(t-1) + (1 - lambda) * signal(latest_decision)
```

Where:
- `lambda` = 0.9 (slow-moving — confidence doesn't jump on a single decision)
- `signal(decision)` = sigmoid(pnl_r / expected_pnl_r) — maps R-multiple outcome to [0, 1]
...
Where:
- `current_drawdown` = (peak_score - current_score) / peak_score
- `max_acceptable_drawdown` = configurable (default: 0.20 = 20%)

Properties:
- At score high (DD = 0%): risk_appetite = 1.0 (full risk)
...
This creates a **concave** risk reduction — the agent reduces risk slowly at first, then aggressively as drawdown deepens. This is consistent with the practical decision-making wisdom of "fractional Kelly" (betting less than the Kelly optimal amount to reduce ruin probability) and with the academic finding that geometric growth is maximized by reducing position size during drawdowns (Vince, 1990).
...
retrieval_bias(memory) = {
    if memory.pnl_r > 0 and drawdown_state > 0.5:
        ...
        boost = 1.5   # Surface large-outcome setback memories as warnings (prevent compounding)
    else:
        boost = 1.0   # Normal retrieval
}
```

This models the empirical finding in behavioral decisions that outcome setbacks loom larger than gains (Kahneman & Tversky, 1979). But instead of treating this as a bias to correct, OWM uses it constructively: during drawdowns, the agent should indeed pay more attention to memories of past outcome setbacks (as warnings) and past recoveries (as stabilizers).
...
CREATE TABLE affective_state (
    ...
    current_drawdown_pct REAL GENERATED ALWAYS AS (
        CASE WHEN peak_score > 0
        THEN (peak_score - current_score) / peak_score
        ELSE 0.0 END
    ) STORED,
    ...
    consecutive_outcome setbacks INTEGER NOT NULL DEFAULT 0,
    -- History (for analysis)
    last_updated TEXT NOT NULL,
    history_json TEXT DEFAULT '[]'  -- Array of {timestamp, confidence, risk_appetite, score}
);
```

#### Write Operation

Affective state is updated after every decision close and periodically (every hour) even without decisions:

```python
def update_affective(decision: Optional[EpisodicMemory] = None, score: float = None):
    state = load_affective_state()

    if decision:
        # Update confidence via EWMA
        signal = sigmoid(decision.pnl_r / expected_pnl_r_for_strategy(decision.strategy))
        state.confidence_level = 0.9 * state.confidence_level + 0.1 * signal

        # Update streaks
        if decision.pnl_r > 0:
            state.consecutive_wins += 1
            state.consecutive_outcome setbacks = 0
        else:
            state.consecutive_outcome setbacks += 1
    ...
    if score:
        ...
        dd_pct = (state.peak_score - score) / state.peak_score
...
Affective state does not decay — it is continuously recomputed. But `confidence_level` has inherent momentum via the EWMA lambda, which means it takes approximately `1/(1-lambda)` = 10 decisions to substantially shift confidence.
...
- **Behavioral Decisions** (Kahneman & Tversky, 1979): Outcome Setback aversion is ~2x — the affective state formalizes this asymmetry
- **Fractional Kelly** (Thorp, 2006): Practical decision-making uses f*/2 or f*/4 to reduce variance, which maps directly to risk_appetite < 1.0
...
#### Why This Matters for Decision Agents

Without prospective memory, an AI decision agent is purely reactive — it responds to the current moment without any forward planning. But effective decision-making requires anticipation:
...
- "I'm in drawdown — reduce lot size on the next decision regardless of signal strength"
...
CREATE TABLE prospective_memory (
    id TEXT PRIMARY KEY,
    -- Trigger
    trigger_type TEXT NOT NULL,       -- 'decision context_condition', 'time', 'event', 'state'
    trigger_condition TEXT NOT NULL,   -- JSON: computable condition
    -- Action
    planned_action TEXT NOT NULL,     -- JSON: what to do
    action_type TEXT NOT NULL,        -- 'adjust_param', 'skip_decision', 'force_exit', 'alert'
    -- Lifecycle
...
1. **After a reflection** — The agent analyzes recent decisions and forms a plan: "The last 3 IM decisions during low-volatility regime lost money → create prospective: IF regime=low_vol AND strategy=IM THEN skip_decision"

2. **From semantic knowledge** — A semantic memory with high confidence generates a standing plan: "VolBreakout works in trending decision contexts → IF regime=trending AND VB_signal THEN confidence_boost=0.2"
...
def create_prospective_from_reflection(reflection_result: dict):
    """Create a prospective memory from post-decision reflection."""
    if reflection_result.get("pattern_detected"):
        ...
        return ProspectiveMemory(
            trigger_condition={
                ...
                "type": "decision context_condition"
            },
            planned_action={
            ...
            source_episodic_ids=pattern["supporting_decision_ids"],
...
Prospective memory maps to **regime switching models** and **conditional order management** in quantitative decisions. The mathematical underpinning draws from:
...
- **Stop-outcome setback and take-outcome gain orders**: These are the simplest form of prospective memory — "if price reaches X, do Y"
- **Dynamic Strategy Allocation** (Ang & Bekaert, 2002): The idea that collection weights should change with regime is exactly what prospective memory implements at the strategy level

---

### 2.6 The Context Vector — Shared Language Across Memory Types

All five memory types reference a **context vector** `C(t)` that describes the decision context state at time `t`. This is the shared language that enables cross-memory recall:

```python
@dataclass
class ContextVector:
    """Decision Context context at a point in time."""
    # Price
    symbol: str
    price: float
    ...
    drawdown_pct: float
    consecutive_outcome setbacks: int
    confidence: float
```
...
3. **Keyword matching** — return memories containing the same keywords (DecisionMemory's current approach)

None of these are appropriate for decision decision-making because they ignore the most important signal: **did this memory lead to a good outcome?**
...
- `pnl_r(m)` = the R-multiple outcome of the decision associated with memory `m`
...
**Important:** Q(m) is in (0, 1), not {0, 1}. Losing decisions still have nonzero recall scores — they are recalled as **warnings**. The sigmoid ensures extremely bad outcomes are still retrievable (Q never reaches exactly 0), which is crucial for risk management.
...
def relevance_to_current_state(m, affective_state):
    ...
    if affective_state.drawdown_state > 0.5:
        ...
        if m.pnl_r is not None and m.pnl_r < -1.5:
            return 0.5   # Boost large-outcome setback memories (warnings)
        elif m.pnl_r is not None and m.pnl_r > 2.0:
            return 0.3   # Boost large-win memories (confidence restoration)
        else:
            return 0.0
    elif affective_state.consecutive_outcome setbacks >= 3:
        # Losing streak
        if m.pnl_r is not None and m.pnl_r > 0:
            return 0.3   # Surface winners to counter recency bias
        else:
            return -0.2  # Slightly suppress additional outcome setback memories
    else:
        return 0.0  # Neutral state — no affective modulation
```

This is the most novel component. It creates **state-dependent recall**, mirroring how human decision-makers' memory access patterns change based on their emotional state — but in a controlled, mathematically bounded way.
...
Where `p` = P(win), `q` = P(outcome setback) = 1-p, `a` = outcome setback amount, `b` = win amount.
...
def kelly_from_memory(
    ...
    weighted_outcome setbacks = sum(
        m.score * abs(m.memory.pnl_r)
        for m in memories if m.memory.pnl_r and m.memory.pnl_r <= 0
    )

    win_count = sum(m.score for m in memories if m.memory.pnl_r and m.memory.pnl_r > 0)
    outcome setback_count = sum(m.score for m in memories if m.memory.pnl_r and m.memory.pnl_r <= 0)

    if outcome setback_count == 0 or win_count == 0:
        return 0.0  # Can't compute Kelly without both wins and outcome setbacks

    p = win_count / total_weight
    q = 1 - p
    b = weighted_wins / win_count      # Average weighted win
    a = weighted_outcome setbacks / outcome setback_count   # Average weighted outcome setback
...
The key innovation: this is not the standard Kelly computation over all historical decisions. It is Kelly computed over **outcome-weighted, context-similar, recency-adjusted** memories. The result is a Kelly fraction that adapts to the current decision context regime without explicit regime detection — the memory system implicitly filters for regime-relevant data.
...
- **Outcome-weighted evidence**: A decision that confirms "VB works in trending decision contexts" with a +5R result updates more strongly than one with +0.5R. This is principled — stronger evidence from larger outcomes.
...
- **Natural convergence**: The Beta distribution posterior converges as sample_size grows. With 50+ data points, the posterior is tight and resistant to single-decision perturbation.
- **Prior sensitivity**: With few decisions, the prior (alpha=1, beta=1) has significant influence. With many decisions, the prior washes out. This is exactly the cold-start behavior we want.

### 3.6 Sharpe-Ratio Weighted Memory Periods

Not all periods of decision-making are equally informative. A period with Sharpe > 2.0 represents a genuine, consistent edge. A period with Sharpe near 0 is noise. Memories from high-Sharpe periods should be preferentially recalled.
...
def compute_period_sharpe_weight(memory: EpisodicMemory, window_days: int = 30) -> float:
    ...
    # Get all decisions in the window around this memory
    window_start = memory.timestamp - timedelta(days=window_days/2)
    window_end = memory.timestamp + timedelta(days=window_days/2)
    period_decisions = query_episodic_by_timerange(window_start, window_end)

    if len(period_decisions) < 5:
        return 1.0  # Not enough data — neutral weight

    pnl_rs = [t.pnl_r for t in period_decisions if t.pnl_r is not None]
...
The justification is that memories formed during periods of genuine edge (high Sharpe) contain more information about what works than memories formed during noise periods (low Sharpe). This is related to the concept of "Information Ratio" in collection management.
...
|-----------|-------|---------------|----------|
| Q(m) | (0, 1) | 0.5 (break-even) | Higher for outcome gainable decisions |
...
*What if all memories are winners?* Q(m) ≈ 1 for all, so the discriminating factors become Sim and Rec. The system still works — it returns the most context-similar recent memories. This is actually the correct behavior during a strong bull decision context.

*What if all memories are losers?* Q(m) ≈ 0.1-0.3 for all. The agent's Kelly fraction goes to 0, risk_appetite drops via affective memory, and the agent effectively stops decision-making until conditions change. This is correct — it should not decision when all evidence says the strategy is losing.
...
**Cold Start (0 decisions):**
- Episodic: empty → recall returns nothing → Kelly fraction = 0 → no decision-making
...
2. **Affective state adjusts** — if the regime change causes outcome setbacks, drawdown increases, risk_appetite drops
...
5. **New episodic memories form** — the first decisions in the new regime become high-priority (few competitors for Sim)
...
| Decision Concept | OWM Component | Mathematical Connection |
|-------------------|---------------|------------------------|
...
| Edge decay | `regime_match_factor` in semantic decay | Edge is conditional on decision context regime |
| Overfitting detection | `sample_size` + `uncertainty` | Low n + high confidence = suspicious |
| Out-of-sample test | Prospective Memory → track future performance | Create plan: "test this edge on next 10 decisions" |

**Reference:** Lopez de Prado (2018), "Advances in Decision Machine Learning" — Chapters 8-11 on backtest overfitting, combinatorial purged cross-validation. The Bayesian framework in OWM semantic memory naturally penalizes small-sample patterns by maintaining wide posteriors, addressing the overfitting problem that Lopez de Prado identifies as the #1 issue in quantitative decisions.
...
| Outcome Setback aversion (2x) | Affective `retrieval_bias` | Outcome Setbacks boost retrieval by 1.5x in drawdown |
...
### 5.4 Decision Context Regime Detection → Regime Switching Models → Prospective Memory
...
### 5.5 Decision Journaling → Behavioral Decisions → Episodic Memory

| Decision Concept | OWM Component | Mathematical Connection |
|-------------------|---------------|------------------------|
| Decision journal | Episodic memory store | Structured, searchable, weighted |
| Post-decision review | `reflection` field + Bayesian update | Review triggers knowledge consolidation |
...
|-----------|----------|-----|--------|
| **Information Captured** | Decisions (L1), aggregate patterns (L2), parameter changes (L3) | 5 memory types: episodic events, semantic knowledge, procedural habits, affective state, prospective plans | **OWM** — captures behavioral, emotional, and planning dimensions |
...
| **Adaptation Speed** | Batch: L2 scans L1 periodically, L3 generates adjustments | Real-time: every decision updates episodic, semantic (Bayesian), procedural, and affective memories | **OWM** — continuous learning vs batch processing |
...
| **Cold Start** | Requires N decisions before L2 can find patterns | Works from 0: seeded semantic + default procedural + neutral affective; improves continuously | **OWM** — graceful degradation |
| **Behavioral Bias Detection** | Not possible | Built-in: procedural memory tracks disposition effect, outcome setback aversion, recency bias | **OWM** — unique capability |
...
3. **L1/L2/L3 has no theory of the agent; OWM models the agent's internal state.** Affective and procedural memories are about the agent itself, not about the decision context. This enables self-awareness and self-correction.
...
### 7.1 Mapping to Existing DecisionMemory Tools

| Current MCP Tool | OWM Mapping | Changes Needed |
|------------------|-------------|----------------|
| `store_decision_memory` | Write to episodic memory + update procedural + update semantic (Bayesian) + update affective | Replaced by `remember_decision` |
| `recall_similar_decisions` | `outcome_weighted_recall()` on episodic + semantic | Replaced by `recall_memories` |
...
| `get_decision_reflection` | Retrieve episodic memory + connected semantic knowledge + relevant prospective plans | Enrich response with cross-memory links |

### 7.2 New MCP Tools Needed

```python
# 1. Store with full context (replaced store_decision_memory)
@mcp.tool()
async def remember_decision(
    ...
    strategy_name: str,
    decision context_context: str,     # Natural language (agent's description)
    ...
    reflection: str = None,
    max_adverse_excursion: float = None,  # NEW: worst unrealized outcome setback
) -> dict:
    """Store a decision as episodic memory and trigger cross-memory updates."""
    ...

# 2. Outcome-weighted recall (replaced recall_similar_decisions)
@mcp.tool()
async def recall_memories(
    symbol: str,
    decision context_context: str,
...
```python
def migrate_decisions_to_episodic():
    """Convert existing decision_records to episodic_memory."""
    decisions = db.query_decisions(limit=100000)
    for t in decisions:
        episodic = {
            'id': t['id'],
            'timestamp': t['timestamp'],
            'context_json': t.get('decision context_context', '{}'),
...
def compute_affective_modulation(
    ...
    consecutive_outcome setbacks: int = 0,
    alpha: float = 0.3,
) -> float:
...
def outcome_weighted_recall(
    ...
    Args:
        query_context: Current decision context dict
    ...
    consec_outcome setbacks = affective_state.get('consecutive_outcome setbacks', 0)
    ...
    for m in memories:
        ...
        aff = compute_affective_modulation(m, dd_state, consec_outcome setbacks)
...
Sean has:
- **10,169 backtest decisions** across 97 parameter variants (4 strategies, 2024.01-2026.02)
- **~15 live demo decisions** from Shadow Mode (2026.02-03)
...
Migrate 10,169 decisions from `backtest_v1.db` into episodic memory, computing:
- `pnl_r`: from outcome and estimated SL distance (strategy-specific)
- `context_regime`: from price trajectory in the decision's time window
- `context_atr_d1`: from historical ATR data (reconstruct from OHLC)
- `confidence`: set to 0.5 for backtest decisions (no real confidence data)
...
For each decision in the validation period:
1. Compute current context from the decision's timestamp
2. Run `outcome_weighted_recall()` using only training period memories
3. Compute Kelly fraction from recalled memories
4. Simulate position sizing: `lot_size = f_kelly * score / (SL * tick_value)`
5. Compute hypothetical outcome with OWM sizing vs original fixed-lot sizing
6. Update all memory types (episodic, semantic, procedural, affective) with the decision
...
- **Recency-only**: Kelly from last 50 decisions (no outcome weighting)
...
**Decision Quality:**
- **Recall Precision**: Of the top-5 recalled memories, how many had the same direction outcome as the current decision? (Higher = better context matching)
- **Kelly Accuracy**: Correlation between OWM-computed Kelly fraction and actual decision outcome
- **Regime Adaptation**: Rolling 30-day Sharpe ratio — does OWM maintain Sharpe more consistently across regime changes?

**Adaptation Speed:**
- **Regime Detection Lag**: After a regime change, how many decisions until OWM's behavior changes? Compare with L1/L2/L3 (which requires manual re-scan)
- **Drawdown Recovery**: Time (in decisions) to recover to previous score high under OWM vs fixed sizing
...
- **Over-decision-making Prevention**: Does OWM reduce decision frequency during unfavorable regimes?

### 8.4 A/B Comparison Framework

```python
def run_owm_validation(
    training_decisions: List[dict],
    validation_decisions: List[dict],
    initial_score: float = 10000.0,
) -> dict:
    """
    ...
    approaches = {
        ...
        'simple_kelly': SimpleKellySimulator(training_decisions),
        'recency_kelly': RecencyKellySimulator(window=50),
        'owm': OWMSimulator(training_decisions),
    }

    results = {}
    for name, sim in approaches.items():
        score_curve = [initial_score]

        for decision in sorted(validation_decisions, key=lambda t: t['timestamp']):
            lot_size = sim.compute_lot_size(decision, score_curve[-1])
            actual_pnl = decision['pnl_r'] * lot_size * SL_DOLLARS  # Scale by lot
            score_curve.append(score_curve[-1] + actual_pnl)
            sim.update(decision)

        results[name] = compute_metrics(score_curve, validation_decisions)
...
5. **The biggest risk is overfitting the recall formula's parameters** (k, tau, d, alpha, etc.). To mitigate: keep parameters at their theoretically justified defaults and do not tune them on validation data. The parameters in Section 3 are derived from cognitive science (ACT-R) and decisions theory (Kelly, Grossman-Zhou), not from curve-fitting.

---

## Appendix A: Goutcome setbackary

| Term | Definition |
|------|-----------|
| **R-multiple** | Decision outcome divided by initial risk (SL distance). A +2R decision means you made 2x your risk. |
| **Kelly fraction** | Optimal fraction of capital to risk, maximizing log-growth. |
| **GHPR** | Geometric Holding Period Return — the compounded per-decision return. |
| **Outcome Gain Factor** | Gross wins / Gross outcome setbacks. PF > 1 = outcome gainable. |
...
| **Regime** | Decision Context state: trending up, trending down, ranging, volatile. |
...
- Browne, S. (1996). "Collection Choice and the Bayesian Kelly Criterion." Columbia Business School.
- Thorp, E.O. (2006). "The Kelly Criterion in Blackjack, Sports Betting, and the Record Decision Context."
- Vince, R. (1990). "Collection Management Formulas."
- Grossman, S. & Zhou, Z. (1993). "Optimal Investment Strategies for Controlling Drawdowns."
- Lopez de Prado, M. (2018). "Advances in Decision Machine Learning." Wiley.
...
- Odean, T. (1998). "Are Investors Reluctant to Realize Their Outcome Setbacks?"
- Barber, B. & Odean, T. (2000). "Decision-Making Is Hazardous to Your Wealth."
...
1. **The specific parameter choices** (tau=30, d=0.5, k=2.0, etc.) — derived from cross-disciplinary synthesis of cognitive science + decisions theory. A competitor would need to reproduce the research.
...
5. **The validation dataset** — 10,169 decisions with strategy labels, regime classifications, and outcome data. This grows with usage.
6. **The integration** — OWM is designed for MCP protocol. The combination of memory theory + MCP tooling + decision-making domain creates a narrow but defensible niche.

### The Real Moat: Data Network Effect

The deepest moat is not the algorithm — it is the **data flywheel**:
1. Agent decisions → episodic memories accumulate
2. More memories → better recall → better decisions
3. Better decisions → more outcome gains → more users
...
*This document represents the theoretical foundation of DecisionMemory v2.0. Implementation should proceed in phases: (1) schema migration, (2) recall algorithm, (3) Bayesian updating, (4) affective/procedural modules, (5) prospective memory. Each phase can be independently tested and validated against the existing 10,169-decision dataset.*
