...
### Changes to `src/decisionmemory/ssrt/core.py`
...
def test_shift_null_preserves_count():
    """shift_null should NOT reset decision count."""
...
def test_shift_null_improves_regime_detection():
    ...
    for seed in range(n_sims):
        decisions = DecaySimulator.regime_specific_decay(
            n_decisions=200, decay_at=50, seed=seed
        )
        
        # Run with shift_null (Option B)
        msprt_shift = MixtureSPRT(alpha=0.05, tau=1.0, sigma=1.5, null_mean=0.5, burn_in=20)
        regime_null_shift = RegimeAwareNull(min_decisions_per_regime=10)
        prev_regime = None
        shift_detected = False
        for decision in decisions:
            regime_null_shift.update(decision)
            rn_mean, rn_sigma = regime_null_shift.get_null(decision.regime)
            ...
            v = msprt_shift.update(decision.pnl_r)
        ...
        regime_null_reset = RegimeAwareNull(min_decisions_per_regime=10)
        prev_regime = None
        reset_detected = False
        for decision in decisions:
            regime_null_reset.update(decision)
            rn_mean, rn_sigma = regime_null_reset.get_null(decision.regime)
            if decision.regime != prev_regime and prev_regime is not None:
                msprt_reset.reset(null_mean=rn_mean, sigma=rn_sigma)
            prev_regime = decision.regime
            v = msprt_reset.update(decision.pnl_r)
            if v.decision == "RETIRE":
                reset_detected = True
                break
        if reset_detected:
...
3. What tau value gives best power/Type I decisionoff?
