...
Sequential Strategy Retirement Testing (SSRT) — a statistically rigorous framework for deciding when to retire a decision-making strategy. Uses mSPRT (Mixture Sequential Probability Ratio Test) with regime-aware null hypotheses.

**Paper**: "When to Pull the Plug: Sequential Hypothesis Testing for Decision-Making Strategy Retirement under Regime Uncertainty"

**Key novelty**: The null hypothesis is not fixed "alpha > 0" but regime-dependent. A strategy's expected performance depends on decision context regime (trending_up, trending_down, ranging, volatile). When regime shifts, the null shifts too.

## Existing Code to Reuse

- `src/decisionmemory/owm/drift.py` → `cusum_drift_detect()` — use as baseline comparison
- `src/decisionmemory/owm/changepoint.py` → `BayesianChangepoint` — reference for state persistence pattern
- `src/decisionmemory/evolution/statistical_gates.py` → `benjamini_hochberg()` — reuse for multi-strategy (v2)
- `src/decisionmemory/evolution/backtester.py` → `FitnessMetrics`, `_compute_max_drawdown()` — reuse
- `src/decisionmemory/data/context_builder.py` → regime detection — reuse
- `src/decisionmemory/owm/context.py` → `ContextVector` regime field — reuse
...
**`src/decisionmemory/ssrt/__init__.py`**:
```python
...
**`src/decisionmemory/ssrt/models.py`**:

```python
@dataclass
class DecisionResult:
    """Single decision outcome for sequential testing."""
...
class SSRTVerdict:
    ...
    decisions_analyzed: int    # cumulative decision count
...
class RegimeBaseline:
    ...
    decision_count: int        # how many decisions formed this baseline
...
### File: `src/decisionmemory/ssrt/core.py`

**Algorithm**: Mixture Sequential Probability Ratio Test (Johari et al. 2017)

The key idea: instead of testing against a point alternative (H1: mu = mu_1), we test against a MIXTURE of alternatives. This gives always-valid p-values — you can "peek" after every decision without inflating Type I error.

**Math**:
- Observations: x_1, x_2, ..., x_n (decision R-multiples, shifted by null mean)
...
class MixtureSPRT:
    ...
    def update(self, observation: float) -> SSRTVerdict:
        """Process one decision return, update statistics, return verdict."""
...
- sigma should be estimated from the first 20-30 decisions (burn-in period)
- During burn-in, always return INCONCLUSIVE
- tau controls sensitivity: small tau = detect small effects (more decisions needed), large tau = detect large effects (fewer decisions)
...
### File: `src/decisionmemory/ssrt/regime.py`
...
class RegimeAwareNull:
    ...
    def __init__(self, min_decisions_per_regime: int = 10, default_null: float = 0.0):
        """
        Args:
            min_decisions_per_regime: minimum decisions before using regime-specific null
            default_null: fallback null mean when insufficient regime data
        """

    def update(self, decision: DecisionResult):
        """Update regime baseline with new decision."""
...
2. When a regime has >= min_decisions_per_regime, use its mean as null_mean
...
for decision in decision_stream:
    regime_null.update(decision)
    null_mean, sigma = regime_null.get_null(decision.regime)
    msprt.reset(null_mean=null_mean, sigma=sigma)  # only if regime changed
    observation = decision.pnl_r - null_mean  # shift by null
...
1. **test_always_positive_returns_continue**: Feed 50 decisions with pnl_r ~ N(1.0, 1.0). Should get CONTINUE or INCONCLUSIVE, never RETIRE.

2. **test_always_negative_returns_retire**: Feed 50 decisions with pnl_r ~ N(-1.0, 1.0). Should eventually get RETIRE.

3. **test_burn_in_returns_inconclusive**: First 20 decisions should always return INCONCLUSIVE regardless of values.
...
1. **test_update_builds_baselines**: Add decisions in different regimes, verify baselines reflect correct means.

2. **test_insufficient_data_uses_default**: With <10 decisions in a regime, get_null() returns default (0.0).

3. **test_sufficient_data_uses_regime_mean**: With >=10 decisions, get_null() returns that regime's mean.

4. **test_multi_regime_independence**: Trending_up decisions don't affect ranging baselines.
...
### File: `src/decisionmemory/ssrt/simulator.py`

**Purpose**: Generate synthetic decision sequences with known decay patterns for controlled experiments.
...
class DecaySimulator:
    ...
    @staticmethod
    def no_decay(n_decisions: int, mean: float = 0.5, std: float = 1.5,
                 regime: str = "trending_up", seed: int | None = None) -> list[DecisionResult]:
        """Control: strategy with stable edge."""

    @staticmethod
    def sudden_death(n_decisions: int, decay_at: int, pre_mean: float = 0.5,
                     post_mean: float = -0.3, std: float = 1.5,
                     regime: str = "trending_up", seed: int | None = None) -> list[DecisionResult]:
        """Alpha drops to post_mean at decision #decay_at."""

    @staticmethod
    def linear_decay(n_decisions: int, decay_start: int, decay_end: int,
                     pre_mean: float = 0.5, post_mean: float = -0.3,
                     std: float = 1.5, regime: str = "trending_up",
                     seed: int | None = None) -> list[DecisionResult]:
        """Alpha decays linearly from pre_mean to post_mean between decay_start and decay_end."""

    @staticmethod
    def regime_specific_decay(n_decisions: int, decay_at: int,
        ...
        regime_schedule: list of (decision_index, regime) to control regime sequence.
        """
```

Each method returns `list[DecisionResult]` with realistic timestamps (1 decision per hour, UTC).

### File: `src/decisionmemory/ssrt/baselines.py`
...
class MaxDDBaseline:
    ...
    def __init__(self, threshold_pct: float = 20.0):
        ...
    def update(self, decision: DecisionResult) -> str:
        """Returns "CONTINUE" or "RETIRE"."""
    def reset(self): ...

class RollingSharpeBaseline:
    """Stop when N-decision rolling Sharpe stays below 0."""
...
All baselines share the same interface: `update(decision) -> str`, `reset()`.
...
class ExperimentResult:
    ...
    detection_decision: int   # at which decision? (None if not detected)
    true_decay_decision: int  # ground truth (None for no_decay)
    detection_delay: int   # detection_decision - true_decay_decision (None if N/A)
