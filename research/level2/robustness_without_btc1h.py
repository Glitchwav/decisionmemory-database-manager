...
def main():
    ...
    # --- vs BaseAgent ---
    diffs = [r["agents"]["base"]["score_dd"] - r["agents"]["cusum"]["score_dd"] for r in filtered]
    n = len(diffs)
    m = mean(diffs)
    s = stdev(diffs)
    ...
    # --- vs SimpleWR ---
    diffs_swr = [r["agents"]["simple_wr"]["score_dd"] - r["agents"]["cusum"]["score_dd"] for r in filtered]
    ...
    for key, group in sorted(groups.items()):
        seg_diffs = [r["agents"]["base"]["score_dd"] - r["agents"]["cusum"]["score_dd"] for r in group]
        seg_n = len(seg_diffs)
