...
def load_cached_results() -> Dict[str, Dict]:
    ...
    for r in data:
        ...
        cache[key] = {
            "base_dd": r["agents"]["base"]["score_dd"],
            "cusum_dd": r["agents"]["cusum"]["score_dd"],
            "is_decisions": r["is_decisions"],
        }
    return cache
...
async def main():
    ...
    for symbol in SYMBOLS:
        for tf_str, tf_enum in TIMEFRAMES.items():
            ...
            # Run IS qualification to get IS decisions for warm-start
            ...
            for j, (strategy, is_result, cached) in enumerate(qualified):
                ...
                maxdd.warm_start(is_decisions)
                r_maxdd = Simulator(maxdd, oos_series, tf_str).run()

                base_dds.append(cached["base_dd"])
                cusum_dds.append(cached["cusum_dd"])
                maxdd_dds.append(r_maxdd.score_max_dd)

                if (j + 1) % 10 == 0:
                    print(f"    [{j+1}/{len(qualified)}] maxdd_dd={r_maxdd.score_max_dd:.0f}", flush=True)
