"""Tests for scripts/research/export_backtest_decisions.py."""
...
from decisionmemory.evolution.backtester import Decision
...
from export_backtest_decisions import (
...
class TestFastBacktestWithDecisions:
    def test_empty_bars(self):
        fitness, decisions = fast_backtest_with_decisions([], [], [], build_strategy_e())
    ...
    def test_returns_tuple(self):
        ...
        contexts = [None] * 30
        atrs = [None] * 30
        result = fast_backtest_with_decisions(
            series.bars, contexts, atrs, build_strategy_e()
        )
        assert isinstance(result, tuple)
        assert len(result) == 2
        fitness, decisions = result
        assert isinstance(fitness, FitnessMetrics)
...
class TestDecisionToDict:
    def test_output_fields(self):
        ...
        d = decision_to_dict(decision, bars, contexts, atrs, strategy_id="STRAT-E", symbol="BTCUSDT")
        ...
        assert d["decision_type"] == "backtest"
    ...
    def test_none_context(self):
        ...
        d = decision_to_dict(decision, bars, contexts, atrs)
