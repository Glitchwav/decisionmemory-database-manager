"""Tests for decisionmemory.auth — bearer-token middleware + tenant routing.
...
from decisionmemory.auth import (
    AuthContext,
...
def test_middleware_blocks_request_without_auth(monkeypatch):
    monkeypatch.setenv("DECISIONMEMORY_API_KEYS", "secret:acme")
    client = TestClient(_app_with_middleware())
    r = client.get("/protected")
...
def test_middleware_multiple_keys_route_to_different_tenants(monkeypatch):
    monkeypatch.setenv("DECISIONMEMORY_API_KEYS", "k1:t1,k2:t2")
    client = TestClient(_app_with_middleware())
...
def test_decision_records_has_tenant_id_column(tmp_path):
