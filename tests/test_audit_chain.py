"""Tests for decisionmemory.audit — chain + Merkle root.
...
Covers:
  ...
  - End-to-end backfill from a populated decision_records table
"""
...
from decisionmemory.audit.chain import (
    GENESIS_HASH,
    ChainBuilder,
    chained_hash,
)
from decisionmemory.audit.merkle import merkle_root
...
# ---------------------------------------------------------------------------
# Insert hook — Database.insert_decision auto-appends to chain
# ---------------------------------------------------------------------------

def _sample_decision(decision_id: str, ts: str, direction: str = "long") -> dict:
    return {
        "id": decision_id,
        "timestamp": ts,
        "symbol": "XAUUSD",
...
def test_insert_decision_appends_to_chain(db):
    db.insert_decision(_sample_decision("T-1", "2026-05-14T01:00:00+00:00"))
    db.insert_decision(_sample_decision("T-2", "2026-05-14T02:00:00+00:00"))
...
def test_insert_decision_duplicate_id_skips_chain(db):
    db.insert_decision(_sample_decision("T-1", "2026-05-14T01:00:00+00:00"))
...
def test_backfill_rebuilds_chain_from_decision_records(db):
    # Seed three decisions; the insert_decision hook will already chain them.
    db.insert_decision(_sample_decision("T-1", "2026-05-13T10:00:00+00:00"))
    db.insert_decision(_sample_decision("T-2", "2026-05-13T11:00:00+00:00"))
    db.insert_decision(_sample_decision("T-3", "2026-05-14T09:00:00+00:00"))
    ...
    def _decision_iter():
        ...
        with db.get_connection() as conn:
            rows = conn.execute(
                ...
                "reasoning, market_context FROM decision_records "
        ...
        for row in rows:
            ...
            yield (
                row["id"],
                DecisionMakingDecisionRecord.compute_hash(
                    decision_id=row["id"],
    ...
    cb = _builder(db)
    summary = cb.backfill(_decision_iter())
    cb.conn.commit()
