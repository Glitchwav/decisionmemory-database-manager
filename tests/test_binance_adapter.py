...
from decisionmemory.data.binance import (
    BASE_URL,
    KLINES_ENDPOINT,
    BinanceDataSource,
    TIMEFRAME_MAP,
...
class TestAvailableSymbols:
    ...
    async def test_available_symbols(self, source, mock_client):
        """Returns list of decision_making symbols."""
        mock_resp = MagicMock()
        mock_resp.status_code = 200
        mock_resp.json.return_value = {
            "symbols": [
                {"symbol": "BTCUSDT", "status": "DECISION_MAKING"},
                {"symbol": "ETHUSDT", "status": "DECISION_MAKING"},
                {"symbol": "OLDCOIN", "status": "BREAK"},  # not decision_making
            ]
        }
        mock_client.get = AsyncMock(return_value=mock_resp)
