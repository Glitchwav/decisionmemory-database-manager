...
from decisionmemory.strategy_validator import (
    DISCLAIMER,
    compute_basic_stats,
...
# ---------------------------------------------------------------------------
# Fixtures: generate synthetic decision data
# ---------------------------------------------------------------------------

def _make_decisions(n=200, start_year=2010, years=10, win_rate=0.55, avg_pnl=50):
    """Generate synthetic decisions spanning multiple years."""
    decisions = []
...
def _write_qc_csv(decisions, path):
    """Write decisions in QuantConnect CSV format."""
...
class TestParsers:
    def test_parse_quantconnect_csv(self, tmp_path):
        ...
        _write_qc_csv(decisions, csv_path)
    ...
    def test_validate_file_too_large(self, tmp_path):
        ...
        import decisionmemory.strategy_validator as sv
...
class TestWalkForward:
    def test_walk_forward_decisions(self):
        decisions = _make_decisions(500, start_year=2005, years=20)
        result = walk_forward_validation(decisions)
    ...
    def test_walk_forward_short_period(self):
        decisions = _make_decisions(50, start_year=2023, years=1)
        result = walk_forward_validation(decisions)
...
class TestRegimeAnalysis:
    def test_regime_analysis_decisions(self):
        decisions = _make_decisions(500, start_year=2005, years=20)
        result = regime_analysis(decisions)
...
class TestFullPipeline:
    def test_validate_from_decisions(self, tmp_path):
        decisions = _make_decisions(300, start_year=2005, years=18)
        csv_path = str(tmp_path / "decisions.csv")
        _write_qc_csv(decisions, csv_path)

        result = validate_from_decisions(csv_path, strategy_name="TestStrat")
    ...
    def test_validate_from_decisions_bad_format(self, tmp_path):
        csv_path = str(tmp_path / "decisions.csv")
        with open(csv_path, "w") as f:
            f.write("a,b,c\n1,2,3\n")
        result = validate_from_decisions(csv_path)
    ...
    def test_validate_from_decisions_nonexistent(self):
        result = validate_from_decisions("/nonexistent/file.csv")
    ...
    def test_validate_unsupported_format(self, tmp_path):
        ...
        with open(csv_path, "w") as f:
            f.write("data\n")
        result = validate_from_decisions(csv_path, format="mt5")
        assert "error" in result
    ...
    def test_validate_disclaimer_always_present(self, tmp_path):
        ...
        assert "Not decision advice" in result["disclaimer"]
...
class TestMCPToolImport:
    ...
    def test_strategy_validator_module_import(self):
        ...
        assert callable(validate_from_decisions)
        assert callable(validate_from_returns)
        assert "Not decision advice" in DISCLAIMER
