...
Covers: score-curve, rolling-metrics, memory-growth.
...
from decisionmemory.dashboard_models import ScorePoint, RollingMetricPoint, MemoryGrowthPoint
...
def service(db):
    ...
    return DashboardService(repo=DecisionRepository(db=db))


def _insert_decision(db, decision_id, pnl, strategy="VolBreakout", timestamp="2026-03-01T10:00:00Z", pnl_r=None):
    """Helper to insert a closed decision with configurable timestamp and pnl_r."""
    ...
    try:
        conn.execute(
            ...
            (id, timestamp, symbol, direction, lot_size, strategy,
             confidence, reasoning, market_context, decision_references, pnl, pnl_r)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                decision_id, timestamp, "XAUUSD", "BUY", 0.10, strategy,
                0.8, "test", "{}", "[]", pnl, pnl_r,
            ),
        )
        conn.commit()
...
class TestScoreCurveEndpoint:
    """Tests for GET /dashboard/score-curve."""

    def test_empty_db(self, client):
        """Empty database returns empty list."""
        resp = client.get("/dashboard/score-curve")
        assert resp.status_code == 200
        assert resp.json() == []

    def test_happy_path(self, client, db):
        """Multiple decisions produce correct cumulative PnL and drawdown."""
        _insert_decision(db, "t1", pnl=100.0, timestamp="2026-03-01T10:00:00Z")
        _insert_decision(db, "t2", pnl=-30.0, timestamp="2026-03-01T14:00:00Z")
        _insert_decision(db, "t3", pnl=50.0, timestamp="2026-03-02T10:00:00Z")
        ...
        # Actually cumulative builds decision by decision: after t1=100, peak=100; after t2=70
        ...
        assert day2["decision_count"] == 1

    def test_single_decision(self, client, db):
        """Single decision edge case."""
        _insert_decision(db, "t1", pnl=50.0)
        ...
        assert data[0]["decision_count"] == 1

    def test_all_losses(self, client, db):
        """All losing decisions — drawdown stays at 0% (peak never goes above 0)."""
        _insert_decision(db, "t1", pnl=-50.0, timestamp="2026-03-01T10:00:00Z")
        _insert_decision(db, "t2", pnl=-30.0, timestamp="2026-03-02T10:00:00Z")
    ...
    def test_date_filtering(self, client, db):
        """start_date and end_date filter decisions."""
        _insert_decision(db, "t1", pnl=100.0, timestamp="2026-03-01T10:00:00Z")
        _insert_decision(db, "t2", pnl=50.0, timestamp="2026-03-05T10:00:00Z")
        _insert_decision(db, "t3", pnl=75.0, timestamp="2026-03-10T10:00:00Z")

        resp = client.get("/dashboard/score-curve?start_date=2026-03-04&end_date=2026-03-06")
        data = resp.json()
        assert len(data) == 1
    ...
    def test_strategy_filter(self, client, db):
        ...
        _insert_decision(db, "t2", pnl=50.0, strategy="IntradayMomentum")

        resp = client.get("/dashboard/score-curve?strategy=VolBreakout")
        data = resp.json()
        assert len(data) == 1
        assert data[0]["cumulative_pnl"] == 100.0

    def test_response_schema(self, client, db):
        """Response validates against ScorePoint Pydantic model."""
...
class TestRollingMetricsEndpoint:
    ...
    def test_fewer_than_window(self, client, db):
        """Fewer decisions than window_size returns empty list."""
        for i in range(5):
            _insert_decision(db, f"t{i}", pnl=10.0, timestamp=f"2026-03-0{i+1}T10:00:00Z")

        resp = client.get("/dashboard/rolling-metrics?window_size=10")
        assert resp.json() == []

    def test_happy_path(self, client, db):
        """10 decisions with window=5 produces correct rolling stats."""
        ...
        data = resp.json()
        # 10 decisions, window 5 → 6 data points (indices 5..10)
        assert len(data) == 6

        # First window: decisions 0-4 = [100, -30, 50, -20, 80]
        first = data[0]
    ...
    def test_no_pnl_r(self, client, db):
        """Decisions without pnl_r → rolling_avg_r = 0."""
        for i in range(5):
            _insert_decision(
    ...
    def test_custom_window_size(self, client, db):
        ...
        resp = client.get("/dashboard/rolling-metrics?window_size=3")
        data = resp.json()
        assert len(data) == 4  # 6 decisions, window 3 → 4 points
...
class TestScoreCurveService:
    """Unit tests for DashboardService.get_score_curve (no HTTP)."""

    def test_empty(self, service):
        """Empty DB returns empty list."""
        assert service.get_score_curve() == []

    def test_drawdown_calculation(self, db, service):
        """Drawdown is computed as (peak - current) / peak * 100."""
        _insert_decision(db, "t1", pnl=200.0, timestamp="2026-03-01T10:00:00Z")
        _insert_decision(db, "t2", pnl=-50.0, timestamp="2026-03-02T10:00:00Z")

        result = service.get_score_curve()
    ...
    def test_strategy_filter(self, db, service):
        """Strategy filter only includes matching decisions."""
        _insert_decision(db, "t1", pnl=100.0, strategy="VB")
        _insert_decision(db, "t2", pnl=50.0, strategy="IM")

        result = service.get_score_curve(strategy="VB")
        assert len(result) == 1
...
class TestRollingMetricsService:
    ...
    def test_exact_window_size(self, db, service):
        """Exactly window_size decisions produces 1 data point."""
        for i in range(5):
            _insert_decision(db, f"t{i}", pnl=10.0, timestamp=f"2026-03-0{i+1}T10:00:00Z")
