...
SAMPLE_EXECUTED = {
    ...
    "filters_json": {"filters": ["spread_ok", "risk_ok", "collection_ok"]},
...
class TestDecisionLogReaderMatch:
    ...
    def test_match_by_exec_ticket(self, tmp_dir):
        ...
        result = reader.find_decision_for_decision(
            strategy="VolBreakout",
...
class TestDecisionLogReaderEdgeCases:
    ...
    def test_empty_dir(self, tmp_dir):
        reader = DecisionLogReader(tmp_dir)
        result = reader.find_decision_for_decision("VolBreakout", 1774600505)
        assert result is None
    ...
    def test_unknown_strategy_returns_none(self, tmp_dir):
        reader = DecisionLogReader(tmp_dir)
        result = reader.find_decision_for_decision("UnknownStrategy", 1774600505)
        assert result is None
