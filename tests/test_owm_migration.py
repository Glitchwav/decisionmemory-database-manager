"""
Tests for OWM migration: decision_records → episodic, patterns → semantic, affective init.
"""
...
from decisionmemory.owm.migration import (
...
def _insert_decision(db, decision_id="T-001", strategy="VolBreakout", direction="long",
                  market_context=None, pnl_r=1.5, confidence=0.7, pnl=100.0):
    """Helper to insert a decision_record directly via SQL."""
    ...
    try:
        ...
        """, (
            decision_id, "2026-03-02T10:00:00", "XAUUSD", direction, 0.03,
            strategy, confidence, "Test decision", json.dumps(market_context),
...
class TestMigrateToEpisodic:
    ...
    def test_basic_migration(self, db):
        _insert_decision(db, "T-001")
        count = migrate_decisions_to_episodic(db)
        assert count == 1
    ...
    def test_context_fields_parsed(self, db):
        ...
        _insert_decision(db, "T-002", market_context=ctx)
        migrate_decisions_to_episodic(db)
    ...
    def test_missing_context_fields_are_none(self, db):
        ...
        _insert_decision(db, "T-003", market_context=ctx)
        migrate_decisions_to_episodic(db)
    ...
    def test_confidence_default(self, db):
        """Decision with None confidence → episodic gets 0.5."""
        conn = db._get_connection()
        try:
            conn.execute("""
                INSERT INTO decision_records (
    ...
    def test_multiple_decisions(self, db):
        for i in range(5):
            _insert_decision(db, f"T-{i:03d}")
        count = migrate_decisions_to_episodic(db)
        assert count == 5
    ...
    def test_duplicate_no_error(self, db):
        ...
        _insert_decision(db, "T-DUP")
        count1 = migrate_decisions_to_episodic(db)
        count2 = migrate_decisions_to_episodic(db)
        assert count1 == 1
    ...
    def test_empty_table(self, db):
        """Migration on empty decision_records should return 0 and not crash."""
        count = migrate_decisions_to_episodic(db)
        assert count == 0

    def test_hold_duration_mapped(self, db):
        _insert_decision(db, "T-HD")
        migrate_decisions_to_episodic(db)
...
class TestInitializeAffective:
    ...
    def test_basic_init(self, db):
        result = initialize_affective(db, score=15000.0)
        ...
        try:
            ...
            assert row is not None
            assert row["peak_score"] == 15000.0
            assert row["current_score"] == 15000.0
    ...
    def test_default_score(self, db):
        initialize_affective(db)

        conn = db._get_connection()
        try:
            row = conn.execute(
                "SELECT peak_score, current_score FROM affective_state WHERE id = 'current'"
            ).fetchone()
            assert row["peak_score"] == 10000.0
            assert row["current_score"] == 10000.0
        finally:
            conn.close()

    def test_duplicate_no_error(self, db):
        initialize_affective(db, score=10000.0)
        initialize_affective(db, score=20000.0)  # should not raise, should not overwrite

        conn = db._get_connection()
        try:
            row = conn.execute(
                "SELECT peak_score FROM affective_state WHERE id = 'current'"
            ).fetchone()
