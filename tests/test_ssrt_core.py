...
from decisionmemory.ssrt.models import SSRTVerdict
...
class TestMixtureSPRT:
    ...
    def test_burn_in_returns_inconclusive(self):
        """First 20 decisions should always return INCONCLUSIVE."""
        m = MixtureSPRT(burn_in=20)
        for i in range(19):
            v = m.update(-5.0)  # extreme negative
            assert v.decision == "INCONCLUSIVE", f"Decision {i+1} should be INCONCLUSIVE"

    def test_matching_null_returns_continue(self):
        """Decisions matching null_mean should not trigger RETIRE."""
    ...
    def test_always_negative_returns_retire(self):
        """50 very negative decisions should eventually trigger RETIRE."""
        rng = np.random.default_rng(42)
        ...
        for _ in range(50):
            ...
            if v.decision == "RETIRE":
                retired = True
                break
        assert retired, "Should retire with consistently negative decisions"
    ...
    def test_shift_null_improves_regime_detection(self):
        ...
        Uses frequent regime switches (every 10 decisions) to amplify
