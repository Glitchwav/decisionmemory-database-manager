...
from decisionmemory.evolution.regime_detector import (
    DecayAssessment,
    RegimeDecayDetector,
...
def make_baseline(win_rate=0.6, decision_count=100, max_drawdown_pct=15.0):
    return FitnessMetrics(
        ...
        profit_factor=1.8,
        total_pnl=1000.0,
        avg_decision_pnl=10.0,
    )


def make_decision(is_win: bool, pnl: float, atr_ratio=0.5, trend=0.1, atr_pct=50.0):
...
# --- Test: insufficient decisions ---


class TestInsufficientDecisions:
    def test_below_min_decisions_no_assessment(self):
        detector = RegimeDecayDetector(baseline=make_baseline())
        for _ in range(19):
            detector.add_decision(make_decision(True, 10.0))
        result = detector.assess()
        assert not result.decay_confirmed
        assert result.total_decisions == 19
    ...
    def test_exactly_min_decisions_runs_assessment(self):
        detector = RegimeDecayDetector(baseline=make_baseline())
        for _ in range(20):
            detector.add_decision(make_decision(True, 10.0))
        result = detector.assess()
...
class TestStableScenario:
    def test_stable_no_decay(self):
        ...
        for i in range(30):
            is_win = i % 5 < 3  # 3/5 = 60%
            detector.add_decision(make_decision(is_win, 10.0 if is_win else -8.0))
    ...
    def test_stable_with_features_no_ood(self):
        ...
        for i in range(25):
            is_win = i % 5 < 3
            detector.add_decision(make_decision(
...
class TestDecayScenario:
    def test_win_rate_drops_to_30pct(self):
        ...
        detector = RegimeDecayDetector(baseline=make_baseline(win_rate=0.6, decision_count=50))
        detector.fit_market_baseline(make_training_features())
    ...
    def test_only_win_rate_decay_no_confirm_with_one_signal(self):
        ...
        detector = RegimeDecayDetector(
            baseline=make_baseline(win_rate=0.6, decision_count=20, max_drawdown_pct=99.0),
        )
        ...
        for i in range(25):
            ...
            detector.add_decision(make_decision(is_win, 5.0 if is_win else -1.0))

        result = detector.assess()
        wr_signal = next(s for s in result.signals if s.name == "win_rate_decay")
        # Win rate decay should fire (20% << 60% baseline with only 20 prior decisions)
...
class TestOODScenario:
    def test_atr_suddenly_3x(self):
        ...
        # Normal decisions but extreme market features
        for i in range(25):
            detector.add_decision(make_decision(
                True, 10.0,
    ...
    def test_no_features_no_ood(self):
        """Decisions without features → OOD signal doesn't fire."""
        detector = RegimeDecayDetector(baseline=make_baseline())
        detector.fit_market_baseline(make_training_features())

        for i in range(25):
            detector.add_decision(DecisionResult(is_win=True, pnl=10.0))
...
class TestDrawdownExceedance:
    def test_large_drawdown_fires(self):
        ...
        # Build up score then crash
        for _ in range(5):
            detector.add_decision(make_decision(True, 100.0))
        # Now lose a lot (>15% of peak = 500)
        for _ in range(15):
            detector.add_decision(make_decision(False, -50.0))
...
class TestCoolingPeriod:
    def test_cooling_after_trigger(self):
        """After decay is confirmed, next 5 decisions are in cooling."""
        detector = RegimeDecayDetector(
            baseline=make_baseline(win_rate=0.6, decision_count=20, max_drawdown_pct=5.0),
        ...
        # Add 3 more decisions — should be in cooling
        for _ in range(3):
            detector.add_decision(make_decision(False, -10.0))
...
class TestEdgeCases:
    ...
    def test_custom_config(self):
        config = RegimeDetectorConfig(min_decisions=10, cooling_period=3)
    ...
    def test_all_winning_decisions_no_decay(self):
