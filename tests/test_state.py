...
from decisionmemory.models import SessionState
...
def test_update_warm_memory(state_manager):
    ...
    # Update memory
    success = state_manager.update_warm_memory("agent-003", "last_decision", "T-2026-100")
    assert success is True
    
    success = state_manager.update_warm_memory("agent-003", "total_decisions", 42)
    assert success is True
    
    # Load and verify
    loaded = state_manager.load_state("agent-003")
    assert loaded.warm_memory["last_decision"] == "T-2026-100"
    assert loaded.warm_memory["total_decisions"] == 42
