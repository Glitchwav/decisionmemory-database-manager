...
def real_client(tmp_path):
    ...
    with patch("decisionmemory.server.journal", journal), \
         patch("decisionmemory.server.state_manager", state_mgr), \
         patch("decisionmemory.server.reflection_engine", reflection), \
         patch("decisionmemory.server.adaptive_risk", risk):
...
class TestHealthEndpoint:
    ...
    def test_health_check(self, real_client):
        ...
        assert data["service"] == "DecisionMemory Protocol"
...
class TestDecisionEndpoints:
    ...
    def test_record_decision(self, real_client):
        """POST /decision/record_decision creates a decision and returns its ID."""
        resp = real_client.post("/decision/record_decision", json={
        ...
        assert data["decision_id"] == "T-2026-0001"
        assert "timestamp" in data

    def test_record_decision_invalid_confidence(self, real_client):
        """POST /decision/record_decision with confidence > 1.0 returns 422 (validation error)."""
        resp = real_client.post("/decision/record_decision", json={
            "decision_id": "T-2026-0002",
            "symbol": "XAUUSD",
            "direction": "long",
            "lot_size": 0.05,
            "strategy": "VolBreakout",
    ...
    def test_record_outcome(self, real_client):
        """POST /decision/record_outcome updates decision with exit data."""
        # First create a decision
        real_client.post("/decision/record_decision", json={
            "decision_id": "T-2026-0010",
            "symbol": "XAUUSD",
            "direction": "long",
            "lot_size": 0.05,
            "strategy": "VolBreakout",
        ...
        # Record outcome
        resp = real_client.post("/decision/record_outcome", json={
            "decision_id": "T-2026-0010",
            "exit_price": 2900.0,
            "pnl": 50.0,
        ...
        data = resp.json()
        assert data["success"] is True
        assert data["decision_id"] == "T-2026-0010"

    def test_query_history(self, real_client):
        """POST /decision/query_history returns filtered decisions."""
        # Create two decisions with different strategies
        for i, strategy in enumerate(["VolBreakout", "Pullback"], 1):
            real_client.post("/decision/record_decision", json={
                "decision_id": f"T-2026-00{i}0",
                "symbol": "XAUUSD",
                "direction": "long",
                "lot_size": 0.05,
                "strategy": strategy,
        ...
        resp = real_client.post("/decision/query_history", json={
            "strategy": "VolBreakout",
        })
        assert resp.status_code == 200
        data = resp.json()
        assert data["success"] is True
        assert data["count"] == 1
        assert data["decisions"][0]["strategy"] == "VolBreakout"

    def test_query_history_empty(self, real_client):
        """POST /decision/query_history with no matches returns empty list."""
        resp = real_client.post("/decision/query_history", json={
    ...
    def test_get_active(self, real_client):
        """GET /decision/get_active returns decisions without recorded outcome."""
        # Create a decision but don't close it
        real_client.post("/decision/record_decision", json={
            "decision_id": "T-2026-0099",
            "symbol": "XAUUSD",
            "direction": "short",
            "lot_size": 0.03,
            "strategy": "Pullback",
            "confidence": 0.65,
            "reasoning": "Open decision",
            "market_context": {"price": 2910.0},
        })

        resp = real_client.get("/decision/get_active")
        assert resp.status_code == 200
        data = resp.json()
        assert data["success"] is True
        assert data["count"] >= 1
...
class TestReflectionEndpoint:
    ...
    def test_run_daily_no_decisions(self, real_client):
        """POST /reflect/run_daily with no decisions returns summary text."""
    ...
    def test_run_weekly_no_decisions(self, real_client):
        """POST /reflect/run_weekly with no decisions returns weekly summary."""
        resp = real_client.post("/reflect/run_weekly?week_ending=2026-02-16")
        assert resp.status_code == 200
        data = resp.json()
        assert data["success"] is True
        assert "WEEKLY SUMMARY" in data["summary"]
        assert "No decisions this week" in data["summary"]

    def test_run_monthly_no_decisions(self, real_client):
        """POST /reflect/run_monthly with no decisions returns monthly summary."""
        ...
        assert "No decisions this month" in data["summary"]
...
class TestRiskEndpoints:
    ...
    def test_check_decision_approved(self, real_client):
        """POST /risk/check_decision approves normal decision."""
        resp = real_client.post("/risk/check_decision", json={
