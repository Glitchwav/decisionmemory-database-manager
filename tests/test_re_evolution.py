...
from decisionmemory.evolution.re_evolution import (
    GridCandidate,
    GridSearchSpace,
    ReEvolutionConfig,
    ReEvolutionPipeline,
...
from decisionmemory.evolution.strategy_registry import StrategyRegistry
...
def make_mock_backtest(sharpe: float = 1.0, decisions: int = 20):
...
class TestReEvolutionPipeline:
    ...
    def test_run_happy_path(self):
        pipeline = ReEvolutionPipeline(
            backtest_fn=make_mock_backtest(sharpe=2.0, decisions=30),
    ...
    def test_run_no_viable_candidates(self):
        pipeline = ReEvolutionPipeline(
            backtest_fn=make_mock_backtest(sharpe=-1.0, decisions=5),
            grid_space=self._small_space(),
        )
        result = pipeline.run(
            is_bars=[], is_contexts=[], is_atrs=[],
    ...
    def test_run_no_oos_viable(self):
        """IS passes but OOS has 0 decisions."""
        call_count = {"n": 0}
        ...
        def is_good_oos_bad(bars, contexts, atrs, pattern, timeframe="1h"):
            ...
            if call_count["n"] <= 2:  # 2 candidates in small space
                return FitnessMetrics(sharpe_ratio=2.0, decision_count=20)
            return FitnessMetrics(sharpe_ratio=-1.0, decision_count=2)
    ...
    def test_deploy_to_registry(self):
        ...
        pipeline = ReEvolutionPipeline(
            backtest_fn=make_mock_backtest(sharpe=3.0, decisions=50),
            grid_space=self._small_space(),
        )
        result = pipeline.run(
            is_bars=[], is_contexts=[], is_atrs=[],
    ...
    def test_config_min_is_decisions_filter(self):
        """Candidates below min_is_decisions should be filtered."""
        config = ReEvolutionConfig(min_is_decisions=50)
        pipeline = ReEvolutionPipeline(
            backtest_fn=make_mock_backtest(sharpe=2.0, decisions=20),
            config=config,
            grid_space=self._small_space(),
        )
        result = pipeline.run(
    ...
    def test_config_min_oos_decisions_filter(self):
        """OOS candidates below min_oos_decisions should be filtered."""
        config = ReEvolutionConfig(min_oos_decisions=100)
        pipeline = ReEvolutionPipeline(
            backtest_fn=make_mock_backtest(sharpe=2.0, decisions=20),
            config=config,
            grid_space=self._small_space(),
    ...
    def test_cumulative_trials_always_accumulate_on_dsr_fail(self):
        ...
        pipeline = ReEvolutionPipeline(
            backtest_fn=make_mock_backtest(sharpe=0.5, decisions=20),
    ...
    def test_cumulative_trials_accumulate_on_no_oos_viable(self):
        ...
        def is_good_oos_bad(bars, contexts, atrs, pattern, timeframe="1h"):
            ...
            if call_count["n"] <= 2:  # IS calls
                return FitnessMetrics(sharpe_ratio=2.0, decision_count=20)
            return FitnessMetrics(sharpe_ratio=-1.0, decision_count=2)  # OOS fails
