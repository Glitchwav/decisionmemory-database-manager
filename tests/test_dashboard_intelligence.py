...
from decisionmemory.dashboard_models import CalibrationPoint, StrategyDetailResponse
from decisionmemory.repositories.decision import DecisionRepository
from decisionmemory.services.dashboard import DashboardService, BATCH_001_BASELINES
    ...
    try:
        conn.execute(
            ...
            (id, timestamp, symbol, direction, lot_size, strategy,
             confidence, reasoning, market_context, decision_references, pnl, pnl_r, hold_duration)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
...
class TestConfidenceCalibration:
    ...
    def test_happy_path(self, client, db):
        ...
        data = resp.json()
        assert len(data) == 2
        ids = {d["decision_id"] for d in data}
        assert ids == {"m1", "m2"}
        m1 = next(d for d in data if d["decision_id"] == "m1")
...
class TestStrategyDetail:
    ...
    def test_unknown_strategy_baseline_zeros(self, client, db):
        ...
        _insert_decision(db, "t1", pnl=100.0, strategy="CustomStrat")
...
class TestCalibrationService:
    ...
    def test_data_shape(self, db, service):
        ...
        assert len(result) == 1
        assert set(result[0].keys()) == {"decision_id", "entry_confidence", "actual_pnl_r", "strategy"}
...
class TestStrategyDetailService:
    ...
    def test_not_found_raises(self, service):
        ...
        from decisionmemory.exceptions import StrategyNotFoundError
