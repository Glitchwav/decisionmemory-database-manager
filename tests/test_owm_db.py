...
def _make_episodic(id="E-001", strategy="VolBreakout", direction="long",
    ...
    return {
        ...
        "reflection": "Good breakout decision",
        "confidence": 0.7,
...
class TestEpisodicMemory:
    ...
    def test_insert_duplicate_fails(self, db):
        db.insert_episodic(_make_episodic())
        with pytest.raises(DecisionMemoryDBError):
            db.insert_episodic(_make_episodic())
...
class TestAffectiveState:
    ...
    def test_init_and_load(self, db):
        ...
        assert state["peak_score"] == 10000.0
        assert state["current_score"] == 10000.0
    ...
    def test_init_idempotent(self, db):
        ...
        state = db.load_affective()
        assert state["peak_score"] == 10000.0  # unchanged
    ...
    def test_save_and_load(self, db):
        ...
        assert loaded["current_score"] == 9500.0
    ...
    def test_save_overwrites(self, db):
        ...
        state = db.load_affective()
        assert state["peak_score"] == 12000.0
...
class TestProspectiveMemory:
    ...
    def _make_prospective(self, id="F-001", trigger_type="market_condition",
                          action_type="skip_decision", status="active"):
        return {
            "id": id,
            "trigger_type": trigger_type,
            "trigger_condition": {"regime": "ranging", "strategy": "VolBreakout"},
            "planned_action": {"type": "skip_decision", "reason": "low edge in ranging"},
    ...
    def test_insert_and_query(self, db):
        ...
        assert r["planned_action"]["type"] == "skip_decision"
    ...
    def test_update_status_triggered(self, db):
        ...
        assert db.update_prospective_status(
            ...
            outcome_pnl_r=2.5, outcome_reflection="Avoided a bad decision"
        ) is True
        ...
        assert r["outcome_reflection"] == "Avoided a bad decision"
...
class TestExistingTablesUnaffected:
    ...
    def test_decision_records_still_work(self, db):
        decision = {
            "id": "T-001",
            "timestamp": "2026-03-02T10:00:00",
            "symbol": "XAUUSD",
