...
from decisionmemory.adaptive_risk import AdaptiveRisk
from decisionmemory.models import (
    RiskStatus, RiskConstraints, DecisionProposal, DecisionCheckResult, DecisionDirection,
)
...
class TestSafeDefaults:
    ...
    def test_zero_decisions_returns_defaults(self, risk):
        """No decisions -> safe defaults with ACTIVE status."""
    ...
    def test_fewer_than_min_decisions(self, risk, journal):
        """< 5 closed decisions -> safe defaults."""
        for i in range(1, 4):
            _create_decision(journal, i, pnl=50.0)
...
class TestKellyCriterion:
    ...
    def test_kelly_with_edge(self, risk, journal):
        ...
        for i in range(1, 8):
            _create_decision(journal, i, pnl=100.0)
        for i in range(8, 11):
            _create_decision(journal, i, pnl=-50.0)
    ...
    def test_kelly_no_edge(self, risk, journal):
        ...
        for i in range(1, 4):
            _create_decision(journal, i, pnl=10.0)
        for i in range(4, 11):
            _create_decision(journal, i, pnl=-50.0)
...
class TestDrawdownScaling:
    ...
    def test_no_drawdown(self, risk, journal):
        ...
        for i in range(1, 6):
            _create_decision(journal, i, pnl=100.0)
        _create_decision(journal, 6, pnl=-10.0)
        _create_decision(journal, 7, pnl=100.0)
        _create_decision(journal, 8, pnl=-10.0)
        _create_decision(journal, 9, pnl=100.0)
    ...
    def test_moderate_drawdown(self, risk, journal):
        ...
        # Start with profit, then sustained losses ~6% of $10k score
        for i in range(1, 4):
            _create_decision(journal, i, pnl=100.0)
        for i in range(4, 11):
            _create_decision(journal, i, pnl=-120.0)
    ...
    def test_severe_drawdown(self, risk, journal):
        ...
        for i in range(1, 3):
            _create_decision(journal, i, pnl=50.0)
        for i in range(3, 13):
            _create_decision(journal, i, pnl=-150.0)
...
class TestSessionAdjustments:
    ...
    def test_low_session_win_rate(self, risk, journal):
        ...
        _create_decision(journal, 1, pnl=50.0, session="asian")
        for i in range(2, 6):
            _create_decision(journal, i, pnl=-30.0, session="asian")

        # London: enough to pass MIN_DECISIONS
        for i in range(6, 11):
            _create_decision(journal, i, pnl=50.0, session="london")
    ...
    def test_insufficient_session_data(self, risk, journal):
        """< 3 decisions in a session -> conservative 0.75."""
        # Only 2 asian decisions + 8 london decisions
        for i in range(1, 3):
            _create_decision(journal, i, pnl=50.0, session="asian")
        for i in range(3, 11):
            _create_decision(journal, i, pnl=50.0, session="london")
...
class TestConsecutiveLosses:
    ...
    def test_consecutive_losses_stopped(self, risk, journal):
        ...
        We need >= 5 total closed decisions to pass MIN_DECISIONS.
        """
        # 2 wins first, then 3 consecutive losses
        for i in range(1, 3):
            _create_decision(journal, i, pnl=50.0)
        for i in range(3, 6):
            _create_decision(journal, i, pnl=-30.0)
    ...
    def test_win_resets_streak(self, risk, journal):
        ...
        for i in range(3, 5):
            _create_decision(journal, i, pnl=-30.0)
        _create_decision(journal, 5, pnl=50.0)
...
class TestDailyLossLimit:
    ...
    def test_daily_loss_exceeded(self, risk, journal):
        ...
        # All decisions are today (days_ago=0 is default)
    ...
    def test_daily_loss_warning(self, risk, journal):
        ...
        for i in range(4, 7):
            _create_decision(journal, i, pnl=-60.0)
...
class TestCheckDecision:
    """Tests for decision proposal validation."""

    def test_approved_decision(self, risk, state_manager):
        """Normal decision within constraints is approved."""
        ...
        result = risk.check_decision("agent-check", proposal)
    ...
    def test_stopped_rejection(self, risk, state_manager):
        ...
        result = risk.check_decision("agent-stop", proposal)
    ...
    def test_session_adjustment_applied(self, risk, state_manager):
        ...
        result = risk.check_decision("agent-sess", proposal)
        assert result.approved is True
        assert result.adjusted_lot_size == 0.05
    ...
    def test_floor_applied(self, risk, state_manager):
        ...
        result = risk.check_decision("agent-floor", proposal)
        assert result.approved is True
...
class TestEndToEnd:
    ...
    def test_calculate_then_check(self, risk, journal):
        ...
        for i in range(1, 6):
            _create_decision(journal, i, pnl=100.0)
        _create_decision(journal, 6, pnl=-50.0)
        _create_decision(journal, 7, pnl=100.0)
        _create_decision(journal, 8, pnl=-50.0)
        _create_decision(journal, 9, pnl=100.0)
        _create_decision(journal, 10, pnl=-50.0)
        ...
        result = risk.check_decision(agent, proposal)
        assert result.approved is True
