...
from decisionmemory.evolution.llm import LLMError, MockLLMClient
from decisionmemory.evolution.models import (
    EvolutionConfig,
    EvolutionRun,
    HypothesisStatus,
)
from decisionmemory.evolution.generator import GenerationConfig
from decisionmemory.evolution.selector import SelectionConfig
...
class TestEvolveBasic:
    ...
    async def test_single_generation_produces_run(self):
        ...
        config = EngineConfig(
            ...
            selection=SelectionConfig(
                top_n=3,
                min_is_decision_count=0,
                min_is_sharpe=-999,
                min_oos_sharpe=-999,
                min_oos_decision_count=0,
    ...
    async def test_hypotheses_get_generation_number(self):
        ...
        config = EngineConfig(
            ...
            selection=SelectionConfig(
                min_is_decision_count=0, min_is_sharpe=-999,
            ),
        )
        engine = EvolutionEngine(mock, config)
...
class TestMinBarsWarning:
    ...
    async def test_warning_logged_for_small_dataset(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="decisionmemory.evolution.engine"):
