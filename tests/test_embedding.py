"""Tests for embedding module (embedding backend protocol + decision context embedding)."""
...
import decisionmemory.embedding as emb_module
from decisionmemory.embedding import EmbeddingBackend, embed_decision_context
...
def test_embed_decision_context_no_backend():
    """When backend is unavailable, embed_decision_context returns None without crashing."""
    with patch.dict(sys.modules, {"sentence_transformers": None}):
        result = embed_decision_context({"strategy": "VolBreakout", "direction": "BUY"})
    assert result is None


def test_embed_decision_context_text_format():
    ...
    with patch.object(emb_module, "_backend_cache", fake_backend), \
        ...
        result = embed_decision_context(decision)
...
def test_embed_decision_context_missing_fields():
    """When decision_data is missing some or all context fields, no crash occurs."""
    fake_backend = MagicMock()
    ...
    with patch.object(emb_module, "_backend_cache", fake_backend), \
        ...
        result = embed_decision_context({"strategy": "VolBreakout"})
        assert result is not None
        call_text = fake_backend.embed.call_args[0][0]
