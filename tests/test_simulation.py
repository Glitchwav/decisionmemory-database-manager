...
def test_base_agent_generates_signals():
    """BaseAgent should generate decision signals when conditions are met."""
    from decisionmemory.data.context_builder import MarketContext, Regime
    ...
    signal = agent.should_decision(ctx)
    ...
    assert agent.should_decision(ctx_neg) is None
...
def test_calibrated_agent_skips_low_dqs():
    """CalibratedAgent should skip decisions when DQS is in 'skip' tier."""
    ...
    agent.db.init_affective(peak_score=10000.0, current_score=7000.0)
...
def test_calibrated_agent_reduces_on_changepoint():
    ...
    from decisionmemory.simulation.agent import CalibratedAgent, SimulatedDecision
    ...
    agent.db.init_affective(peak_score=10000.0, current_score=10000.0)
...
def test_simulator_fitness_metrics_valid():
    ...
    assert fm.decision_count >= 0
    # All decisions should have valid pnl_r
    for t in result.decisions:
...
def test_ab_experiment_same_data():
    ...
    from decisionmemory.simulation.experiment import ABExperiment
...
def test_comparison_report_fields():
    ...
    assert isinstance(report.decisions_skipped_by_b, int)
    assert isinstance(report.pnl_of_skipped_decisions, float)
...
def test_preset_strategies_valid():
    ...
    from decisionmemory.simulation.presets import PRESET_STRATEGIES
...
def test_report_generation():
    ...
    from decisionmemory.simulation.report import FullExperimentReport
    ...
    assert "# Self-Calibrating DecisionMaking Agent" in md
