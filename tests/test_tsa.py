"""Tests for decisionmemory.audit.tsa — RFC 3161 TimeStampReq construction
...
from decisionmemory.audit.tsa import (
    DEFAULT_TSA_URL,
    TSAError,
    build_tsq,
...
def test_request_timestamp_uses_env_url():
    ...
    with patch.dict("os.environ", {"DECISIONMEMORY_TSA_URL": custom}):
        with patch(
...
def test_build_daily_root_with_tsa_failure_still_persists_root(tmp_path):
    ...
    from decisionmemory.audit.chain import ChainBuilder, GENESIS_HASH, chained_hash
    ...
    # Force TSA to fail.
    with patch(
        "decisionmemory.audit.tsa.request_timestamp",
...
def test_build_daily_root_with_tsa_success_stores_token(tmp_path):
    from decisionmemory.audit.chain import ChainBuilder, GENESIS_HASH, chained_hash
    from decisionmemory.audit.tsa import TSAResponse
    from decisionmemory.db import Database
    import sqlite3
    ...
    with patch(
        "decisionmemory.audit.tsa.request_timestamp", side_effect=_fake_request
    ):
        root = ChainBuilder(conn).build_daily_root(
            "2026-05-14", request_tsa=True
        )
