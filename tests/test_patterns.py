...
def _insert_backtest_decision(db, variant_tag, idx, pnl, strategy, symbol,
                           direction_filter, session="london",
                           lot_size=0.1, timestamp="2025-06-15T10:00:00"):
    """Insert a backtest decision with proper tags and ID format."""
    decision_id = f"BT-{variant_tag}-{idx:04d}"
...
def _seed_multi_strategy_data(db):
    ...
    # IM XAUUSD BUY — profitable (4 decisions)
    for i in range(4):
        pnl = 500.0 if i < 3 else -200.0  # 75% WR
        _insert_backtest_decision(db, "IM_XAUUSD_BUY_RR3_TH0.5", i, pnl,
                               "IntradayMomentum", "XAUUSD", "BUY",
                               timestamp=f"2025-06-{15+i:02d}T10:00:00")

    # IM XAUUSD BOTH — less profitable (4 decisions)
    for i in range(4):
        pnl = 300.0 if i < 2 else -250.0  # 50% WR
        _insert_backtest_decision(db, "IM_XAUUSD_BOTH_RR3_TH0.5", i, pnl,
                               "IntradayMomentum", "XAUUSD", "BOTH",
                               timestamp=f"2025-06-{15+i:02d}T11:00:00")

    # IM EURUSD BUY — slightly positive (4 decisions)
    for i in range(4):
        pnl = 100.0 if i < 2 else -80.0
        _insert_backtest_decision(db, "IM_EURUSD_BUY_RR3_TH0.5", i, pnl,
                               "IntradayMomentum", "EURUSD", "BUY",
                               timestamp=f"2025-06-{15+i:02d}T12:00:00")

    # VB XAUUSD BUY — profitable (4 decisions)
    for i in range(4):
        pnl = 400.0 if i < 3 else -100.0
        _insert_backtest_decision(db, "VB_XAUUSD_BUY_RR3_BUF0.1", i, pnl,
                               "VolBreakout", "XAUUSD", "BUY",
                               timestamp=f"2025-06-{15+i:02d}T13:00:00")

    # VB EURUSD BUY — unprofitable (4 decisions)
    for i in range(4):
        pnl = -200.0 if i < 3 else 50.0
        _insert_backtest_decision(db, "VB_EURUSD_BUY_RR3_BUF0.1", i, pnl,
                               "VolBreakout", "EURUSD", "BUY",
                               timestamp=f"2025-06-{15+i:02d}T14:00:00")

    # MR XAUUSD BUY — unprofitable (4 decisions)
    for i in range(4):
        pnl = -300.0 if i < 3 else 100.0
        _insert_backtest_decision(db, "MR_XAUUSD_BUY_BB2_RSI30", i, pnl,
                               "MeanReversion", "XAUUSD", "BUY",
                               timestamp=f"2025-06-{15+i:02d}T15:00:00")

    # MR XAUUSD BOTH — very unprofitable (4 decisions)
    for i in range(4):
        pnl = -400.0
        _insert_backtest_decision(db, "MR_XAUUSD_BOTH_BB2_RSI30", i, pnl,
...
class TestTopVariants:
    ...
    def test_detect_top_variants_min_decisions(self, temp_db, journal, reflection):
        """With only 4 decisions per variant (< 10 threshold), no patterns should be generated."""
        _seed_multi_strategy_data(temp_db)
        conn = temp_db._get_connection()
        try:
        ...
        # Each variant in seed data has only 4 decisions, all below n>=10 threshold
        assert len(patterns) == 0

    def test_detect_top_variants_with_enough_data(self, temp_db, journal, reflection):
        """With 12 decisions per variant, should generate top/bottom patterns."""
        # Insert 12 decisions per variant (above n>=10 threshold)
        for i in range(12):
            _insert_backtest_decision(temp_db, "IM_XAUUSD_BUY_RR3_TH0.5", i, 500.0,
                                   "IntradayMomentum", "XAUUSD", "BUY",
                                   timestamp=f"2025-06-{(i % 28) + 1:02d}T10:00:00")
            _insert_backtest_decision(temp_db, "MR_XAUUSD_BUY_BB2_RSI30", i, -300.0,
                                   "MeanReversion", "XAUUSD", "BUY",
