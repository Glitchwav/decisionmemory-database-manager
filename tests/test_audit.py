...
from decisionmemory.domain.tdr import (
...
@pytest.fixture
def client(db):
    from decisionmemory.server import app, journal
...
def _insert_decision(db, decision_id="TEST-001", strategy="VolBreakout", pnl=117.80):
    conn = db._get_connection()
    try:
        conn.execute("""
            INSERT INTO decision_records
            (id, timestamp, symbol, direction, lot_size, strategy, confidence,
             reasoning, market_context, decision_references, exit_timestamp,
             exit_price, pnl, pnl_r, hold_duration, exit_reasoning,
             slippage, execution_quality, lessons, tags, grade)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            decision_id, "2026-03-25T10:00:00Z", "XAUUSD", "long", 0.01,
            strategy, 0.7,
...
class TestTDRSchema:
    def test_create_tdr_from_dict(self):
        ...
        tdr = DecisionMakingDecisionRecord.from_decision_record(decision)
    ...
    def test_data_hash_deterministic(self):
        h1 = DecisionMakingDecisionRecord.compute_hash("T-001", "2026", "XAUUSD", "long", "VB", 0.7, "test", {})
        h2 = DecisionMakingDecisionRecord.compute_hash("T-001", "2026", "XAUUSD", "long", "VB", 0.7, "test", {})
        assert h1 == h2

    def test_data_hash_changes_on_tamper(self):
        h1 = DecisionMakingDecisionRecord.compute_hash("T-001", "2026", "XAUUSD", "long", "VB", 0.7, "test", {})
        h2 = DecisionMakingDecisionRecord.compute_hash("T-001", "2026", "XAUUSD", "long", "VB", 0.8, "test", {})
        assert h1 != h2

    def test_memory_context_defaults(self):
        mem = MemoryContext()
        assert mem.similar_decisions == []
...
# =====================================================================
# Task 2.2: /audit/decision-record/{decision_id}
# =====================================================================
...
class TestAuditEndpoint:
    def test_get_decision_record(self, client, db):
        ...
        assert len(data["memory"]["similar_decisions"]) == 2
...
class TestAuditExport:
    ...
    def test_export_with_strategy_filter(self, client, db):
        _insert_decision(db, "T-VB", "VolBreakout")
        _insert_decision(db, "T-IM", "IntradayMomentum")
