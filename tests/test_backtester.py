...
from decisionmemory.data.context_builder import (
    ContextConfig,
    MarketContext,
    Regime,
    Session,
...
class TestBacktest:
    def test_basic_long_uptrend(self):
        ...
        assert fitness.decision_count > 0
    ...
    def test_too_few_bars(self):
        ...
        pattern = make_pattern()
        fitness = backtest(series, pattern)
        assert fitness.decision_count == 0  # not enough for ATR computation
    ...
    def test_time_based_exit(self):
        ...
        fitness = backtest(series, pattern)
        # All decisions should exit by time
        assert fitness.decision_count > 0
        assert fitness.avg_holding_bars <= 3

    def test_no_entry_conditions_no_decisions(self):
    ...
    def test_specific_hour_filter(self):
        ...
        fitness = backtest(series, pattern)
        # Should have fewer decisions than the "always enter" version
        all_entry = backtest(series, make_pattern(max_bars=5))
        assert fitness.decision_count < all_entry.decision_count
...
class TestComputeFitness:
    def test_all_wins(self):
        decisions = [
            Decision(entry_bar=0, exit_bar=5, direction="long", entry_price=100, exit_price=110, pnl=10, holding_bars=5),
            Decision(entry_bar=10, exit_bar=15, direction="long", entry_price=110, exit_price=120, pnl=10, holding_bars=5),
        ]
        f = _compute_fitness(decisions)
        assert f.win_rate == 1.0
        assert f.total_pnl == 20.0
        assert f.decision_count == 2
        assert f.profit_factor == 999.0  # capped inf

    def test_mixed_results(self):
        decisions = [
            Decision(entry_bar=0, exit_bar=5, direction="long", entry_price=100, exit_price=110, pnl=10, holding_bars=5),
            Decision(entry_bar=10, exit_bar=15, direction="long", entry_price=110, exit_price=105, pnl=-5, holding_bars=5),
            Decision(entry_bar=20, exit_bar=25, direction="long", entry_price=105, exit_price=115, pnl=10, holding_bars=5),
        ]
        f = _compute_fitness(decisions)
        assert f.decision_count == 3
    ...
    def test_empty_decisions(self):
        f = _compute_fitness([])
        assert f.decision_count == 0
        assert f.sharpe_ratio == 0

    def test_single_decision(self):
        decisions = [Decision(entry_bar=0, exit_bar=5, direction="long", entry_price=100, exit_price=110, pnl=10, holding_bars=5)]
        f = _compute_fitness(decisions)
    ...
    def test_avg_holding(self):
        decisions = [
            Decision(entry_bar=0, exit_bar=3, direction="long", entry_price=100, exit_price=110, pnl=10, holding_bars=3),
            Decision(entry_bar=10, exit_bar=17, direction="long", entry_price=100, exit_price=110, pnl=10, holding_bars=7),
        ]
        f = _compute_fitness(decisions)
...
class TestAnnualizationFactor:
    ...
    def test_sharpe_uses_timeframe(self):
        ...
        decisions = [
            ...
            Decision(entry_bar=10, exit_bar=15, direction="long", entry_price=110, exit_price=105, pnl=-5, holding_bars=5),
            Decision(entry_bar=20, exit_bar=25, direction="long", entry_price=105, exit_price=115, pnl=10, holding_bars=5),
            Decision(entry_bar=30, exit_bar=35, direction="long", entry_price=115, exit_price=120, pnl=5, holding_bars=5),
            Decision(entry_bar=40, exit_bar=45, direction="long", entry_price=120, exit_price=118, pnl=-2, holding_bars=5),
        ]
        f_h1 = _compute_fitness(decisions, timeframe="1h")
        f_d1 = _compute_fitness(decisions, timeframe="1d")
...
class TestTrailingStop:
    def test_trailing_stop_long(self):
        ...
        decision = check_exit_position(pos, bar3, 3)
        assert decision is not None
        assert decision.exit_reason == "trailing"
        assert decision.exit_price == 105.0  # 110 - 5
    ...
    def test_trailing_stop_short(self):
        ...
        decision = check_exit_position(pos, bar2, 2)
        assert decision is not None
        assert decision.exit_reason == "trailing"
        assert decision.exit_price == 95.0  # 90 + 5
    ...
    def test_trailing_stop_not_set(self):
        ...
        decision = check_exit_position(pos, bar, 1)
...
class TestSLTPPriority:
    def test_sltp_priority_over_time_exit(self):
        ...
        decision = check_exit_position(pos, bar, 5)
        assert decision is not None
        assert decision.exit_reason == "sl"  # SL takes priority, not "time"
        assert decision.exit_price == 95.0


# --- Sharpe guard for low decision counts ---


class TestSharpeGuard:
    """Sharpe ratio should be 0.0 for decision counts below MIN_DECISIONS_FOR_SHARPE."""

    def test_constant_value(self):
        """MIN_DECISIONS_FOR_SHARPE is 5."""
        assert MIN_DECISIONS_FOR_SHARPE == 5

    def test_one_decision_sharpe_zero(self):
        """1 decision → sharpe_ratio=0.0 (not an extreme value)."""
        decisions = [
            Decision(entry_bar=0, exit_bar=5, direction="long",
    ...
    def test_four_decisions_sharpe_zero(self):
        """4 decisions (< MIN_DECISIONS_FOR_SHARPE) → sharpe_ratio=0.0."""
        decisions = [
            Decision(entry_bar=i * 10, exit_bar=i * 10 + 5, direction="long",
                  entry_price=100, exit_price=110, pnl=10, holding_bars=5)
            for i in range(4)
        ]
        f = _compute_fitness(decisions)
        assert f.decision_count == 4
        assert f.sharpe_ratio == 0.0

    def test_five_decisions_sharpe_computed(self):
        """5 decisions (= MIN_DECISIONS_FOR_SHARPE) → sharpe computed normally."""
        decisions = [
            Decision(entry_bar=i * 10, exit_bar=i * 10 + 5, direction="long",
                  entry_price=100, exit_price=110, pnl=10, holding_bars=5)
            for i in range(5)
        ...
        f = _compute_fitness(decisions)
        assert f.decision_count == 5
        # All same PnL → std=0 → sharpe=0, so use mixed PnLs
        # This test just verifies it doesn't force to 0 via the guard

    def test_five_mixed_decisions_sharpe_nonzero(self):
        """5 decisions with mixed PnL → sharpe computed (nonzero)."""
        decisions = [
            Decision(entry_bar=0, exit_bar=5, direction="long", entry_price=100, exit_price=115, pnl=15, holding_bars=5),
            Decision(entry_bar=10, exit_bar=15, direction="long", entry_price=100, exit_price=95, pnl=-5, holding_bars=5),
            Decision(entry_bar=20, exit_bar=25, direction="long", entry_price=100, exit_price=112, pnl=12, holding_bars=5),
            Decision(entry_bar=30, exit_bar=35, direction="long", entry_price=100, exit_price=108, pnl=8, holding_bars=5),
            Decision(entry_bar=40, exit_bar=45, direction="long", entry_price=100, exit_price=103, pnl=3, holding_bars=5),
        ]
        f = _compute_fitness(decisions)
        assert f.decision_count == 5
        assert f.sharpe_ratio != 0.0  # should compute a real Sharpe

    def test_two_decisions_extreme_pnl_sharpe_zero(self):
        """2 decisions with extreme PnL difference → sharpe still 0.0 (guarded)."""
        decisions = [
            Decision(entry_bar=0, exit_bar=5, direction="long", entry_price=100, exit_price=200, pnl=100, holding_bars=5),
            Decision(entry_bar=10, exit_bar=15, direction="long", entry_price=100, exit_price=50, pnl=-50, holding_bars=5),
        ]
        f = _compute_fitness(decisions)
        assert f.decision_count == 2
        assert f.sharpe_ratio == 0.0
