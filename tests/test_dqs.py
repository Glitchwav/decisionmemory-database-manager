...
def _insert_decision(db, decision_id, strategy, pnl, pnl_r=None, regime=None, confidence=0.5):
    """Helper to insert a decision into episodic + decision_records."""
...
def test_dqs_no_history():
    """With no decision history, DQS should return neutral scores around 5.0."""
...
def test_dqs_good_conditions():
    ...
    # Insert 10 winning decisions in trending_up regime
    for i in range(10):
        _insert_decision(db, f"good-{i}", "VolBreakout", pnl=200.0, pnl_r=1.5,
...
def test_dqs_regime_mismatch():
    ...
    for i in range(8):
        _insert_decision(db, f"up-{i}", "VolBreakout", pnl=200.0, pnl_r=1.0,
                       regime="trending_up")

    # 5 losses in ranging
    for i in range(5):
        _insert_decision(db, f"range-{i}", "VolBreakout", pnl=-150.0, pnl_r=-1.0,
                       regime="ranging")
...
def test_dqs_during_drawdown():
    ...
    db.init_affective(peak_score=10000.0, current_score=7500.0)
...
def test_dqs_calibrate_with_data():
    """50+ decisions should produce valid calibration weights."""
    from decisionmemory.owm.dqs import DQSEngine

    db = _get_db()

    # Insert 60 decisions: 40 wins, 20 losses
    for i in range(40):
        _insert_decision(db, f"cal-win-{i}", "VolBreakout", pnl=200.0, pnl_r=1.0,
                       regime="trending_up", confidence=0.8)
    for i in range(20):
        _insert_decision(db, f"cal-loss-{i}", "VolBreakout", pnl=-100.0, pnl_r=-0.5,
                       regime="ranging", confidence=0.4)

    db.init_affective(peak_score=10000.0, current_score=10000.0)

    engine = DQSEngine(db)
    cal_result = engine.calibrate(min_decisions=50)
...
def test_dqs_calibrate_insufficient_data():
    """Fewer than min_decisions should return insufficient_data."""
    from decisionmemory.owm.dqs import DQSEngine

    db = _get_db()

    for i in range(10):
        _insert_decision(db, f"few-{i}", "VolBreakout", pnl=100.0)

    engine = DQSEngine(db)
    result = engine.calibrate(min_decisions=50)
...
async def test_dqs_mcp_tool():
    ...
    from decisionmemory.mcp_server import compute_dqs
...
# -----------------------------------------------------------------------
# Test 9: DQS stored in remember_decision context_json
# -----------------------------------------------------------------------

@pytest.mark.asyncio
async def test_dqs_stored_in_remember_decision():
    """remember_decision should store dqs_score and dqs_tier in context_json."""
