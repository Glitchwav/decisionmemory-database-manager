...
from decisionmemory.evolution.selector import (
    SelectionConfig,
    SelectionResult,
    rank_by_is_fitness,
...
def make_fitness(
    ...
    return FitnessMetrics(
        ...
        expectancy=total_pnl / decision_count if decision_count > 0 else 0,
    )
...
class TestSelectionConfig:
    def test_defaults(self):
        ...
        assert c.min_oos_decision_count == 30
...
class TestRankByISFitness:
    def test_ranks_by_sharpe(self):
        h1 = make_hypothesis("Low", is_fitness=make_fitness(sharpe=0.5, decision_count=20))
        h2 = make_hypothesis("High", is_fitness=make_fitness(sharpe=2.0, decision_count=20))
        h3 = make_hypothesis("Mid", is_fitness=make_fitness(sharpe=1.2, decision_count=20))
    ...
    def test_filters_low_decision_count(self):
        h1 = make_hypothesis("Good", is_fitness=make_fitness(sharpe=2.0, decision_count=20))
        h2 = make_hypothesis("Few Decisions", is_fitness=make_fitness(sharpe=3.0, decision_count=5))
    ...
    def test_filters_negative_sharpe(self):
        h1 = make_hypothesis("Winner", is_fitness=make_fitness(sharpe=1.0, decision_count=20))
        h2 = make_hypothesis("Loser", is_fitness=make_fitness(sharpe=-0.5, decision_count=20))
    ...
    def test_top_n_limit(self):
        hypotheses = [
            make_hypothesis(f"H{i}", is_fitness=make_fitness(sharpe=float(i), decision_count=20))
            for i in range(20)
        ]
    ...
    def test_no_fitness_skipped(self):
        h1 = make_hypothesis("Has Fitness", is_fitness=make_fitness(decision_count=20))
    ...
    def test_rank_by_profit_factor(self):
        h1 = make_hypothesis("Low PF", is_fitness=make_fitness(sharpe=2.0, profit_factor=1.2, decision_count=20))
        h2 = make_hypothesis("High PF", is_fitness=make_fitness(sharpe=1.0, profit_factor=3.0, decision_count=20))
...
class TestValidateOOS:
    def test_passes_all_thresholds(self):
        h = make_hypothesis(
            oos_fitness=make_fitness(sharpe=1.5, decision_count=50, win_rate=0.55, profit_factor=1.8, max_dd=10.0),
        )
    ...
    def test_fails_sharpe(self):
        h = make_hypothesis(
            oos_fitness=make_fitness(sharpe=0.5, decision_count=50),
        )
        passed, reason = validate_oos(h)
        assert passed is False
        assert "Sharpe" in reason

    def test_fails_decision_count(self):
        h = make_hypothesis(
            oos_fitness=make_fitness(decision_count=10),
        )
        passed, reason = validate_oos(h)
        assert passed is False
        assert "decision_count" in reason

    def test_fails_drawdown(self):
        h = make_hypothesis(
            oos_fitness=make_fitness(max_dd=25.0, decision_count=50),
        )
        passed, reason = validate_oos(h)
        assert passed is False
    ...
    def test_fails_profit_factor(self):
        h = make_hypothesis(
            oos_fitness=make_fitness(profit_factor=0.9, decision_count=50),
        )
        passed, reason = validate_oos(h)
        assert passed is False
        assert "PF" in reason
    ...
    def test_fails_win_rate(self):
        h = make_hypothesis(
            oos_fitness=make_fitness(win_rate=0.3, decision_count=50),
        )
        passed, reason = validate_oos(h)
        assert passed is False
    ...
    def test_custom_thresholds(self):
        h = make_hypothesis(
            oos_fitness=make_fitness(sharpe=1.8, decision_count=50),
        )
    ...
    def test_borderline_passes(self):
        ...
        h = make_hypothesis(
            oos_fitness=make_fitness(
                sharpe=1.0, decision_count=30, win_rate=0.4,
...
class TestSelectAndEliminate:
    def test_full_pipeline(self):
        ...
        good = make_hypothesis(
            "Winner",
            is_fitness=make_fitness(sharpe=2.0, decision_count=50),
            oos_fitness=make_fitness(sharpe=1.5, decision_count=40, profit_factor=1.8),
        )
        bad_oos = make_hypothesis(
            "Bad OOS",
            is_fitness=make_fitness(sharpe=1.5, decision_count=50),
            oos_fitness=make_fitness(sharpe=0.3, decision_count=40),
        )
        bad_is = make_hypothesis(
            "Bad IS",
            is_fitness=make_fitness(sharpe=-1.0, decision_count=50),
            oos_fitness=make_fitness(sharpe=2.0, decision_count=40),
        )
    ...
    def test_graveyard_entries(self):
        ...
        h = make_hypothesis(
            "Loser",
            is_fitness=make_fitness(sharpe=1.0, decision_count=50),
            oos_fitness=make_fitness(sharpe=0.2, decision_count=40),
        )
    ...
    def test_all_graduate(self):
        ...
        hypotheses = [
            make_hypothesis(
                f"H{i}",
                is_fitness=make_fitness(sharpe=2.0 + i * 0.1, decision_count=50),
                oos_fitness=make_fitness(sharpe=1.5 + i * 0.1, decision_count=40, profit_factor=1.8),
            )
            for i in range(3)
        ]
    ...
    def test_all_eliminated(self):
        ...
        hypotheses = [
            make_hypothesis(
                f"H{i}",
                is_fitness=make_fitness(sharpe=0.5 + i * 0.1, decision_count=50),
                oos_fitness=make_fitness(sharpe=0.1, decision_count=10),
            )
            for i in range(3)
        ]
