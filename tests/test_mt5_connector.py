"""Tests for MT5Connector with mocked MetaDecisionMaker5 library."""
...
from decisionmemory.mt5_connector import MT5Connector
...
class TestMT5Init:
    ...
    def test_init_without_mt5_library(self, db):
        ...
        with patch("decisionmemory.mt5_connector.MT5Connector._init_mt5"):
            conn = MT5Connector(journal=DecisionJournal(db=db), state_manager=StateManager(db=db))
...
class TestConnect:
    ...
    def test_connect_without_mt5_raises(self, connector):
        ...
        connector.mt5 = None
        with pytest.raises(RuntimeError, match="MetaDecisionMaker5 library not installed"):
...
class TestSyncDecisions:
    """Tests for MT5Connector.sync_decisions()."""

    def test_sync_without_mt5_raises(self, connector):
        """sync_decisions() raises RuntimeError when MT5 is not connected."""
        connector.mt5 = None
        with pytest.raises(RuntimeError, match="MT5 not connected"):
            connector.sync_decisions()

    def test_sync_no_history(self, connector):
        """sync_decisions() returns error stats when history_deals_get returns None."""
        mock_mt5 = MagicMock()
        mock_mt5.history_deals_get.return_value = None
        connector.mt5 = mock_mt5

        result = connector.sync_decisions()
        assert result == {"synced": 0, "skipped": 0, "errors": 1}

    def test_sync_empty_history(self, connector):
        """sync_decisions() returns zero stats when no deals found."""
        mock_mt5 = MagicMock()
    ...
    def test_sync_one_complete_decision(self, connector):
        """sync_decisions() records decision and outcome for a complete position."""
        # Create mock deals
        ...
        decision = connector.journal.get_decision("MT5-1001")
        assert decision is not None
        assert decision.symbol == "XAUUSD"
        assert decision.direction.value == "long"
        assert decision.pnl == 45.0  # -5 + 50

    def test_sync_skips_already_synced(self, connector):
        """sync_decisions() skips decisions that were already recorded."""
        entry_deal = MagicMock()
...
class TestExtractDecisionData:
    """Tests for MT5Connector._extract_decision_data()."""

    def test_empty_deals(self, connector):
        """Empty deal list returns None."""
        assert connector._extract_decision_data([]) is None
    ...
    def test_single_deal_no_exit(self, connector):
        ...
        data = connector._extract_decision_data([deal])
    ...
    def test_two_deals_complete_decision(self, connector):
        """Two deals (entry + exit) extracts full decision data."""
        entry = MagicMock()
        entry.symbol = "XAUUSD"
        entry.volume = 0.05
        entry.type = 0  # BUY
        ...
        data = connector._extract_decision_data([entry, exit_d])
