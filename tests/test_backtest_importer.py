...
<table>
   ...
   <tr bgcolor="#FFFFFF" align=right><td>2024.01.05 14:00:00</td><td>5</td><td>XAUUSD</td><td>buy</td><td>out</td><td>0.03</td><td>2065.00</td><td>5</td><td>0.00</td><td>0.00</td><td>-15.00</td><td>10007.50</td><td></td></tr>
    ...
    def test_parse_two_decisions(self, sample_report):
        decisions = parse_mt5_report(sample_report)
        assert len(decisions) == 2

    def test_first_decision_long(self, sample_report):
        decisions = parse_mt5_report(sample_report)
        t = decisions[0]
    ...
    def test_second_decision_short(self, sample_report):
        decisions = parse_mt5_report(sample_report)
        t = decisions[1]
    ...
    def test_hold_duration(self, sample_report):
        decisions = parse_mt5_report(sample_report)
        # First decision: 15:15 to 16:30 = 75 min
        assert decisions[0]['hold_duration_min'] == 75
        # Second decision: 10:00 to 14:00 = 240 min
        assert decisions[1]['hold_duration_min'] == 240

    def test_empty_report(self, tmp_path):
        html = "<!DOCTYPE html><html><body><table></table></body></html>"
        report_file = tmp_path / "empty_report.htm"
        report_file.write_text(html, encoding='utf-16-le')
        decisions = parse_mt5_report(str(report_file))
        assert len(decisions) == 0

    def test_nonexistent_file(self):
        decisions = parse_mt5_report("/nonexistent/path/report.htm")
        assert len(decisions) == 0


class TestBuildDecisionRecords:
    def test_builds_correct_records(self):
        ...
        records = build_decision_records(decisions, "VB_XAUUSD_BUY_RR3_BUF0.1")
    ...
    def test_decision_id_format(self):
        ...
        records = build_decision_records(decisions, "IM_EURUSD_BOTH_RR2.5_TH0.55")
    ...
    def test_session_classification_in_records(self):
        ...
        records = build_decision_records(decisions, "MR_XAUUSD_BUY_BB2.0_RSI30")
...
class TestWithRealReports:
    ...
    def test_parse_real_vb_gbpusd(self, real_report):
        decisions = parse_mt5_report(real_report)
        assert len(decisions) > 100  # VB GBPUSD had 503 decisions
    ...
    def test_build_records_from_real(self, real_report):
        decisions = parse_mt5_report(real_report)
        records = build_decision_records(decisions[:5], "VB_GBPUSD_BOTH_RR3.5_BUF0.15")
