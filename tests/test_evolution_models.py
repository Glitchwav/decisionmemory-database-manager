...
class TestFitnessMetrics:
    def test_viable(self):
        f = FitnessMetrics(sharpe_ratio=1.5, decision_count=50)
        assert f.is_viable is True

    def test_not_viable_low_decisions(self):
        f = FitnessMetrics(sharpe_ratio=2.0, decision_count=10)
        assert f.is_viable is False

    def test_not_viable_negative_sharpe(self):
        f = FitnessMetrics(sharpe_ratio=-0.5, decision_count=100)
        assert f.is_viable is False

    def test_passes_oos(self):
        f = FitnessMetrics(sharpe_ratio=1.5, decision_count=50, max_drawdown_pct=10)
        assert f.passes_oos_filter is True

    def test_fails_oos_sharpe(self):
        f = FitnessMetrics(sharpe_ratio=0.8, decision_count=50, max_drawdown_pct=10)
        assert f.passes_oos_filter is False

    def test_fails_oos_drawdown(self):
        f = FitnessMetrics(sharpe_ratio=2.0, decision_count=50, max_drawdown_pct=25)
        assert f.passes_oos_filter is False
...
class TestHypothesis:
    ...
    def test_graveyard_entry(self):
        h = Hypothesis(
            ...
            fitness_is=FitnessMetrics(sharpe_ratio=-0.5, decision_count=20),
