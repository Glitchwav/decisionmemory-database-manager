...
from decisionmemory.replay.engine import ReplayEngine, run_replay
...
class TestHoldOnly:
    @patch("decisionmemory.replay.engine.LLMClient")
    def test_all_holds_no_decisions(self, MockLLMClient):
        """If LLM always returns HOLD, no decisions should open."""
        mock_llm = MagicMock()
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert summary["decisions"] == 0
            assert summary["score"] == config.initial_score
            assert mock_llm.decide.call_count > 0


class TestSingleDecisionSL:
    @patch("decisionmemory.replay.engine.LLMClient")
...
class TestMemoryStorage:
    @patch("decisionmemory.replay.engine.LLMClient")
    @patch("decisionmemory.db.Database")
    def test_closed_decision_stored_to_db(self, MockDatabase, MockLLMClient):
        """Closed decisions should be stored via db.insert_episodic()."""
        call_count = [0]
...
class TestScoreTracking:
    @patch("decisionmemory.replay.engine.LLMClient")
    def test_score_changes_after_decision(self, MockLLMClient):
        """Score should change after a decision closes."""
        call_count = [0]
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            config = ReplayConfig(
                ...
                window_size=96,
                decision_interval=4,
                initial_score=10000.0,
                store_to_memory=False,
            )
            engine = ReplayEngine(config)
            summary = engine.run()

            # Score should differ from initial after decision
            if summary["decisions"] > 0:
                assert summary["score"] != 10000.0
...
class TestMaxOnePosition:
    ...
    def test_second_buy_ignored_while_position_open(self, MockLLMClient):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Multiple BUY calls but only 1 position opened (closed at EOD = 1 decision)
