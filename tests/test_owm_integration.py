...
DECISION_3_WIN = {
    ...
    "confidence": 0.9,
    "reflection": "Momentum decisions in trend work well",
    "decision_id": "owm-test-003",
}


class TestOWMRemember:
    """Test POST /owm/remember stores decisions correctly."""

    def test_remember_single_decision(self, owm_client):
        resp = owm_client.post("/owm/remember", json=DECISION_1_WIN)
    ...
    def test_remember_invalid_direction(self, owm_client):
        bad = {**DECISION_1_WIN, "direction": "sideways", "decision_id": "owm-bad-dir"}
        resp = owm_client.post("/owm/remember", json=bad)
        assert resp.status_code == 400

    def test_remember_three_decisions(self, owm_client):
        for decision in [DECISION_1_WIN, DECISION_2_LOSS, DECISION_3_WIN]:
            resp = owm_client.post("/owm/remember", json=decision)
            assert resp.status_code == 200
...
class TestOWMRecall:
    ...
    def _store_three_decisions(self, client):
        for decision in [DECISION_1_WIN, DECISION_2_LOSS, DECISION_3_WIN]:
            resp = client.post("/owm/remember", json=decision)
            assert resp.status_code == 200

    def test_recall_after_storing(self, owm_client):
        self._store_three_decisions(owm_client)
    ...
    def test_recall_scoring_order(self, owm_client):
        ...
        if len(episodic_memories) >= 2:
            ...
            # The losing decision should score lower than the best winner
...
class TestOWMBehavioral:
    ...
    def test_behavioral_with_data(self, owm_client):
        ...
        # Should have entries for strategies that had decisions
...
class TestOWMState:
    ...
    def test_state_after_decisions(self, owm_client):
        """After 3 decisions (W, L, W), affective state should be updated."""
        self._store_three_decisions(owm_client)
        ...
        # Net P&L = +500 -300 +750 = +950, so current_score > 10000
        assert data["current_score"] > 10000.0

        # Last decision was a win, so consecutive_wins >= 1
        assert data["consecutive_wins"] >= 1
        assert data["consecutive_losses"] == 0

        # Positive score → drawdown should be 0 or near 0
...
class TestOWMPlan:
    ...
    def test_create_plan(self, owm_client):
        resp = owm_client.post("/owm/plan", json={
            ...
            "planned_action": '{"type": "skip_decision", "reason": "Low edge in ranging market"}',
...
class TestOWMFullPipeline:
    ...
    def test_full_pipeline(self, owm_client):
        ...
        # Step 4: Agent state — should reflect 3 decisions
        resp = owm_client.get("/owm/state")
        assert resp.status_code == 200
        state_data = resp.json()
        assert state_data["status"] == "ok"
        # Net PnL = +500 -300 +750 = +950
        assert state_data["current_score"] > 10000.0
        assert state_data["consecutive_wins"] >= 1

        # Step 5: Create a decision_making plan
        resp = owm_client.post("/owm/plan", json={
            "trigger_type": "market_condition",
            "trigger_condition": '{"regime": "ranging"}',
