...
2. 用 GitHub 登入
3. 選擇 repo：`mnemox-ai/decisionmemory-protocol`
...
# 方法 A：從本地 repo 發布
smithery mcp publish --name @mnemox-ai/decisionmemory-protocol --transport stdio

# 方法 B：如果 CLI 支援 --api-key
smithery mcp publish --name @mnemox-ai/decisionmemory-protocol --transport stdio --api-key 9e8fbb37-1374-444a-a771-0b6b20084da3
```

### 驗證上架

```bash
smithery mcp search decisionmemory
```
...
curl -X POST https://registry.smithery.ai/servers \
  ...
  -d '{
    "name": "@mnemox-ai/decisionmemory-protocol",
    "description": "MCP memory for AI decision agents. Store decisions, recall similar setups, track strategy performance.",
...
- [x] `pyproject.toml` — 包含 `decisionmemory-protocol` 入口點
- [x] GitHub repo public — `mnemox-ai/decisionmemory-protocol`
...
2. 使用者安裝時需要先 `pip install decisionmemory-protocol`，Smithery 會在頁面顯示安裝指令
3. 如果 Smithery CLI 指令格式有變，查最新文檔：https://smithery.ai/docs/build/publish
4. 上架後可在 https://smithery.ai/server/@mnemox-ai/decisionmemory-protocol 查看頁面
...
2. 填寫表單：
   - **MCP Name**: DecisionMemory Protocol
   - **Type**: Server
   - **GitHub URL**: https://github.com/mnemox-ai/decisionmemory-protocol
   - **Description**: MCP memory layer for AI decision agents. 5-type outcome-weighted memory, evolution engine, MT5/Binance integration.
3. 提交
...
- [x] DecisionMemory 已在 Decisions & Fintech section（之前的 PR 已 merge）
- [x] 更新描述到 v0.5.0：5-type OWM + evolution engine
- [x] PR: https://github.com/punkpeye/awesome-mcp-servers/pull/3302

## 描述更新

- **舊**: Structured 3-layer memory system (decisions → patterns → strategy) for AI decision agents. Supports MT5, Binance, and Alpaca.
- **新**: MCP memory layer for AI decision agents. 5-type outcome-weighted memory, evolution engine, MT5/Binance integration.
