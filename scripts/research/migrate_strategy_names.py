"""
One-time migration: Fix strategy names in decisionmemory.db using MT5 magic numbers.

Existing decisions were all stored as strategy="NG_Gold". This script:
1. Queries MT5 for historical deals to get magic numbers
2. Maps magic → strategy name
3. Updates decision_records in decisionmemory.db

Usage:
    python scripts/research/migrate_strategy_names.py [--dry-run]

Requires: MetaDecisionMaker5 package + MT5 terminal running
"""
...
DB_PATH = os.getenv("DECISIONMEMORY_DB", "data/decisionmemory.db")
...
def get_mt5_magic_map() -> dict:
    ...
    try:
        ...
        print("[ERROR] MetaDecisionMaker5 not installed")
        sys.exit(1)
...
def migrate(dry_run: bool = False):
    ...
    print(f"\nFound {len(rows)} decisions in DB:")
    print("-" * 80)
    ...
    for row in rows:
        ...
        # Extract position_id from decision_id (format: MT5-{position_id})
        if not decision_id.startswith("MT5-"):
            print(f"  {decision_id}: SKIP (not MT5 decision)")
            continue

        try:
            position_id = int(decision_id.split("-")[1])
        except (IndexError, ValueError):
            print(f"  {decision_id}: SKIP (bad format)")
            continue

        magic = magic_map.get(position_id)
        if magic is None:
            print(f"  {decision_id}: SKIP (position {position_id} not found in MT5 history)")
            continue
        ...
        print(
            f"  {decision_id}: magic={magic} | "
            f"{current_strategy} → {new_strategy} [{status}]"
        )

        if current_strategy != new_strategy or "magic" not in row["reasoning"]:
            updates.append((new_strategy, new_reasoning, new_ctx, decision_id))

    print(f"\n{len(updates)} decisions need updating.")
    ...
    for new_strategy, new_reasoning, new_ctx, decision_id in updates:
        conn.execute(
            "UPDATE decision_records SET strategy=?, reasoning=?, market_context=? WHERE id=?",
            (new_strategy, new_reasoning, new_ctx, decision_id),
        )
    conn.commit()
    conn.close()

    print(f"\n[OK] Updated {len(updates)} decisions successfully.")
