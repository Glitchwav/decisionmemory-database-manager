"""
Daily DecisionMaking Monitor — Option C (AI-Assisted)

Reads decisionmemory DB + MT5 open positions to detect anomalies.
...
DB_PATH = PROJECT_ROOT / "data" / "decisionmemory.db"
...
def detect_anomalies(decisions, open_positions, balance):
    """Detect decision_making anomalies based on rules."""
    alerts = []

    # Group closed decisions by strategy
    strategy_decisions = defaultdict(list)
    for t in decisions:
        strategy_decisions[t["strategy"]].append(t)

    # 1. Consecutive losses per strategy
    for strategy, sdecisions in strategy_decisions.items():
        sorted_decisions = sorted(sdecisions, key=lambda x: x.get("timestamp", ""))
        consecutive_losses = 0
        for t in reversed(sorted_decisions):
    ...
    # 2. Drawdown check
    total_pnl = sum(t.get("pnl", 0) or 0 for t in decisions)
    ...
    # 3. Strategy silence — not decisiond in X days
    deployed = ["VolBreakout", "IntradayMomentum", "Pullback"]
    for s in deployed:
        sdecisions = strategy_decisions.get(s, [])
        if len(sdecisions) == 0:
            alerts.append({
                "level": "MEDIUM",
                ...
                "strategy": s,
                "days": 7,
                "message": f"{s}: ZERO decisions in the monitoring period",
            })
        else:
            last_decision_time = max(t.get("timestamp", "") for t in sdecisions)
            if last_decision_time:
                parsed = datetime.fromisoformat(last_decision_time.replace("+00:00", "").replace("Z", ""))
                ...
                if days_since > 5:
                    alerts.append({
                        ...
                        "strategy": s,
                        "days": days_since,
                        "message": f"{s}: No decisions in {days_since} days",
                    })
    ...
    for strategy, sdecisions in strategy_decisions.items():
        if strategy not in BACKTEST_BASELINE:
            continue
        closed = [t for t in sdecisions if t.get("pnl") is not None]
        if len(closed) >= 5:
...
def generate_report(decisions, open_positions, balance, alerts):
    ...
    for t in decisions:
        ...
        if s not in strategy_stats:
            strategy_stats[s] = {"decisions": 0, "wins": 0, "total_pnl": 0}
        strategy_stats[s]["decisions"] += 1
...
def format_report_text(report):
    ...
    lines = []
    lines.append("=" * 60)
    lines.append(f"  Daily DecisionMaking Monitor — {report['generated_at'][:10]}")
    lines.append("=" * 60)
    ...
    lines.append(f"  {'Strategy':<20} {'Decisions':>7} {'Wins':>6} {'WR':>6} {'PnL':>12}")
    ...
    for s in deployed:
        if s in report["strategy_summary"]:
            ...
            wr = f"{st['wins']/st['decisions']*100:.0f}%" if st["decisions"] > 0 else "N/A"
            lines.append(f"  {s:<20} {st['decisions']:>7} {st['wins']:>6} {wr:>6} ${st['total_pnl']:>10,.2f}")
...
def main():
    parser = argparse.ArgumentParser(description="Daily DecisionMaking Monitor")
    ...
    print("Reading decisionmemory DB...")
    decisions = get_db_decisions(days_back=args.days)
    print(f"  Found {len(decisions)} decisions in last {args.days} days")
    ...
    alerts = detect_anomalies(decisions, open_positions, balance)
    print(f"  Found {len(alerts)} alerts")

    print("Generating report...")
    report = generate_report(decisions, open_positions, balance, alerts)
