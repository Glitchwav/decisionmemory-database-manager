...
Experiment matrix:
- N symbols x K timeframes x M strategies (from grid, filtered to 30+ IS decisions)
...
# Suppress noisy logs
logging.getLogger("decisionmemory.owm_helpers").setLevel(logging.WARNING)
logging.getLogger("decisionmemory.owm.changepoint").setLevel(logging.WARNING)
logging.getLogger("decisionmemory.owm.dqs").setLevel(logging.WARNING)
...
def compute_dsr_safe(sharpe: float, n_decisions: int) -> Dict[str, Any]:
    """Compute DSR, return dict with verdict."""
    try:
        from decisionmemory.strategy_validator import compute_dsr
        if n_decisions >= 30:
            return compute_dsr(sharpe_raw=sharpe, num_obs=n_decisions, num_trials=1)
...
def filter_strategies_is(
    ...
    """Pre-filter: run BaseAgent IS on all strategies, keep those with 30+ decisions.

    Returns list of (strategy, is_result) tuples for qualifying strategies.
    This avoids re-running IS for each of the 5 agents.
    """
    from decisionmemory.simulation.agent import BaseAgent
    from decisionmemory.simulation.simulator import Simulator
    ...
    for strategy in strategies:
        ...
        result = sim.run()
        if result.fitness.decision_count >= min_decisions:
...
def run_single_experiment(
    ...
    Args:
        ...
        is_result: Pre-computed BaseAgent IS result (for warm-start + decision count)
        oos_series: OOS data split
        symbol: DecisionMaking pair name
    ...
    """
    from decisionmemory.simulation.agent import BaseAgent, CalibratedAgent
    from decisionmemory.simulation.baselines import (
        PeriodicReduceAgent,
        RandomSkipAgent,
        SimpleWRAgent,
    )
    ...
    is_decisions = is_result.decisions
    ...
    _warm_start(agent_b, is_decisions, symbol)
    ...
    for t in is_decisions:
        agent_e.on_decision_complete(t)
    agent_e.decisions = []
    ...
    return {
        ...
        "is_decisions": is_result.fitness.decision_count,
        "agents": {
...
def _extract_metrics(result) -> Dict[str, Any]:
    ...
    return {
        "decisions": result.fitness.decision_count,
        ...
        "score_total_pnl": round(result.score_total_pnl, 4),
        "score_max_dd": round(result.score_max_dd, 4),
        "score_calmar": round(result.score_calmar, 4),
        "skipped": getattr(result, 'skipped_signals', 0),
    }


def _warm_start(agent, is_decisions, symbol):
    """Warm-start CalibratedAgent with IS decisions."""
    agent._current_symbol = symbol
    for decision in is_decisions:
        agent.on_decision_complete(decision)

    # Compute DQS on IS decisions for adaptive thresholds
    is_dqs_scores = []
    for decision in is_decisions:
        try:
...
def run_sensitivity(
    ...
    base_dd = result_a_oos.score_max_dd

    for hz in hazard_values:
        try:
            agent_b = CalibratedAgent(strategy, fixed_lot=0.01, hazard_lambda=float(hz))
            _warm_start(agent_b, result_a_is.decisions, series.symbol)
            result_b = Simulator(agent_b, oos_series, timeframe_str).run()

            dd_reduction = (base_dd - result_b.score_max_dd) / max(base_dd, 0.01)

            results.append({
                "hazard_rate": hz,
                "score_max_dd": round(result_b.score_max_dd, 4),
                "dd_reduction": round(dd_reduction, 4),
                "decisions": result_b.fitness.decision_count,
...
def generate_report(
    ...
    cal_vs_base_wins = sum(
        ...
        if r["agents"]["calibrated"]["score_max_dd"] < r["agents"]["base"]["score_max_dd"]
    )
    ...
    lines.append("")
    lines.append(f"- **Strategies**: Parameter grid ({n_total} passed IS filter of 30+ decisions)")
    lines.append(f"- **Symbols**: {', '.join(symbols)}")
    lines.append(f"- **Timeframes**: {', '.join(timeframes)}")
    lines.append(f"- **Walk-forward**: 67% IS / 33% OOS")
    lines.append(f"- **Warm-start**: CalibratedAgent seeded with IS decisions + adaptive DQS thresholds")
    ...
    # --- 3.5 Decision Activity Analysis ---
    lines.append("### 3.5 Decision Activity (Critical)")
    lines.append("")
    cal_decisions = [r["agents"]["calibrated"]["decisions"] for r in successful]
    base_decisions = [r["agents"]["base"]["decisions"] for r in successful]
    cal_skipped = [r["agents"]["calibrated"]["skipped"] for r in successful]
    zero_decision_count = sum(1 for t in cal_decisions if t == 0)
    lines.append(f"- BaseAgent mean decisions: {_mean(base_decisions):.1f}")
    lines.append(f"- CalibratedAgent mean decisions: {_mean(cal_decisions):.1f}")
    lines.append(f"- CalibratedAgent mean skipped signals: {_mean(cal_skipped):.1f}")
    lines.append(f"- CalibratedAgent with 0 OOS decisions: {zero_decision_count}/{n_total}")
    lines.append("")
    if zero_decision_count > n_total * 0.3:
        lines.append("**WARNING**: CalibratedAgent executes very few decisions. DD reduction is largely from")
        lines.append("NOT DECISION_MAKING rather than intelligent calibration. DQS skip tier is too aggressive")
        lines.append("on cold-start / small DB, causing near-total decision rejection in OOS.")
    ...
    for sym in symbols:
        for tf in timeframes:
            ...
            cal_dd = _mean([r["agents"]["calibrated"]["score_max_dd"] for r in subset])
            base_dd = _mean([r["agents"]["base"]["score_max_dd"] for r in subset])
            lines.append(
    ...
    # Check if the "win" is just from not decision_making
    cal_decisions = [r["agents"]["calibrated"]["decisions"] for r in successful]
    zero_pct = sum(1 for t in cal_decisions if t == 0) / max(n_total, 1)
    mean_cal_decisions = _mean(cal_decisions) if cal_decisions else 0
    base_decisions = [r["agents"]["base"]["decisions"] for r in successful]
    mean_base_decisions = _mean(base_decisions) if base_decisions else 1
    decision_ratio = mean_cal_decisions / max(mean_base_decisions, 1)

    if beats_base and beats_naive and zero_pct < 0.3 and decision_ratio > 0.3:
        ...
        verdict = (
            "INVALID -- CalibratedAgent reduces DD by NOT DECISION_MAKING "
            f"({zero_pct:.0%} zero-decision experiments, decision ratio {decision_ratio:.1%}). "
...
async def main():
    ...
    from decisionmemory.data.models import Timeframe
    from decisionmemory.simulation.strategy_generator import generate_strategy_grid
    ...
    for symbol in symbols:
        for tf_str in timeframes:
            ...
            print(f"  Filtering {len(all_strategies)} strategies (IS >=30 decisions)...")
            t0 = time.time()
            qualified_pairs = filter_strategies_is(
                all_strategies, is_series, tf_str, min_decisions=30,
            )
            ...
            for q_idx, (strategy, is_result) in enumerate(qualified_pairs):
                ...
                # Progress
                cal_dd = result["agents"]["calibrated"]["score_max_dd"]
                base_dd = result["agents"]["base"]["score_max_dd"]
    ...
    dd_base = [r["agents"]["base"]["score_max_dd"] for r in all_results]
    dd_cal = [r["agents"]["calibrated"]["score_max_dd"] for r in all_results]
    dd_periodic = [r["agents"]["periodic_reduce"]["score_max_dd"] for r in all_results]
    dd_random = [r["agents"]["random_skip"]["score_max_dd"] for r in all_results]
    dd_wr = [r["agents"]["simple_wr"]["score_max_dd"] for r in all_results]
    ...
    for r in all_results:
        ...
        dsr = compute_dsr_safe(cal["sharpe"], cal["decisions"])
    ...
    dd_reductions = [
        (i, (r["agents"]["base"]["score_max_dd"] - r["agents"]["calibrated"]["score_max_dd"]))
