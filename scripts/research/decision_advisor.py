"""
Decision Advisor — Event-driven memory recall + behavioral check.

NOT a scheduled report. Only speaks when it has something worth saying.

Called by mt5_sync.py when a new position is OPENED.
Queries DecisionMemory for similar past decisions and behavioral warnings.
...
# If past similar decisions have avg PnL below this, warn
LOSS_PATTERN_THRESHOLD = -50.0

# Minimum number of SAME-STRATEGY decisions to make a judgment
# Below this → stay silent on recall-based warnings (not enough data)
MIN_SIMILAR_DECISIONS = 5
...
def recall_similar(symbol: str, strategy: str, session: str) -> List[Dict]:
    """Query OWM for similar past decisions — filtered by SAME strategy."""
    try:
        resp = requests.post(
            f"{DECISIONMEMORY_API}/owm/recall",
            json={
                "symbol": symbol,
...
def get_behavioral() -> Optional[Dict]:
    ...
    try:
        resp = requests.get(f"{DECISIONMEMORY_API}/owm/behavioral", timeout=10)
        if resp.status_code == 200:
            return resp.json()
    except Exception:
        pass
...
def get_agent_state() -> Optional[Dict]:
    ...
    try:
        resp = requests.get(f"{DECISIONMEMORY_API}/owm/state", timeout=10)
        if resp.status_code == 200:
            return resp.json()
    except Exception:
        pass
...
def advise_on_open(
    ...
    if len(relevant) >= MIN_SIMILAR_DECISIONS:
        ...
        if pnls:
            ...
            if avg_pnl < LOSS_PATTERN_THRESHOLD:
                worst = min(pnls)
                warnings.append(
                    f"**{strategy} past decisions averaged ${avg_pnl:+.0f}** "
            ...
            if win_rate < 0.35 and len(pnls) >= 3:
                warnings.append(
                    f"{strategy} win rate in similar setups: **{win_rate:.0%}** ({len(pnls)} decisions)"
                )
                has_recall_warning = True
    ...
    if behavioral:
        # Revenge decision_making detection: new decision within 30 min of a loss
    ...
    if state:
        ...
        if action == "stop_decision_making":
            warnings.append(
                f"**Agent recommends STOP DECISION_MAKING** "
            ...
            warnings.append(
                f"**{consec_losses} consecutive losses** — are you revenge decision_making?"
            )
