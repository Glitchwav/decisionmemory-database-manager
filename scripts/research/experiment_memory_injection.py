#!/usr/bin/env python3
"""Autoresearch-style autonomous experiment: Memory-Enhanced DecisionMaking.
...
def should_skip_decision(memories: list, hour_utc: int, trend_12h_pct: float,
                      regime: str, atr_h1: float, cfg: MemoryConfig) -> tuple[bool, str]:
    """Decision: skip this decision based on memory recall?
...
def evaluate_with_memory(baseline_decisions, test_contexts, test_atrs, memories,
                         cfg: MemoryConfig, atr_period: int):
    """Run memory filter on baseline decisions, return metrics."""
    offset = atr_period + 1
    skipped_bars = set()
    skipped_winners = 0
    skipped_losers = 0
    ...
    skipped_list = [t for t in baseline_decisions if t.entry_bar in skipped_bars]
...
async def main():
    ...
    print("=" * 70)
    print("  AUTORESEARCH: Memory-Enhanced DecisionMaking")
    ...
    try:
        ...
        train_fitness, train_decisions = fast_backtest_with_decisions(
            train_data.bars, train_contexts, train_atrs, strategy,
            config=CONTEXT_CONFIG, timeframe="1h"
        )
        print(f"  Training: {train_fitness.decision_count} decisions | "
        ...
        baseline_fitness, baseline_decisions = fast_backtest_with_decisions(
            test_data.bars, test_contexts, test_atrs, strategy,
            config=CONTEXT_CONFIG, timeframe="1h"
        )
        print(f"  Baseline: {baseline_fitness.decision_count} decisions | "
        ...
        if not baseline_decisions:
            print("\n  FAIL No baseline decisions — nothing to filter. Experiment aborted.")
            return
        ...
        for i in range(args.iterations):
            ...
            (n_kept, wr, pf, pnl, sharpe, dd,
             n_skipped, skip_pnl, skip_w, skip_l) = evaluate_with_memory(
                baseline_decisions, test_contexts, test_atrs, memories,
            ...
            result = ExperimentResult(
                iteration=i + 1,
                config=cfg,
                decisions=baseline_fitness.decision_count,
                kept_decisions=n_kept,
                skipped=n_skipped,
                win_rate=wr,
                profit_factor=pf,
                total_pnl=pnl,
            ...
            print(f"  [{i+1:2d}] {marker} Sharpe {sharpe:+6.2f} | "
                  ...
                  f"Skip {n_skipped}/{baseline_fitness.decision_count} "
        ...
        print(f"\n  Baseline (no memory):")
        print(f"    Decisions: {baseline_fitness.decision_count} | WR: {baseline_fitness.win_rate:.1%} | "
        ...
        if agent.best_result:
            ...
            print(f"    Decisions: {b.kept_decisions} | WR: {b.win_rate:.1%} | "
                  f"PF: {b.profit_factor:.2f} | Sharpe: {b.sharpe:.2f} | "
                  f"PnL: ${b.total_pnl:.2f}")
            print(f"    Skipped: {b.skipped} decisions (PnL ${b.skipped_pnl:+.2f}, "
            ...
            if sharpe_delta >= 0.5 and wr_delta >= -0.05 and b.skipped_losers > b.skipped_winners:
                print(f"\n  PASS HYPOTHESIS VALIDATED: Memory injection improves decision_making")
            elif sharpe_delta > 0:
