...
Runs the L1 → L2 → L3 pipeline with simulated decisions and saves
...
# Same simulated decisions as demo.py (subset for concise output)
DECISIONS = [
...
def run_pipeline():
    ...
    # --- L1: Record decisions ---
    lines = []
    lines.append("== Step 1: L1 - Recording decisions to DecisionJournal ==\n")
    ...
    for i, t in enumerate(DECISIONS, 1):
        ...
        journal.record_decision(
            ...
            reasoning=f"{t['session']} session decision",
    ...
    winners = sum(1 for t in DECISIONS if t["pnl"] > 0)
    total_pnl = sum(t["pnl"] for t in DECISIONS)
    wr = winners / len(DECISIONS) * 100
    lines.append(f"\n  Total: {len(DECISIONS)} decisions | Winners: {winners} | Win rate: {wr:.0f}% | Net P&L: ${total_pnl:+.2f}")
    sections["l1"] = "\n".join(lines)
    ...
    high_conf = [t for t in DECISIONS if t["confidence"] >= 0.75]
    low_conf = [t for t in DECISIONS if t["confidence"] < 0.55]
    ...
    adjustments = [
        ...
        ("min_confidence_threshold", "0.40", "0.55", f"Decisions below 0.55 have {low_wr:.0f}% WR"),
    ]
