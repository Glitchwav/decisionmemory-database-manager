...
from decisionmemory.evolution.llm import AnthropicClient, LLMError
...
def print_hypothesis(h: Hypothesis, rank: int) -> None:
    ...
    print(f"     Decisions: {f.decision_count}  Win: {f.win_rate:.1%}  "
...
async def main(
    ...
    config = EngineConfig(
        ...
        selection=SelectionConfig(
            top_n=10,
            min_is_decision_count=3,
            min_is_sharpe=-5.0,
            min_oos_decision_count=2,
            min_oos_sharpe=-5.0,
    ...
    print(f"  Selection: moderate thresholds (min 3 IS decisions, 2 OOS decisions)")
