...
from decisionmemory.data.context_builder import ContextConfig, MarketContext, build_context, compute_atr
...
async def main():
    ...
    for name, pattern in strategies.items():
        ...
        print(f"  {name}: Sharpe={metrics.sharpe_ratio:.4f}, "
              f"decisions={metrics.decision_count}, WR={metrics.win_rate:.1%}, "
    ...
    print(f"\n{'Strategy':<30} {'Sharpe':>8} {'Pctile':>8} {'Decisions':>8} "
          f"{'WR':>8} {'Pass?':>8}")
    print("-" * 70)
    for name, rank in rankings.items():
        ...
        print(f"{name:<30} {rank['sharpe']:>8.4f} {rank['percentile']:>7.1f}% "
              f"{r['decisions']:>8d} {r['win_rate']:>7.1%} {status:>8}")
