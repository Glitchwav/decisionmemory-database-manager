...
Core question: Memory skipped 36/48 decisions and got Sharpe 9.57.
But what if we randomly skip 36/48 decisions 1000 times?
...
def memory_skip_indices(decisions, contexts, atrs, memories, top_k=7):
    """Return set of decision indices that memory-based filter would skip."""
    skip = set()
    for idx, t in enumerate(decisions):
...
def compute_sharpe(decisions, skip_indices):
    """Compute Sharpe for decisions NOT in skip_indices."""
    kept = [t for i, t in enumerate(decisions) if i not in skip_indices]
    if len(kept) < 2:
        return 0.0, 0.0, 0.0
...
async def main():
    ...
    print("  Question: Is memory better than randomly skipping the same # decisions?")
    print("=" * 70)
    ...
    try:
        for label, train_s, train_e, test_s, test_e in WINDOWS:
            ...
            print(f"  Baseline: {n_decisions} decisions | Sharpe {base_sharpe:.2f} | "
                  f"PnL ${base_fit.total_pnl:.2f}")

            if n_decisions < 5:
                print("  (too few decisions, skipping)")
                all_results.append({"window": label, "skip": True})
                continue

            # Memory-based skip
            mem_skip = memory_skip_indices(base_decisions, test_ctx, test_atrs, memories)
            n_skip = len(mem_skip)
            mem_sharpe, mem_pnl, mem_wr = compute_sharpe(base_decisions, mem_skip)
            print(f"  Memory filter: skip {n_skip}/{n_decisions} | "
                  f"Sharpe {mem_sharpe:.2f} | PnL ${mem_pnl:.2f} | WR {mem_wr:.0%}")

            if n_skip == 0:
                print("  (memory skips 0 decisions, nothing to permute)")
                all_results.append({
                    "window": label, "skip": False, "n_decisions": n_decisions,
            ...
            # ── Permutation test ──
            print(f"\n  Running {N_PERMUTATIONS} random permutations (skip {n_skip}/{n_decisions})...")
            random_sharpes = []
            random_pnls = []
            decision_indices = list(range(n_decisions))

            for p in range(N_PERMUTATIONS):
                random_skip = set(random.sample(decision_indices, n_skip))
                r_sharpe, r_pnl, _ = compute_sharpe(base_decisions, random_skip)
    ...
    for r in all_results:
        ...
        print(f"  {r['window']:<35} {r['n_skip']:>3}/{r['n_decisions']:<2} "
    ...
    if sig_count == testable and testable > 0:
        ...
        print(f"     The Sharpe improvement comes from skipping decisions (lower variance),")
        print(f"     not from memory's ability to identify bad decisions.")
