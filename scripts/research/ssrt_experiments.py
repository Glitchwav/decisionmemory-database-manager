...
from decisionmemory.ssrt.simulator import DecaySimulator
from decisionmemory.ssrt.baselines import MaxDDBaseline, RollingSharpeBaseline, CUSUMBaseline
...
class ExperimentResult:
    ...
    method: str
    detected: bool
    detection_decision: int | None
    true_decay_decision: int | None
...
def generate_decisions(scenario: dict, seed: int) -> list[DecisionResult]:
    """Generate decision sequence for a scenario."""
    method_name = scenario["method"]
    params = dict(scenario["params"])
    params["n_decisions"] = scenario["n"]
    params["seed"] = seed
...
def run_msprt(
    ...
    """Run mSPRT on decisions. Returns (detected, detection_decision, final_pnl)."""
    msprt = MixtureSPRT(alpha=0.05, tau=tau, sigma=sigma, null_mean=null_mean, burn_in=20)
    regime_null = RegimeAwareNull(min_decisions_per_regime=10) if use_regime else None

    cum_pnl = 0.0
    prev_regime = None

    for i, decision in enumerate(decisions):
        cum_pnl += decision.pnl_r

        if regime_null is not None:
            regime_null.update(decision)
            rn_mean, rn_sigma = regime_null.get_null(decision.regime)
            # Reset mSPRT if regime changed
            if decision.regime != prev_regime and prev_regime is not None:
                msprt.reset(null_mean=rn_mean, sigma=rn_sigma)
            prev_regime = decision.regime

        verdict = msprt.update(decision.pnl_r)
...
def run_msprt_shift(
    ...
    msprt = MixtureSPRT(alpha=0.05, tau=tau, sigma=sigma, null_mean=null_mean, burn_in=20)
    regime_null = RegimeAwareNull(min_decisions_per_regime=10)
...
def run_baseline(decisions: list[DecisionResult], baseline) -> tuple[bool, int | None, float]:
    """Run a baseline method. Returns (detected, detection_decision, final_pnl)."""
    baseline.reset()
    cum_pnl = 0.0
    for i, decision in enumerate(decisions):
        cum_pnl += decision.pnl_r
        result = baseline.update(decision)
        if result == "RETIRE":
            return True, i + 1, cum_pnl
    return False, None, cum_pnl


def compute_full_pnl(decisions: list[DecisionResult]) -> float:
    """Cumulative P&L for full sequence."""
    return sum(t.pnl_r for t in decisions)
...
def run_experiments() -> list[ExperimentResult]:
    ...
    for scenario_name, scenario_cfg in SCENARIOS.items():
        ...
        methods = {
            "mSPRT":              lambda decisions, pm=pre_mean, s=std: run_msprt(decisions, use_regime=False, null_mean=pm, sigma=s, tau=1.0),
            "mSPRT_t05":          lambda decisions, pm=pre_mean, s=std: run_msprt(decisions, use_regime=False, null_mean=pm, sigma=s, tau=0.5),
            "mSPRT_t03":          lambda decisions, pm=pre_mean, s=std: run_msprt(decisions, use_regime=False, null_mean=pm, sigma=s, tau=0.3),
            "mSPRT_regime_reset": lambda decisions, pm=pre_mean, s=std: run_msprt(decisions, use_regime=True, null_mean=pm, sigma=s, tau=1.0),
            "mSPRT_regime_shift": lambda decisions, pm=pre_mean, s=std: run_msprt_shift(decisions, null_mean=pm, sigma=s, tau=1.0),
            "MaxDD_5R":           lambda decisions: run_baseline(decisions, MaxDDBaseline(threshold_r=5.0)),
            "MaxDD_8R":           lambda decisions: run_baseline(decisions, MaxDDBaseline(threshold_r=8.0)),
            "RollingSharpe":      lambda decisions: run_baseline(decisions, RollingSharpeBaseline(window=30, consecutive=3)),
            "CUSUM":              lambda decisions: run_baseline(decisions, CUSUMBaseline(threshold=4.0, target_wr=0.5)),
        }
        ...
        for method_name, method_fn in methods.items():
            ...
            for sim_idx in range(N_SIMULATIONS):
                decisions = generate_decisions(scenario_cfg, seed=sim_idx)
                full_pnl = compute_full_pnl(decisions)

                detected, detection_decision, final_pnl = method_fn(decisions)

                detection_delay = None
                if detected and true_decay is not None and detection_decision is not None:
                    detection_delay = detection_decision - true_decay
                ...
                results.append(ExperimentResult(
                    ...
                    true_decay_decision=true_decay,
...
def aggregate_results(results: list[ExperimentResult]) -> dict:
    ...
    for (scenario, method), group in sorted(groups.items()):
        n = len(group)
        has_decay = group[0].true_decay_decision is not None
