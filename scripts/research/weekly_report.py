"""
Weekly DecisionMaking Report — Option C (AI-Assisted)

Generates a comprehensive weekly performance report comparing
real decisions vs BATCH-001 backtest baselines.
...
def get_decisions_since(since_date):
...
def compute_strategy_metrics(decisions):
    """Compute per-strategy metrics from decision list."""
    by_strategy = defaultdict(list)
    for t in decisions:
        by_strategy[t["strategy"]].append(t)

    metrics = {}
    for strategy, sdecisions in by_strategy.items():
...
def generate_weekly_text(since_date, metrics, all_decisions):
    """Generate weekly report text."""
    lines = []
    lines.append("=" * 65)
    lines.append(f"  Weekly DecisionMaking Report")
    lines.append(f"  Period: {since_date.strftime('%Y-%m-%d')} → {datetime.now().strftime('%Y-%m-%d')}")
    lines.append("=" * 65)

    # Overall
    all_closed = [t for t in all_decisions if t.get("pnl") is not None]
    total_pnl = sum(t["pnl"] for t in all_closed)
    total_decisions = len(all_closed)
    lines.append(f"\nOverall: {total_decisions} decisions, PnL: ${total_pnl:,.2f}")

    # Per strategy with backtest comparison
    lines.append(f"\n{'Strategy':<20} {'Decisions':>7} {'WR':>7} {'PF':>7} {'PnL':>10} {'vs Backtest':>15}")
    ...
    for s in deployed:
        if s in metrics:
            ...
            drift = ""
            if m["total_decisions"] >= 5 and bt_wr > 0:
                ...
                if wr_diff < -0.10:
                    ...
                    drift = "On track"
            elif m["total_decisions"] == 0:
                drift = "NO DECISIONS"
            else:
                drift = f"n={m['total_decisions']} (low)"

            lines.append(f"{s:<20} {m['total_decisions']:>7} {wr_str:>7} {pf_str:>7} ${m['total_pnl']:>8,.2f} {drift:>15}")
        else:
            lines.append(f"{s:<20} {'0':>7} {'N/A':>7} {'N/A':>7} {'$0.00':>10} {'NO DECISIONS':>15}")

    # Other strategies
    for s, m in metrics.items():
        if s not in deployed:
            wr_str = f"{m['win_rate']*100:.0f}%"
            lines.append(f"{s:<20} {m['total_decisions']:>7} {wr_str:>7} {m['profit_factor']:>7.2f} ${m['total_pnl']:>8,.2f} {'(not tracked)':>15}")

    # Known issues
    lines.append("\n" + "-" * 65)
    lines.append("Known Issues:")
    if "IntradayMomentum" not in metrics or metrics.get("IntradayMomentum", {}).get("total_decisions", 0) == 0:
    ...
    if "Pullback" not in metrics or metrics.get("Pullback", {}).get("total_decisions", 0) == 0:
...
def main():
    parser = argparse.ArgumentParser(description="Weekly DecisionMaking Report")
    ...
    decisions = get_decisions_since(since_date)
    print(f"  Found {len(decisions)} decisions")

    metrics = compute_strategy_metrics(decisions)

    # Generate text report
    text = generate_weekly_text(since_date, metrics, decisions)
    print()
    print(text)
