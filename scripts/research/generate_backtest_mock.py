...
WARNING: Generated from backtest data (2024.01-2026.02, n=10,169). Not live decision_making results.
...
DISCLAIMER = (
    ...
    "Not live decision_making results."
)
...
def compute_pf(decisions: list[dict]) -> float:
    """Profit factor = gross profit / gross loss."""
    gross_profit = sum(t["pnl"] for t in decisions if t["pnl"] > 0)
    gross_loss = abs(sum(t["pnl"] for t in decisions if t["pnl"] < 0))
    if gross_loss == 0:
        return 999.0 if gross_profit > 0 else 0.0
    return round(gross_profit / gross_loss, 2)


def compute_wr(decisions: list[dict]) -> float:
    if not decisions:
        return 0.0
    wins = sum(1 for t in decisions if t["pnl"] > 0)
...
def gen_overview(decisions: list[dict], total_all: int) -> dict:
    total_pnl = round(sum(t["pnl"] for t in decisions), 2)
    wr = compute_wr(decisions)
    pf = compute_pf(decisions)

    # max drawdown from score curve (starting from initial balance)
    ...
    last_date = decisions[-1]["timestamp"] if decisions else None
    strat_names = sorted(set(STRATEGY_DISPLAY[t["strategy"]] for t in decisions))
    ...
    return {
        ...
        "win_rate": wr,
        "profit_factor": pf,
        "current_score": round(10000 + total_pnl, 2),
        "max_drawdown_pct": round(max_dd, 4),
        "memory_count": len(decisions),
        "avg_confidence": round(
            sum(t["confidence"] or 0.5 for t in decisions) / len(decisions), 2
        ),
...
def gen_score_curve(decisions: list[dict]) -> list[dict]:
    """Daily score curve with cumulative PnL and drawdown."""
    ...
    for day in sorted(daily.keys()):
        ...
        curve.append({
            ...
            "decision_count": sum(len(daily[d]) for d in sorted(daily.keys()) if d <= day),
        })
    return curve


def gen_strategy(
    strat_name: str, decisions: list[dict]
) -> dict:
    ...
    pnl_rs = [t["pnl_r"] for t in decisions if t["pnl_r"] is not None]
    avg_pnl_r = round(sum(pnl_rs) / len(pnl_rs), 2) if pnl_rs else round(
        sum(t["pnl"] for t in decisions) / len(decisions) / 100, 2
    )

    avg_hold = round(
        sum(t["hold_duration"] or 0 for t in decisions) / len(decisions)
    )
    ...
    # last 20 decisions for the decisions array
    recent = decisions[-20:]
    decision_list = []
    for t in recent:
...
def gen_rolling_metrics(decisions: list[dict], window: int = 20) -> list[dict]:
    """Rolling PF, WR, avg PnL over a sliding window."""
    if len(decisions) < window:
        return []

    # group decisions by date for daily snapshots
    daily_decisions: dict[str, list[dict]] = defaultdict(list)
    for t in decisions:
        daily_decisions[t["timestamp"][:10]].append(t)

    all_dates = sorted(daily_decisions.keys())
    result = []
    # accumulate decisions in order
    acc: list[dict] = []
    for day in all_dates:
        acc.extend(daily_decisions[day])
        if len(acc) >= window:
            w = acc[-window:]
            wr = compute_wr(w)
            pf = compute_pf(w)
...
def gen_memory_growth(decisions: list[dict]) -> list[dict]:
    """Simulated memory growth: episodic = cumulative decisions, semantic/procedural gradual."""
...
def gen_owm_score_trend(decisions: list[dict]) -> list[dict]:
...
def gen_confidence_cal(decisions: list[dict]) -> list[dict]:
    """Confidence calibration buckets. All backtest decisions have confidence=0.5."""
...
def gen_reflections(decisions: list[dict]) -> list[dict]:
    """Generate weekly reflections from decision data."""
    # Group by ISO week
    ...
    for i, (week, wt) in enumerate(sorted(weekly.items())):
        ...
        result.append({
            ...
            "summary": (
                f"Week of {week}: {len(wt)} decisions, "
...
def main():
    ...
    try:
        ...
        print(f"Loaded {len(decisions)} BUY decisions from 3 strategies (total in DB: {total_all})")

        files_written = 0

        # 1. overview.json
        overview = gen_overview(decisions, total_all)
        write_json(MOCK_DIR / "overview.json", overview)
        files_written += 1
        print(f"  overview.json: {overview['total_decisions']} total, PnL ${overview['total_pnl']:+,.2f}")

        # 2. score-curve.json
        eq = gen_score_curve(decisions)
        write_json(MOCK_DIR / "score-curve.json", eq)
        files_written += 1
        print(f"  score-curve.json: {len(eq)} daily points")

        # 3. strategy JSONs
        for strat in STRATEGIES:
            strat_decisions = [t for t in decisions if t["strategy"] == strat]
            slug = STRATEGY_SLUG[strat]
            data = gen_strategy(strat, strat_decisions)
            write_json(MOCK_DIR / f"strategy-{slug}.json", data)
            files_written += 1
            print(f"  strategy-{slug}.json: {data['total_decisions']} decisions, PF={data['profit_factor']}, WR={data['win_rate']:.1%}")

        # 4. rolling-metrics.json
        rolling = gen_rolling_metrics(decisions)
        write_json(MOCK_DIR / "rolling-metrics.json", rolling)
        files_written += 1
        print(f"  rolling-metrics.json: {len(rolling)} data points")

        # 5. memory-growth.json
        mem = gen_memory_growth(decisions)
        write_json(MOCK_DIR / "memory-growth.json", mem)
        files_written += 1
        print(f"  memory-growth.json: {len(mem)} data points")

        # 6. owm-score-trend.json
        owm = gen_owm_score_trend(decisions)
        write_json(MOCK_DIR / "owm-score-trend.json", owm)
        files_written += 1
        print(f"  owm-score-trend.json: {len(owm)} data points")

        # 7. confidence-cal.json
        cal = gen_confidence_cal(decisions)
        ...
        # 10. reflections.json
        refl = gen_reflections(decisions)
