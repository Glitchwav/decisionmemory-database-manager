...
- **Strategies**: Parameter grid (100 passed IS filter of 30+ decisions)
- **Symbols**: BTCUSDT, ETHUSDT
- **Timeframes**: 1h
- **Walk-forward**: 67% IS / 33% OOS
- **Warm-start**: CalibratedAgent seeded with IS decisions + adaptive DQS thresholds
...
### 3.5 Decision Activity (Critical)

- BaseAgent mean decisions: 136.6
- CalibratedAgent mean decisions: 4.4
- CalibratedAgent mean skipped signals: 660.4
- CalibratedAgent with 0 OOS decisions: 48/100

**WARNING**: CalibratedAgent executes very few decisions. DD reduction is largely from
NOT DECISION_MAKING rather than intelligent calibration. DQS skip tier is too aggressive
on cold-start / small DB, causing near-total decision rejection in OOS.
...
**Verdict: INVALID -- CalibratedAgent reduces DD by NOT DECISION_MAKING (48% zero-decision experiments, decision ratio 3.2%). DQS skip tier too aggressive. BOCPD changepoint effect unmeasurable.**
