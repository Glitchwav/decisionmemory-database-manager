...
from decisionmemory.backtest_importer import (
    parse_mt5_report,
    parse_variant_tag,
...
def parse_all_reports(report_dir: str) -> list:
    ...
    for report_path in report_files:
        ...
        # Calculate stats
        wins = [t for t in decisions if t['pnl'] > 0]
        losses = [t for t in decisions if t['pnl'] < 0]
...
def print_summary_table(results: list):
    ...
    for r in results:
        if r['empty']:
            ...
            continue
        print(f"{r['tag']:<40} {r['n_decisions']:>5} ${r['pnl']:>9.2f} {r['pnl_pct']:>6.1f}% {r['win_rate']:>5.1f} {r['profit_factor']:>6.2f} ${r['avg_win']:>7.2f} ${r['avg_loss']:>7.2f} {r['rr']:>5.2f} {r['max_dd_pct']:>5.1f}%")
...
def print_strategy_analysis(results: list):
    ...
    for s, variants in sorted(strategies.items()):
        active = [v for v in variants if not v['empty'] and v['n_decisions'] > 0]
        if not active:
            print(f"\n{s}: ALL EMPTY")
            continue
        ...
        print(f"  BEST:  {best['tag']} → {best['pnl_pct']:+.1f}% (n={best['n_decisions']}, WR={best['win_rate']}%, PF={best['profit_factor']})")
        print(f"  WORST: {worst['tag']} → {worst['pnl_pct']:+.1f}% (n={worst['n_decisions']}, WR={worst['win_rate']}%, PF={worst['profit_factor']})")
...
def print_symbol_analysis(results: list):
    ...
    for sym, variants in sorted(symbols.items()):
        ...
        print(f"\n{sym} ({len(active)} variants, {total_n} total decisions):")
...
def print_top_bottom(results: list, n=10):
    ...
    active = [r for r in results if not r['empty'] and r['n_decisions'] > 10]  # min 10 decisions
    ranked = sorted(active, key=lambda r: r['pnl_pct'], reverse=True)

    print(f"\n{'=' * 80}")
    print(f"=== TOP {n} VARIANTS (min 10 decisions) ===")
    print(f"{'=' * 80}")
    for r in ranked[:n]:
        print(f"  {r['pnl_pct']:>+7.1f}%  PF={r['profit_factor']:<5}  WR={r['win_rate']:<5}  RR={r['rr']:<5}  n={r['n_decisions']:<4}  {r['tag']}")

    print(f"\n{'=' * 80}")
    print(f"=== BOTTOM {n} VARIANTS (min 10 decisions) ===")
    print(f"{'=' * 80}")
    for r in ranked[-n:]:
        print(f"  {r['pnl_pct']:>+7.1f}%  PF={r['profit_factor']:<5}  WR={r['win_rate']:<5}  RR={r['rr']:<5}  n={r['n_decisions']:<4}  {r['tag']}")
...
def validate_ground_truth(results: list):
    ...
    if mr_buy:
        active_mr_buy = [r for r in mr_buy if not r['empty'] and r['n_decisions'] > 0]
        if active_mr_buy:
    ...
    if mr_both:
        active_mr_both = [r for r in mr_both if not r['empty'] and r['n_decisions'] > 0]
        if active_mr_both:
            ...
            if active_mr_buy:
                active_mr_buy2 = [r for r in mr_buy if not r['empty'] and r['n_decisions'] > 0]
    ...
    eurusd = [r for r in results if r['symbol'] == 'EURUSD' and not r['empty'] and r['n_decisions'] > 0]
    if eurusd:
    ...
    vb_xauusd = [r for r in results if r['strategy'] == 'VolBreakout' and r['symbol'] == 'XAUUSD' and not r['empty'] and r['n_decisions'] > 0]
    vb_eurusd = [r for r in results if r['strategy'] == 'VolBreakout' and r['symbol'] == 'EURUSD' and not r['empty'] and r['n_decisions'] > 0]
    ...
    for strat in ['VolBreakout', 'IntradayMomentum', 'MeanReversion']:
        buy_only = [r for r in results if r['strategy'] == strat and r['direction'] == 'BUY' and not r['empty'] and r['n_decisions'] > 0]
        both = [r for r in results if r['strategy'] == strat and r['direction'] == 'BOTH' and not r['empty'] and r['n_decisions'] > 0]
...
def export_json(results: list, output_path: str):
    """Export results as JSON for decisionmemory import."""
...
def main():
    ...
    print(f"Active (>0 decisions): {len(active)}")
    print(f"Empty (0 decisions): {len(empty)}")
    print(f"Profitable: {len(profitable)}/{len(active)} ({len(profitable)/len(active)*100:.0f}%)" if active else "No active variants")

    if active:
        total_decisions = sum(r['n_decisions'] for r in active)
        overall_avg_pnl = sum(r['pnl_pct'] for r in active) / len(active)
        print(f"Total decisions across all variants: {total_decisions}")
