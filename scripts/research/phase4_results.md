...
- **Statistical tests**: Welch's t-test on decision PnLs, DSR (Deflated Sharpe Ratio), Pearson correlation
...
| Symbol | TF | Strategy | Sharpe A | Sharpe B | £G Sharpe | DD A | DD B | £G DD | Decisions A | Decisions B | Skipped | Skip PnL | DQS-PnL r | p-value | Sig? |
|--------|-----|----------|----------|----------|---------|------|------|------|----------|----------|---------|----------|-----------|---------|------|
...
- Range: [-0.3461, 0.3156]
- Positive r indicates DQS score is predictive of decision outcome
...
- Total DQS-scored decisions: 2415
...
- Total decisions skipped by Agent B: 560
- Of those, losers in Agent A: 59
- Skip precision: **10.5%** (higher = DQS more accurate)
- Total PnL of skipped decisions (Agent A): 5533.1995

## 7. Limitations & Risks

- No slippage model: real-world execution would degrade results
- No transaction costs: spreads and commissions not modeled
- Crypto-only: results on BTCUSDT/ETHUSDT may not generalize to forex/subjects
- IS/OOS split is time-based (correct for decision data, but only one split point)
...
---
*Report generated by DecisionMemory Protocol v0.5.1 Simulation Framework*