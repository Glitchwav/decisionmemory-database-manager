#!/usr/bin/env python3
"""Analyze DeFi behavior fingerprint from whale wallet decision CSV.
...
Data source: CSV from fetch_whale_decisions.py (Etherscan V2 API).

Usage:
    cd C:/Users/johns/projects/decisionmemory-protocol
    python scripts/research/analyze_defi_behavior.py data/whale_0xb99a2c_decisions.csv
    python scripts/research/analyze_defi_behavior.py data/whale_0xb99a2c_decisions.csv --output scripts/research/output/abraxas_fingerprint.json
"""
...
def estimate_tx_usd(decision: dict) -> float | None:
    """Estimate USD value of a swap from the most reliable side.

    Priority: stablecoin side > ETH/BTC side > None.
    """
    dt = decision["timestamp"]
    sold_usd = estimate_usd_value(decision["sold_token"], decision["sold_amount"], dt)
    bought_usd = estimate_usd_value(decision["bought_token"], decision["bought_amount"], dt)

    # Prefer the stablecoin side (most accurate)
    sold_is_stable = decision["sold_token"] in (USD_PEGGED_TOKENS | AAVE_STABLECOIN_TOKENS)
    bought_is_stable = decision["bought_token"] in (USD_PEGGED_TOKENS | AAVE_STABLECOIN_TOKENS)
...
def load_decisions(csv_path: str) -> list[dict]:
    ...
    with open(csv_path, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            decision["usd_value"] = estimate_tx_usd(decision)
...
def analyze_fund_flow(decisions: list[dict]) -> dict:
...
def analyze_token_shifts(decisions: list[dict]) -> dict:
    ...
    # Token-level counts
    sold_tokens = Counter(t["sold_token"] for t in decisions)
    bought_tokens = Counter(t["bought_token"] for t in decisions)
...
def analyze_tempo(decisions: list[dict]) -> dict:
...
def analyze_time_preference(decisions: list[dict]) -> dict:
    """Analyze session and day-of-week preferences."""
    if not decisions:
        return {}

    hours = Counter(t["timestamp"].hour for t in decisions)
    weekdays = Counter(t["timestamp"].weekday() for t in decisions)
    total = len(decisions)
...
def analyze_gas_sensitivity(decisions: list[dict]) -> dict:
    """Analyze whether high gas periods reduce activity."""
    if not decisions:
        return {}

    gas_costs = [t["gas_cost_eth"] for t in decisions if t["gas_cost_eth"] > 0]
    if not gas_costs:
...
def analyze_usd_volume(decisions: list[dict]) -> dict:
    """Estimate total USD volume from token amounts and approximate prices."""
    valued_txs = [(t, t["usd_value"]) for t in decisions if t.get("usd_value") is not None]

    if not valued_txs:
        return {"note": "No USD-estimable transactions found", "coverage_pct": 0}

    usd_values = [v for _, v in valued_txs]
    coverage_pct = round(len(valued_txs) / len(decisions) * 100, 1)
...
def print_summary(report: dict):
    ...
    print(f"{'='*70}")
    print(f"  Total swaps: {meta['total_decisions']:,}")
    ...
    for flow, count in list(ts["top_transitions"].items())[:8]:
        pct = round(count / meta["total_decisions"] * 100, 1)
...
def build_report(decisions: list[dict], wallet_label: str = "unknown") -> dict:
    """Build the full fingerprint report."""
    timestamps = [t["timestamp"] for t in decisions]
    return {
        "meta": {
            "wallet": wallet_label,
        ...
        "fund_flow": analyze_fund_flow(decisions),
        "token_shifts": analyze_token_shifts(decisions),
        "tempo": analyze_tempo(decisions),
        "time_preference": analyze_time_preference(decisions),
        "gas_sensitivity": analyze_gas_sensitivity(decisions),
        "usd_volume": analyze_usd_volume(decisions),
    }


def main():
    parser = argparse.ArgumentParser(
        description="Analyze DeFi behavior fingerprint from whale decision CSV"
    )
    parser.add_argument("csv_path", help="Path to whale decisions CSV (from fetch_whale_decisions.py)")
    parser.add_argument(
        "--output", "-o",
    ...
    print(f"Loading decisions from {csv_path}...")
    decisions = load_decisions(str(csv_path))
    if not decisions:
        print("ERROR: No decisions loaded.")
        sys.exit(1)

    print(f"Loaded {len(decisions)} decisions. Analyzing...")
    report = build_report(decisions, wallet_label=args.wallet)
