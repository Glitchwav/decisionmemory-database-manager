...
def generate_hero(theme_name):
    ...
    tagline = "The Memory Layer for AI DecisionMaking Agents"
...
def generate_before_after(theme_name):
    ...
    # Left flow: Decision → Forget → Decision → Forget → Same Mistakes
    flow_font = get_font(16, bold=True)
    flow_y = card_y + 80
    steps_left = ["Decision", "Forget", "Decision", "Forget", "Same Mistakes"]
    ...
    # Right card (With DecisionMemory) - green tones
    right_x = W - 60 - card_w
    ...
    draw.text((right_x + 30, card_y + 20), "With DecisionMemory",
              fill=hex_to_rgb(t["green"]), font=header_font)

    # Right flow: Decision → Remember → Learn → Adapt → Evolve
    steps_right = ["Decision", "Remember", "Learn", "Adapt", "Evolve"]
    ...
    # Left side of bar
    draw.text((100, bar_y + 14), "Manual decision journaling:", fill=hex_to_rgb(t["text2"]), font=compare_font)
    ...
    # Right side of bar
    draw.text((W//2 + 60, bar_y + 14), "DecisionMemory:", fill=hex_to_rgb(t["text2"]), font=compare_font)
...
def generate_architecture(theme_name):
    ...
    memory_types = [
        ("Episodic", "What happened?", "Decision events\n+ context", t["accent"]),
