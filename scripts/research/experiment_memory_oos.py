...
from decisionmemory.data.context_builder import ContextConfig
from decisionmemory.data.models import Timeframe
from scripts.strategy_definitions import build_strategy_e
from scripts.research.export_backtest_decisions import (
    fast_backtest_with_decisions,
    precompute_contexts,
    precompute_atrs,
)
...
def should_skip_decision(memories, hour_utc, trend_12h_pct, regime, atr_h1, cfg):
    scored = []
    for mem in memories:
...
def evaluate_with_memory(baseline_decisions, test_contexts, test_atrs, memories, cfg):
    skipped_bars = set()
    skip_w = skip_l = 0
    for t in baseline_decisions:
        ...
        if should_skip_decision(memories, ctx.hour_utc, ctx.trend_12h_pct or 0,
    ...
    kept = [t for t in baseline_decisions if t.entry_bar not in skipped_bars]
    skipped_pnl = sum(t.pnl for t in baseline_decisions if t.entry_bar in skipped_bars)

    if not kept:
        return {"decisions": 0, "wr": 0, "pf": 0, "pnl": 0, "sharpe": 0,
    ...
    return {"decisions": len(kept), "wr": wr, "pf": pf, "pnl": total_pnl,
...
def agent_search(baseline_decisions, test_contexts, test_atrs, memories, iterations):
    ...
    for i in range(iterations):
        ...
        m = evaluate_with_memory(baseline_decisions, test_contexts, test_atrs, memories, cfg)
...
# ── Generate memories from training decisions ──────────
def decisions_to_memories(decisions, contexts, atrs):
    memories = []
    for t in decisions:
...
async def main():
    ...
    try:
        for label, train_s, train_e, test_s, test_e in WINDOWS:
            ...
            # Training decisions -> memories
            train_fit, train_decisions = fast_backtest_with_decisions(
                train_data.bars, train_ctx, train_atrs, strategy,
                config=CONTEXT_CONFIG, timeframe="1h"
            )
            memories = decisions_to_memories(train_decisions, train_ctx, train_atrs)
            pos_mem = sum(1 for m in memories if m.pnl_r > 0)
            print(f"  Train: {train_fit.decision_count} decisions, WR {train_fit.win_rate:.0%}, "
                  f"Sharpe {train_fit.sharpe_ratio:.2f}")
            print(f"  Memories: {len(memories)} (pos {pos_mem}, neg {len(memories)-pos_mem})")

            # Baseline on test
            base_fit, base_decisions = fast_backtest_with_decisions(
                test_data.bars, test_ctx, test_atrs, strategy,
                config=CONTEXT_CONFIG, timeframe="1h"
            )
            print(f"\n  Baseline: {base_fit.decision_count} decisions | WR {base_fit.win_rate:.0%} | "
                  f"PF {base_fit.profit_factor:.2f} | Sharpe {base_fit.sharpe_ratio:.2f} | "
                  f"PnL ${base_fit.total_pnl:.2f}")

            if not base_decisions:
                print("  (no decisions, skipping)")
                all_results.append({"window": label, "baseline_decisions": 0, "skip": True})
                continue

            # ── Level 1: Fixed config (no re-optimization) ──
            print(f"\n  [L1] FIXED CONFIG: {fixed_cfg.describe()}")
            l1 = evaluate_with_memory(base_decisions, test_ctx, test_atrs, memories, fixed_cfg)
            ...
            print(f"       Decisions: {l1['decisions']}/{base_fit.decision_count} | "
            ...
            best_cfg, l2, search_log = agent_search(
                base_decisions, test_ctx, test_atrs, memories, AGENT_ITERATIONS
            )
            if l2:
                ...
                print(f"       Decisions: {l2['decisions']}/{base_fit.decision_count} | "
            ...
            all_results.append({
                "window": label,
                "baseline_decisions": base_fit.decision_count,
    ...
    for r in all_results:
        if r.get("skip"):
            print(f"  {r['window']:<40} {'(no decisions)':>12} {'(no decisions)':>12}")
            continue
        total += 1
    ...
    if l1_passes >= total * 0.75:
        ...
        print(f"\n  >> REJECTED: Memory injection does not reliably improve decision_making")
