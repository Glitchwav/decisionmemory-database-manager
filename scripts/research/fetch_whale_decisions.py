#!/usr/bin/env python3
"""Fetch on-chain ERC-20 token transfers for a whale wallet and convert to decision CSV.

Pairs token transfers within the same tx hash into swaps (sell A → buy B).
Output is a CSV compatible with analyze_decision_maker.py.

Usage:
    cd C:/Users/johns/projects/decisionmemory-protocol
    python scripts/research/fetch_whale_decisions.py 0xb99a2c4c1c4f1fc27150681b740396f6ce1cbcf5
    python scripts/research/fetch_whale_decisions.py 0xb99a... --output data/abraxas_decisions.csv
    python scripts/research/fetch_whale_decisions.py 0xb99a... --api-key YOUR_KEY
"""
...
# Stablecoins and wrapped tokens (used to determine decision direction)
STABLECOINS = {
...
def pair_swaps(transfers: list[dict], wallet: str) -> list[dict]:
    ...
    Returns list of decision dicts compatible with analyze_decision_maker.py CSV format.
    """
    ...
    for tx_hash, tx_transfers in by_tx.items():
        ...
        # Determine the "base" token (what was decisiond) vs "quote" (what was used to pay)
...
def write_csv(decisions: list[dict], output_path: Path):
...
def print_summary(decisions: list[dict]):
    """Print a quick summary of extracted decisions."""
    if not decisions:
    ...
    timestamps = [t["open_time"] for t in decisions]
    print(f"\n{'='*60}")
    print(f"WHALE DECISION EXTRACTION SUMMARY")
    print(f"{'='*60}")
    print(f"Total swaps: {len(decisions)}")
    ...
    for sym, count in sorted(symbols.items(), key=lambda x: -x[1])[:10]:
        print(f"  {sym}: {count} decisions")

    total_gas = sum(float(t["gas_cost_eth"]) for t in decisions)
    print(f"\nTotal gas cost: {total_gas:.4f} ETH")


def main():
    parser = argparse.ArgumentParser(description="Fetch whale wallet decisions from Etherscan")
    ...
    output_path = Path(args.output) if args.output else Path(f"data/whale_{address[:8]}_decisions.csv")
    ...
    decisions = pair_swaps(transfers, address)

    print_summary(decisions)
    write_csv(decisions, output_path)

    print(f"\nNext step: python scripts/analyze_decision_maker.py {output_path}")
