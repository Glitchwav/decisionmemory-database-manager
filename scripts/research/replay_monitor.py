...
def get_stats():
    ...
    if DB.exists():
        try:
            ...
            decisions, total_pnl, wins = row[0], row[1], row[2] or 0
            conn.close()
        except Exception:
            pass
    ...
    return decisions, last, decisions, total_pnl, wins, cp
...
def main():
    ...
    while True:
        decisions, last, decisions, total_pnl, wins, cp = get_stats()
        ...
        print(f"\033[K  Decisions: {decisions} | Win rate: {win_rate:.0f}% | PnL: ${total_pnl:+.2f} | Score: {last_eq}")
