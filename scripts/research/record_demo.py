#!/usr/bin/env python3
"""
DecisionMemory Protocol — Rich Demo for Recording
================================================
...
def main():
    ...
    console.print(Panel.fit(
        "[bold white]DecisionMemory Protocol[/bold white]\n"
        "[dim]Persistent memory for AI decision_making agents[/dim]",
        border_style="bright_blue",
        padding=(1, 4),
    ))
    console.print()
    console.print("  Simulating [bold]30 XAUUSD decisions[/bold] over 7 days.", style="dim")
    console.print("  No API key required. All data is simulated.", style="dim")
    console.print()
    time.sleep(1.5)

    # ── Step 1: Record Decisions ──
    console.rule("[bold cyan]Step 1: L1 — Recording decisions to DecisionJournal[/bold cyan]")
    console.print()
    ...
    for i, decision in enumerate(SIMULATED_DECISIONS):
        ...
        journal.record_decision(
            decision_id=decision_id, symbol="XAUUSD",
            direction=decision["direction"], lot_size=0.05,
            strategy=decision["strategy"], confidence=decision["confidence"],
            reasoning=f"Day {decision['day']} {decision['session']} — {decision['strategy']}",
            market_context={"price": decision["entry"], "session": decision["session"]},
        )
        journal.record_outcome(
            decision_id=decision_id, exit_price=decision["exit"],
            pnl=decision["pnl"], pnl_r=decision["pnl_r"],
            exit_reasoning="Target hit" if decision["pnl"] > 0 else "Stop hit",
            hold_duration=random.randint(15, 180),
        )

        result = "[green]WIN[/green]" if decision["pnl"] > 0 else "[red]LOSS[/red]"
        sess_style, sess_label = SESSION_STYLE[decision["session"]]
        pnl_style = "green" if decision["pnl"] > 0 else "red"
        pnl_str = f"${decision['pnl']:+.2f}"
    ...
    console.print(
        f"\n  [bold]Total:[/bold] {len(SIMULATED_DECISIONS)} decisions | "
        f"Winners: {winners} | "
    ...
    with Progress(
        ...
        task = progress.add_task("Analyzing 30 decisions...", total=100)
        for _ in range(100):
            time.sleep(0.02)
    ...
    console.print(f"    Insight: High-confidence decisions win [bold]{hc_wr - lc_wr:.0f}%[/bold] more often")
    time.sleep(1.5)
    ...
    adjustments.append({"param": "min_confidence_threshold", "old": "0.40", "new": "0.55",
                        "reason": f"Decisions below 0.55 have {lc_wr:.0f}% WR — filter out"})
    ...
    ba_table.add_column("Decision", width=10, style="bold")
    ba_table.add_column("Without DecisionMemory", width=35, style="red")
    ba_table.add_column("With DecisionMemory", width=35, style="green")

    ba_table.add_row("Decision 1", "AI analyzes market, gives recommendation", "Same")
    ba_table.add_row("Decision 5", "Starts fresh, no memory of past decisions",
                     '"Past 4 Asian: 3 losses. Reducing lot 50%."')
    ba_table.add_row("Decision 15", "No idea what its win rate is",
                     '"London VolBreakout WR 73%. Full size."')
    ba_table.add_row("Decision 30", "Same mistakes repeated. No learning.",
    ...
    console.print(Panel.fit(
        ...
        f"  Decisions recorded:      [bold]{len(SIMULATED_DECISIONS)}[/bold]\n"
        ...
        f"  Next: [bold]python -m decisionmemory.server[/bold]\n"
        f"  Docs: [link=https://github.com/mnemox-ai/decisionmemory-protocol]github.com/mnemox-ai/decisionmemory-protocol[/link]",
