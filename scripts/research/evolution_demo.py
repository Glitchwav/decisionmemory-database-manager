...
from decisionmemory.evolution.engine import EngineConfig, EvolutionEngine
from decisionmemory.evolution.llm import MockLLMClient
from decisionmemory.evolution.models import EvolutionConfig, FitnessMetrics, Hypothesis
...
def print_hypothesis(h: Hypothesis, rank: int) -> None:
    ...
    print(f"     Decisions: {f.decision_count}  Win rate: {f.win_rate:.1%}  "
...
def print_score_curve(hypotheses: List[Hypothesis]) -> None:
    """Print a simple text score curve for the best hypothesis."""
    ...
    if best is None or best.fitness_is is None:
        print("\n  (no decisions to plot)")
        return

    f = best.fitness_is
    print(f"\n  Best IS strategy: {best.pattern.name}")
    print(f"  Decisions: {f.decision_count} | PnL: ${f.total_pnl:,.2f} | "
          f"Sharpe: {f.sharpe_ratio:.2f}")

    # Simulate score curve from fitness
    # We don't have individual decisions, so create a synthetic curve
    n_points = min(f.decision_count, 20)
    if n_points <= 0:
        print("  (no decisions)")
        return

    avg_pnl = f.total_pnl / f.decision_count
    ...
    for _ in range(n_points):
        # Approximate decision PnL distribution
        if rng.random() < f.win_rate:
            decision_pnl = abs(avg_pnl) * (1 + rng.gauss(0, 0.3))
        else:
            decision_pnl = -abs(avg_pnl) * 0.7 * (1 + rng.gauss(0, 0.3))
        score += decision_pnl
        points.append(score)
...
async def run_demo(seed: int = 42) -> dict:
    ...
    # Relax selection thresholds for demo (mock data won't produce 30+ decisions)
    config.selection.min_is_decision_count = 1
    config.selection.min_oos_decision_count = 1
    ...
    # Score curve
    print_header("Score Curve")
    print_score_curve(run.hypotheses)
