...
Usage:
    ...
    python scripts/research/compare_defi_fingerprints.py \
        data/whale_0xb99a2c_decisions.csv data/whale_0x7a16fF_decisions.csv \
...
from analyze_defi_behavior import build_report, load_decisions
...
def aave_usage_ratio(report: dict) -> float:
    ...
    total_decisions = report.get("meta", {}).get("total_decisions", 0)
    if total_decisions == 0:
        return 0.0
    return round(total_aave / total_decisions * 100, 2)
...
def print_comparison(result: dict) -> None:
    ...
    rows = [
        ("Total decisions", [str(w["total_decisions"]) for w in wallets]),
        ("Date range", [
...
def main():
    ...
    parser.add_argument(
        "csv_paths",
        nargs="+",
        help="Paths to whale decision CSVs (2 or more)",
    )
    parser.add_argument(
        "--labels", "-l",
        nargs="+",
    ...
    for csv_path, label in zip(args.csv_paths, labels):
        ...
        if not decisions:
            print(f"ERROR: No decisions loaded from {path}")
            sys.exit(1)
        print(f"  → {len(decisions)} decisions loaded. Building fingerprint...")
        report = build_report(decisions, wallet_label=label)
