...
async def main():
    ...
    for strat_name, info in strategies.items():
        ...
        orig_decisions = orig_metrics.decision_count
        orig_wr = orig_metrics.win_rate
        print(f"  Original (H{info['original_hour']:02d}): "
              f"Sharpe={orig_sharpe:.4f}, decisions={orig_decisions}, WR={orig_wr:.1%}")
        ...
        for h in range(24):
            ...
            hour_decisions[h] = metrics.decision_count
        ...
        # Print all hours
        print(f"\n  Hour | Sharpe  | Decisions | WR     | vs Original")
        print(f"  -----|---------|--------|--------|------------")
        for h in range(24):
            marker = " <<<" if h == info["original_hour"] else ""
            delta = hour_sharpes[h] - orig_sharpe
            print(f"  H{h:02d}  | {hour_sharpes[h]:>7.4f} | {hour_decisions[h]:>6d} | {hour_wrs[h]:>5.1%} | {delta:>+7.4f}{marker}")
        ...
        results[strat_name] = {
            ...
            "hourly_decisions": {str(h): t for h, t in hour_decisions.items()},
