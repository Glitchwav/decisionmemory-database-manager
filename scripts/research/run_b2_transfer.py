...
from decisionmemory.evolution.random_baseline import (
...
def fast_backtest(
    ...
    return _compute_fitness(decisions, timeframe=timeframe)
...
def fast_backtest_slice(
    ...
    if position is not None:
        ...
        decision = force_close_position(position, last_bar, len(slice_bars) - 1, "end")
        decisions.append(decision)
...
| Decisions | {full_metrics['decisions']} |
...
| Window | Period | Sharpe | Decisions | Win Rate | PnL |
|--------|--------|--------|--------|----------|-----|
"""

    for i, w in enumerate(window_results):
        report += f"| {i+1} | {w['period']} | {w['sharpe']:.4f} | {w['decisions']} | {w['win_rate']:.1%} | {w['total_pnl']:.2f} |\n"
...
"""
    ...
    if verdict == "PASS":
        report += "Strategy E shows genuine cross-asset alpha. The US afternoon momentum pattern (long at 14:00 UTC with positive 12h trend) captures a market microstructure effect that exists in both BTC and ETH. This makes sense — both assets share the same institutional decision_making hours and similar intraday liquidity patterns. The strategy is capturing *crypto market structure*, not BTC-specific dynamics.\n"
        ...
        report += f"Strategy E does NOT transfer to ETHUSDT (P{percentile:.0f}). The US afternoon momentum pattern is BTC-specific, likely driven by BTC-dominant institutional flows (ETF inflows, CME basis decisions) that don't directly affect ETH. This is actually useful information — it tells us the Evolution Engine found a *real* BTC-specific signal, not just generic trend-following.\n"

    report += "\n## Business Advisor\n"
    if verdict == "PASS":
        report += "Strong product signal: the Evolution Engine finds patterns that generalize across crypto assets. This means you can market multi-asset support with confidence. One evolution run on BTC could seed strategies for the entire crypto collection.\n"
    elif verdict == "MARGINAL":
...
async def main():
    ...
    full_results = {
        ...
        "decisions": full_metrics.decision_count,
    ...
    print(f"  Sharpe={full_metrics.sharpe_ratio:.4f}, decisions={full_metrics.decision_count}, "
    ...
    for i, (w_start, w_end) in enumerate(windows):
        ...
        result = {
            ...
            "decisions": w_metrics.decision_count,
        ...
        print(f"  Window {i+1}: {period} | Sharpe={w_metrics.sharpe_ratio:.4f}, "
              f"decisions={w_metrics.decision_count}, WR={w_metrics.win_rate:.1%}")
