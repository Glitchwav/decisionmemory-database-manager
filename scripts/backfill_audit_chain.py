"""One-shot backfill: rebuild the audit_chain + audit_roots from decision_records.
...
rebuilding from scratch. The decision_records table is never modified. Run on a
...
from decisionmemory.audit.chain import ChainBuilder  # noqa: E402
from decisionmemory.db import Database  # noqa: E402
from decisionmemory.domain.tdr import DecisionMakingDecisionRecord  # noqa: E402


def _decision_iter(db: Database):
    ...
    for row in rows:
        ...
        # chained_at == the original decision timestamp, normalised to UTC ISO.
...
def main() -> int:
    ...
    print(f"decision_records: {total} rows")
    ...
    with db.get_connection() as conn:
        ...
        summary = builder.backfill(_decision_iter(db))
    print(
