"""Export XAUUSD M15 historical data from MetaDecisionMaker 5 to CSV.
...
Requires: MetaDecisionMaker5 pip package + running MT5 terminal.
...
try:
    ...
    print("ERROR: pip install MetaDecisionMaker5")
    sys.exit(1)
...
def export(symbol: str, from_date: datetime, to_date: datetime, output_dir: str) -> str:
    ...
    if not mt5.initialize():
        ...
        print("Make sure MetaDecisionMaker 5 terminal is running.")
        sys.exit(1)
    ...
    decision_making_days = len(set(datetime.fromtimestamp(r["time"]).date() for r in rates))

    print(f"\nExported: {outpath}")
    print(f"Bars: {len(rates)}")
    print(f"Period: {first_dt.date()} to {last_dt.date()}")
    print(f"DecisionMaking days: {decision_making_days}")
