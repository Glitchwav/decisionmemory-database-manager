"""
MT5 Sync Script v2 - Non-invasive decision synchronization
監控本地 MT5 terminal，將交易記錄同步到 DecisionMemory
...
- Broader exception handling in sync_decision_to_memory
...
# Add scripts/ to path so we can import decision_advisor
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from decision_advisor import advise_on_open, send_discord_alert
...
# Each EA instance on MT5 uses a unique MagicNumber to identify its decisions
MAGIC_TO_STRATEGY = {
...
def init_mt5() -> bool:
    ...
    try:
        ...
        log.error("MetaDecisionMaker5 package not installed. Run: pip install MetaDecisionMaker5")
        return False
    except Exception as e:
...
def get_new_closed_decisions(last_synced_ticket: int) -> Tuple[list, bool]:
    """
    Get newly closed decisions since last sync.

    Args:
        last_synced_ticket: Position ID of the last synced decision
...
def check_new_open_positions():
    """
    Detect newly opened positions and run decision advisor.
...
def sync_decision_to_memory(position: Dict[str, Any]) -> bool:
    """
    Sync one closed position to DecisionMemory.
    ...
    try:
        ...
        if decision_resp.status_code != 200:
            log.error(f"Failed to record decision for {decision_id}: {decision_resp.status_code} {decision_resp.text[:200]}")
            return False
        ...
        if outcome_resp.status_code != 200:
            log.error(f"Failed to record outcome for {decision_id}: {outcome_resp.status_code} {outcome_resp.text[:200]}")
            return False

        log.info(f"SYNC {decision_id}: {strategy} {symbol} {direction} {lot_size} lots, P&L: ${pnl:.2f}, Duration: {hold_duration}min")
        ...
        log.error(f"API network error for {decision_id}: {e}")
        return False
    except (ValueError, KeyError) as e:
        log.error(f"API response parsing error for {decision_id}: {e}")
        return False
    except Exception as e:
...
def main_loop():
    ...
    log.info("=" * 60)
    log.info("MT5 → DecisionMemory Sync Script v2")
    log.info("=" * 60)
    log.info(f"API Endpoint: {DECISIONMEMORY_API}")
    ...
    daily_sync_count = 0     # Decisions synced today
    ...
    try:
        while True:
            try:
                ...
                # Check for new OPEN positions → run decision advisor
                try:
                    check_new_open_positions()
                except Exception as e:
                    log.error(f"[ADVISOR] check_new_open_positions error: {e}")

                # Check for new CLOSED decisions (with timeout protection)
                new_decisions, timed_out = get_new_closed_decisions(last_synced_ticket)
                ...
                if len(new_decisions) > 0:
                    log.info(f"Found {len(new_decisions)} new closed decision(s)")
                    synced_count = 0

                    for position in new_decisions:
                        if sync_decision_to_memory(position):
                            synced_count += 1
                            last_synced_ticket = max(last_synced_ticket, position['ticket'])

                    # Save state AFTER successful syncs
                    save_state(last_synced_ticket)
                    log.info(f"Sync complete. {synced_count}/{len(new_decisions)} synced. Last ticket: {last_synced_ticket}")
                ...
                if today_utc != last_summary_date:
                    if daily_sync_count > 0:
                        ...
                        send_discord(
                            ...
                            f"Decisions synced: **{daily_sync_count}**\n"
                            f"Total P&L: **${daily_pnl:+.2f}**",
                            color=0xF39C12,
                        )
                        log.info(f"[DAILY SUMMARY] {last_summary_date}: {daily_sync_count} decisions, P&L=${daily_pnl:+.2f}")
