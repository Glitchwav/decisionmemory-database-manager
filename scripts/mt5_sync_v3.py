...
# Add scripts/research/ to path for decision_advisor (moved during 2026-03-19 repo reorg)
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), "research"))
from decision_advisor import advise_on_open, send_discord_alert, recall_similar, get_behavioral
...
class EventLogReader:
    """Reads NG_Gold event_log CSVs to extract decision entry context.

    Matching: position_id from MT5 deal == pos_id in DECISION_OPEN event.
    ...
    def find_entry_context(
        ...
        # Search for DECISION_OPEN with matching pos_id (most recent files first)
    ...
    def _search_csv_for_position(
        ...
        """Search a single CSV for DECISION_OPEN with matching pos_id."""
        try:
            with open(csv_path, "r", encoding="utf-8", errors="replace") as f:
                reader = csv.DictReader(f)
                decision_open_row = None
                decision_row = None
                ...
                for row in reader:
                    ...
                    # Match DECISION_OPEN by pos_id
                    if evt == "DECISION_OPEN" and f"pos_id={pos_id_str}" in reason:
                        decision_open_row = row
                ...
                if decision_open_row:
                    return self._build_context(decision_open_row, decision_row)

        except Exception as e:
            log.warning(f"EventLogReader: error reading {csv_path.name}: {e}")
        return None

    def _build_context(self, decision_open: dict, decision: Optional[dict]) -> dict:
        """Build rich context from event_log rows."""
        direction = (decision_open.get("decision") or "").strip()
        atr_m5 = self._safe_float(decision_open.get("atr_m5"))
        spread_pts = self._safe_int(decision_open.get("spread_points"))
        bid = self._safe_float(decision_open.get("bid"))
        ask = self._safe_float(decision_open.get("ask"))
        ema_fast_h1 = self._safe_float(decision_open.get("ema_fast_h1"))
        ema_slow_h1 = self._safe_float(decision_open.get("ema_slow_h1"))
        ema_fast_m30 = self._safe_float(decision_open.get("ema_fast_m30"))
        ema_slow_m30 = self._safe_float(decision_open.get("ema_slow_m30"))
        ...
        if decision:
            ...
            # Context from DECISION_OPEN market data
        ...
        if decision:
            ...
            if spread_pts is not None and atr_m5 and atr_m5 > 0:
                ...
                spread_price = self._safe_float(decision_open.get("spread_price")) or 0
...
class DecisionLogReader:
    """Reads NG_Gold decision_log JSONL files for rich decision decision context.
    ...
    def find_decision_for_decision(
        self, strategy: str, entry_time: int, exec_ticket: int = 0
    ) -> Optional[dict]:
        """Find the EXECUTED decision event matching a decision.
        ...
        # Strategy names in JSONL match DecisionMemory strategy names directly
    ...
    def _build_rich_context(self, evt: dict) -> dict:
        ...
        market_data = {
            ...
            "account_score": evt.get("account_score"),
...
def _get_contract_size(symbol: str) -> float:
    ...
    try:
        ...
        if info and info.decision_contract_size > 0:
            return info.decision_contract_size
    except Exception:
        pass
...
def send_discord(message: str, color: int = 0x00FF00):
    ...
    try:
        payload = {
            "embeds": [{
                "title": "DecisionMemory — MT5 Sync v3",
                "description": message,
                "color": color,
                "timestamp": _now_iso(),
            }]
...
class MT5Poller:
    ...
    def init_mt5(self) -> bool:
        ...
        try:
            ...
            log.error("MetaDecisionMaker5 package not installed. pip install MetaDecisionMaker5")
            return False
        except Exception as e:
    ...
    def _sync_open_positions(self):
        ...
        Detect new opens → sync_log + decision advisor.
        Detect closes → remove from table + sync to DecisionMemory.
        """
        import MetaDecisionMaker5 as MT5
        ...
        with get_db() as conn:
            ...
            for pos in positions:
                ...
                # Decision advisor (non-blocking)
                self._run_decision_advisor(pos, strategy, direction)
            ...
            for ticket in closed_tickets:
                ...
                # NOTE: Do NOT delete here — only after successful sync to DecisionMemory

            closed_data = list(closed_tickets)

        # Sync closed decisions to DecisionMemory (after DB commit)
        if closed_data:
            synced_tickets = self._sync_closed_decisions_to_memory(closed_data)
    ...
    def _run_decision_advisor(self, pos, strategy: str, direction: str):
        """Send new-open Discord embed + run decision advisor warnings."""
        try:
            ...
            recall_text = "No similar decisions found"
            behavioral_text = "N/A"
            ...
            # --- Decision advisor warning check ---
    ...
    # --- Closed decision sync (history-based, same as v2) ---

    def _sync_closed_decisions_to_memory(self, closed_tickets: list[int]) -> list[int]:
        """
        For each closed ticket, look up in MT5 history_deals_get
        and POST to DecisionMemory record_decision + record_outcome.
        ...
        if timed_out or history is None:
            log.error("Cannot fetch deal history for closed decision sync")
            return synced
        ...
        with get_db() as conn:
            for ticket in closed_tickets:
                ...
                success = self._post_decision_to_memory(ticket, deals_sorted)
                ...
                if success:
                    ...
                    _log_event(conn, "DECISION_CLOSED",
                               f"ticket={ticket} synced to DecisionMemory")

        return synced

    def _post_decision_to_memory(self, ticket: int, deals: list) -> bool:
        """POST record_decision + record_outcome to DecisionMemory API."""
        entry_deal = deals[0]
        exit_deal = deals[-1]

        decision_id = f"MT5-{ticket}"
        ...
        decision_ctx = decision_log_reader.find_decision_for_decision(
            strategy=strategy,
        ...
        if decision_ctx and decision_ctx["matched"]:
            ...
            log.info(f"DecisionLog JSONL matched for {decision_id}: confidence={confidence}")
        else:
            ...
            if entry_ctx["matched"]:
                log.info(f"EventLog CSV matched for {decision_id}: confidence={confidence}")
        ...
        # --- (2) Close Discord embed (fire before DecisionMemory API) ---
        ...
        try:
            # Task 0.5: References backfill — recall similar decisions before recording
            references = []
            try:
                ...
                log.debug(f"recall_similar failed for {decision_id}: {e}")
            ...
            if resp1.status_code != 200:
                log.error(f"record_decision failed for {decision_id}: {resp1.status_code}")
                return False
            ...
            if sl_distance > 0 and lot_size > 0:
                ...
                if risk_dollars > 0:
                    ...
                    log.info(
                        f"pnl_r={pnl_r:.4f} for {decision_id} "
            ...
            if resp2.status_code != 200:
                log.error(f"record_outcome failed for {decision_id}: {resp2.status_code}")
                return False

            log.info(
                f"SYNC {decision_id}: {strategy} {symbol} {direction} "
                f"{lot_size} lots, P&L: ${pnl:.2f}, Hold: {hold_duration}min"
            )
            return True

        except requests.exceptions.RequestException as e:
            log.error(f"Network error syncing {decision_id}: {e}")
            return False
        except Exception as e:
            log.error(f"Unexpected error syncing {decision_id}: {e}")
            return False
    ...
    def _poll_loop(self):
        ...
        log.info(f"Interval: {SYNC_INTERVAL}s | API: {DECISIONMEMORY_API}")
        log.info("=" * 60)
...
def recent_decisions() -> list[dict]:
    with get_db() as conn:
        rows = conn.execute(
            "SELECT * FROM sync_log WHERE event_type = 'DECISION_CLOSED' "
