#!/usr/bin/env python3
"""Strategy E live paper decision_making executor.

Runs hourly via GitHub Actions. Checks BTCUSDT 1H bars from Binance,
manages paper positions in Supabase (live_positions / live_decisions tables).

Env vars required:
  SUPABASE_URL
  SUPABASE_SERVICE_ROLE_KEY
Optional:
  DECISIONMEMORY_API_URL  (for OWM memory on close)
"""
...
from decisionmemory.data.context_builder import build_context, compute_atr
from decisionmemory.data.models import Timeframe
from decisionmemory.evolution.backtester import check_entry

USER_AGENT = "decisionmemory-live-executor/1.0 (github.com/mnemox-ai/decisionmemory-protocol)"
...
def fire_and_forget_owm(pos: dict, exit_info: dict):
    """POST decision memory to OWM API (best-effort)."""
    api_url = os.environ.get("DECISIONMEMORY_API_URL")
    if not api_url:
        return
...
async def main():
    ...
    if pos:
        ...
        if exit_info:
            ...
            sb.table("live_decisions").insert({
                "strategy_id": STRATEGY_ID,
                "symbol": "BTCUSDT",
                "direction": pos["direction"],
                "entry_price": pos["entry_price"],
            ...
            print("  Position closed, decision recorded.")
