#!/usr/bin/env python3
"""Analyze decision_maker behavior from CSV decision history.
...
Usage:
    ...
    python scripts/analyze_decision_maker.py decisions.csv
    python scripts/analyze_decision_maker.py decisions.csv --symbol XAUUSD
"""
...
COLUMN_ALIASES = {
    ...
    "side": ["side", "direction", "type", "order_type", "decision_type", "action"],
    ...
    "symbol": ["symbol", "instrument", "pair", "subject", "asset"],
...
def get_session(hour: int) -> str:
    """Get decision_making session name from UTC hour."""
...
def load_decisions(csv_path: str, symbol_override: str | None = None) -> list[dict]:
    """Load and normalize decisions from CSV."""
    path = Path(csv_path)
    if not path.exists():
    ...
    if any(t.get("open_time") for t in decisions):
        decisions.sort(key=lambda x: x.get("open_time") or datetime.min.replace(tzinfo=timezone.utc))
...
def compute_stats(decisions: list[dict]) -> dict:
    ...
    # PnL stats
    pnls = [t["pnl"] for t in decisions if t.get("pnl") is not None]
    ...
    if pnls:
        ...
        stats["best_decision"] = max(pnls)
        stats["worst_decision"] = min(pnls)
    ...
    # Max drawdown (sequential score curve)
    if pnls:
        score = 0
        peak = 0
        max_dd = 0
        ...
        for i, p in enumerate(pnls):
            ...
            if dd > max_dd:
                max_dd = dd
                dd_decisions = (current_dd_start, i)
        stats["max_drawdown"] = max_dd
        stats["max_drawdown_decisions"] = dd_decisions
    ...
    # Post-loss behavior: does decision_maker increase size after losses?
    post_loss_behavior = {"size_after_loss": [], "size_after_win": [], "size_normal": []}
    for i in range(1, len(decisions)):
        prev_pnl = decisions[i - 1].get("pnl")
        curr_vol = decisions[i].get("volume")
...
def generate_report(decisions: list[dict], stats: dict, symbol: str) -> str:
    ...
    # --- Section 1: DecisionMaking Overview ---
    lines.append("## 一、交易概覽")
    lines.append("")

    lines.append("| 指標 | 值 |")
    lines.append("|------|-----|")
    lines.append(f"| 總交易數 | {stats['total_decisions']} |")
    ...
    if stats.get("has_pnl"):
        ...
        lines.append(f"| 最佳單筆 | {format_pnl(stats['best_decision'])} |")
        lines.append(f"| 最差單筆 | {format_pnl(stats['worst_decision'])} |")
    ...
    if stats.get("has_pnl"):
        ...
        if max_loss_streak >= 3:
            # Analyze PnL of decisions right after streaks of 3+
            pnls = [t.get("pnl") for t in decisions if t.get("pnl") is not None]
            post_streak_pnls = []
            streak = 0
            for i, p in enumerate(pnls):
                if p < 0:
    ...
    lines.append("")
    lines.append("---")
    lines.append(f"*由 DecisionMemory analyze_decision_maker.py 生成 | {date_str}*")
...
def main():
    ...
    parser.add_argument("--output", default=None, help="輸出檔案路徑（預設 reports/decision_maker_fingerprint_{symbol}_{date}.md）")
    args = parser.parse_args()

    print(f"讀取 CSV: {args.csv_path}")
    decisions = load_decisions(args.csv_path, args.symbol)
    print(f"載入 {len(decisions)} 筆交易")

    # Detect symbol
    symbols = set(t.get("symbol") for t in decisions if t.get("symbol"))
    symbol = args.symbol or (symbols.pop() if len(symbols) == 1 else "MIXED") if symbols else "UNKNOWN"

    stats = compute_stats(decisions)
    report = generate_report(decisions, stats, symbol)
    ...
    if args.output:
        ...
        output_path = output_dir / f"decision_maker_fingerprint_{safe_symbol}_{date_str}.md"
