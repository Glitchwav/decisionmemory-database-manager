...
DATA_SOURCES = {
    "decisionmemory_db": {
        "type": "sqlite",
        "path": Path.home() / "projects" / "decisionmemory-protocol" / "data" / "decisionmemory.db",
        "parser": "parse_decisionmemory_db",
    },
    "mt5_sync_db": {
        "type": "sqlite",
        "path": Path.home() / "projects" / "decisionmemory-protocol" / "scripts" / "mt5_sync_v3.db",
        "parser": "parse_mt5_sync_db",
    },
    "mt5_sync_state": {
        "type": "json",
        "path": Path.home() / "projects" / "decisionmemory-protocol" / "scripts" / "mt5_sync_state.json",
    ...
    "decisionmemory_roadmap": {
        "type": "file",
        "path": Path.home() / "projects" / "decisionmemory-protocol" / "docs" / "ROADMAP.md",
        "parser": "parse_decisionmemory_roadmap",
    },
    "decisionmemory_health": {
        "type": "sqlite",
        "path": Path.home() / "projects" / "decisionmemory-protocol" / "data" / "decisionmemory.db",
        "parser": "parse_decisionmemory_health",
    },
    "mt5_sync_live": {
        "type": "http",
...
def parse_decisionmemory_db(src: dict) -> str:
    db = str(src["path"])
    lines = ["## DecisionMemory DB", ""]

    # decision_records summary
    decisions = _query(db, "SELECT COUNT(*) as cnt FROM decision_records")
    total = decisions[0]["cnt"]
    pnl_rows = _query(db, "SELECT SUM(pnl) as total_pnl, AVG(pnl) as avg_pnl FROM decision_records WHERE pnl IS NOT NULL")
    total_pnl = pnl_rows[0]["total_pnl"] or 0
    avg_pnl = pnl_rows[0]["avg_pnl"] or 0

    lines.append(f"- **Decision Records**: {total} 筆 | Total PnL: ${total_pnl:+.2f} | Avg: ${avg_pnl:+.2f}")

    # recent 5 decisions
    recent = _query(db, """
        SELECT symbol, direction, strategy, pnl, exit_timestamp
        FROM decision_records ORDER BY exit_timestamp DESC LIMIT 5
    """)
    if recent:
...
def parse_decisionmemory_health(src: dict) -> str:
    db = str(src["path"])
    lines = ["## DecisionMemory 健康度", ""]

    # --- decision_records ---
    if _table_exists(db, "decision_records"):
        total = _query(db, "SELECT COUNT(*) as cnt FROM decision_records")[0]["cnt"]
        last_ts_row = _query(db, "SELECT MAX(timestamp) as ts FROM decision_records")
...
def parse_decisionmemory_roadmap(src: dict) -> str:
    text = Path(src["path"]).read_text(encoding="utf-8")
    lines = ["## DecisionMemory Roadmap Progress", ""]
...
GIT_PROJECTS = [
    {
        "name": "decisionmemory-protocol",
        "path": Path.home() / "projects" / "decisionmemory-protocol",
