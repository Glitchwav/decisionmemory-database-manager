...
def generate_daily_reflection(target_date: date = None) -> str:
    ...
    try:
        response = requests.post(
            f"{DECISIONMEMORY_API}/reflect/run_daily",
...
def generate_weekly_reflection(week_ending: date = None) -> str:
    ...
    try:
        response = requests.post(
            f"{DECISIONMEMORY_API}/reflect/run_weekly",
            params=params,
            timeout=60
        )
...
def generate_monthly_reflection(year: int = None, month: int = None) -> str:
    ...
    try:
        response = requests.post(
            f"{DECISIONMEMORY_API}/reflect/run_monthly",
            params=params,
            timeout=60
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="DecisionMemory Reflection Generator")
    ...
    print("=" * 60)
    print("DecisionMemory Reflection Generator")
    print("=" * 60)
